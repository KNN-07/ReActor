...
enum RgOutput {
	Buffered(BufWriter<reactor_uutils_ctx::CtxStdout>),
	Direct(reactor_uutils_ctx::CtxStdout),
}
...
struct RgWalk {
	request: reactor_walker::WalkRequest,
	filters: PathFilters,
}
...
impl PathFilters {
	fn includes(&self, path: &Path, file_type: reactor_walker::FileType, size: Option<f64>) -> bool {
		let is_dir = file_type == reactor_walker::FileType::Dir;
		...
		if file_type != reactor_walker::FileType::File {
			return true;
		}
		if !explicitly_included
			&& self
...
fn build_path_filters(cli: &RgCli) -> Result<PathFilters, String> {
	let cwd = reactor_uutils_ctx::cwd();
...
fn build_walk(cli: &RgCli, root: &Path) -> Result<RgWalk, String> {
	...
	let request = reactor_walker::WalkRequest::new(root)
		.hidden(include_hidden)
		.gitignore(!no_ignore)
		.skip_git(!no_ignore)
		.skip_node_modules(false)
		.follow_links(reactor_walker::FollowLinks::from(cli.follow && !cli.no_follow))
		.detail(if filters.max_filesize.is_some() {
			reactor_walker::WalkDetail::Full
		} else {
			reactor_walker::WalkDetail::Minimal
		})
		.order(order)
		.emit_root(false)
		.depth(1, cli.max_depth.unwrap_or(usize::MAX))
		.visit_order(reactor_walker::VisitOrder::PreOrder)
...
fn report_path_error(
	...
	if !opts.no_messages {
		...
		let _ = writeln!(reactor_uutils_ctx::stderr(), "rg: {name}: {err}");
	}
	true
}
...
fn search_collected_files<M: Matcher, W: Write>(
	...
	let mut files = match collect_filtered_files(cli, root) {
		Ok(files) => files,
		Err(_) if reactor_uutils_ctx::is_cancelled() => {
			return SearchOutcome { any_match: false, had_error: true };
		},
		Err(err) => {
			if !opts.no_messages {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "{err}");
			}
			return SearchOutcome { any_match: false, had_error: true };
		},
	};
	...
	for path in files {
		...
		if processed_file && reactor_uutils_ctx::is_cancelled() {
			had_error = true;
			break;
		}
		processed_file = true;
		...
		if reactor_uutils_ctx::is_cancelled() {
			had_error = true;
			break;
		}
	}
	...
	let streamed = match walk.request.for_each_entry_with_heartbeat(
		...
		|entry| {
			if opts.quiet && any_match.get() {
				return Ok(reactor_walker::WalkDecision::Stop);
			}
			let path = entry.absolute_path.as_ref();
			if !walk.filters.includes(path, entry.file_type, entry.size) {
				return Ok(if entry.file_type == reactor_walker::FileType::Dir {
					reactor_walker::WalkDecision::SkipDescend
				} else {
					reactor_walker::WalkDecision::Skip
				});
			}
			if entry.file_type != reactor_walker::FileType::File {
				return Ok(reactor_walker::WalkDecision::Skip);
			}
			let display_path = display_path(operand, root, path);
			let display_bytes = display_path.as_os_str().as_encoded_bytes().to_vec();
			let display = (show_names || opts.json).then_some(display_bytes.as_slice());
			...
			Ok(if opts.quiet && any_match.get() {
				reactor_walker::WalkDecision::Stop
			} else {
				reactor_walker::WalkDecision::Include
			})
		},
		|error| {
			had_error.set(true);
		...
		Err(reactor_walker::WalkError::Interrupted(_)) if reactor_uutils_ctx::is_cancelled() => {
		...
		Err(err) => {
			had_error.set(true);
			if !opts.no_messages {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "rg: {err}");
			}
			Some(SearchOutcome { any_match: any_match.get(), had_error: had_error.get() })
		},
	};
...
fn list_files<W: Write>(cli: &RgCli, paths: &[OsString], out: &mut W) -> SearchOutcome {
	...
	for operand in paths {
		if processed_operand && reactor_uutils_ctx::is_cancelled() {
			had_error = true;
			break;
		}
		processed_operand = true;
		let resolved = reactor_uutils_ctx::resolve(operand);
...
fn execute_search<M: Matcher, W: Write>(
	...
	let mut auto_searcher = match build_searcher(cli, opts, BinaryMode::Automatic) {
		...
		Err(error) => {
			let _ = writeln!(reactor_uutils_ctx::stderr(), "rg: {error}");
			return 2;
		},
	};
	...
	let recursive = paths.iter().any(|path| {
		path.as_os_str() != OsStr::new("-")
			&& std::fs::metadata(reactor_uutils_ctx::resolve(path)).is_ok_and(|meta| meta.is_dir())
	});
		...
		if operand.as_os_str() == OsStr::new("-") {
			...
			match process_reader(
				...
				Err(error) => {
					had_error = true;
					if !opts.no_messages {
						let _ = writeln!(reactor_uutils_ctx::stderr(), "rg: <stdin>: {error}");
					}
				},
			}
			if reactor_uutils_ctx::is_cancelled() {
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	let mut out = if cli.line_buffered && !cli.no_line_buffered {
		RgOutput::Direct(reactor_uutils_ctx::stdout())
	} else {
		RgOutput::Buffered(BufWriter::new(reactor_uutils_ctx::stdout()))
	};
	let (patterns, mut paths) = match resolve_patterns(&cli) {
		Ok(resolved) => resolved,
		Err(error) => {
			let _ = writeln!(reactor_uutils_ctx::stderr(), "{error}");
			return 2;
		},
	};
	if cli.type_list {
	...
	default_paths(
		&mut paths,
		!cli.files && !pattern_stdin_consumed && reactor_uutils_ctx::stdin_is_search_input(),
	);
	if cli.files {
...
mod tests {
	...
	use reactor_uutils_ctx::{ScopeIo, scope};
	...
	fn recursive_search_observes_scope_cancellation() {
		...
		// heartbeat to reactor_walker, so cancellation was not observed during
