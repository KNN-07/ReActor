...
#[command(
	name = "grep",
	version = concat!("grep (reactor-uu-grep) ", env!("CARGO_PKG_VERSION")),
...
fn grep_walk_request(
	...
	reactor_walker::WalkRequest::new(root)
		...
		.order(reactor_walker::WalkOrder::Unordered)
		.emit_root(true)
		.depth(0, usize::MAX)
		...
		.cache(false)
		.filter(reactor_walker::WalkFilter::all())
}
...
fn search_dir<M: Matcher, W: Write>(
	...
	let walk = request.for_each_entry_with_heartbeat(
		...
		|entry: reactor_walker::EntryMeta<'_>| {
			...
			if entry.file_type == reactor_walker::FileType::Dir {
		...
		|error: reactor_walker::DirectoryError<'_>| {
	...
	*had_error |= had_error_state.get();
	match walk {
		Ok(reactor_walker::WalkStatus::Complete | reactor_walker::WalkStatus::Stopped) => any,
		...
		Err(reactor_walker::WalkError::Interrupted(error)) => {
			*had_error = true;
			if !opts.no_messages {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "grep: {error}");
			}
			any
		},
...
fn execute_search<M: Matcher>(
	...
	let mut out = BufWriter::new(reactor_uutils_ctx::stdout());
		...
		if operand == OsStr::new("-") {
			...
			match process_reader(
				...
				Err(error) => {
					had_error = true;
					if !opts.no_messages {
						let _ = writeln!(reactor_uutils_ctx::stderr(), "grep: (standard input): {error}");
					}
				},
			}
			if reactor_uutils_ctx::is_cancelled() {
...
mod tests {
	...
	fn version_flag_prints_and_exits_zero() {
		...
		assert!(
			stdout.contains("grep") && stdout.contains("reactor-uu-grep"),
	...
	fn recursive_search_observes_scope_cancellation() {
		...
		// reactor_walker, so directory walks ignored the uutils cancel flag and the
