...
fn collect_ranked_matches(
	...
	let outcome = request
		.collect_ranked_with_heartbeat(
			reactor_walker::WalkRank::MtimeDescPathAsc,
			config.max_results,
			|| ct.heartbeat(),
		)
...
fn run_glob(
	...
	let walk_glob = reactor_walker::CompiledWalkGlob::new([walk_glob_pattern])
	...
	let base_request = reactor_walker::WalkRequest::new(config.root.clone())
...
pub fn glob(
	...
	task::blocking("glob", ct, move |ct| {
		run_glob(
			GlobConfig {
				root: reactor_walker::resolve_search_path(&path).map_err(iofs::map_walker_error)?,
