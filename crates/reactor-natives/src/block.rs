...
impl From<reactor_ast::block::BlockRange> for BlockRange {
	fn from(value: reactor_ast::block::BlockRange) -> Self {
...
pub fn block_range_at(options: BlockRangeOptions) -> Result<Option<BlockRange>> {
	reactor_ast::block::block_range_at(reactor_ast::block::BlockRangeOptions {
		code: options.code,
		lang: options.lang,
		path: options.path,
		line: options.line,
...
pub fn enclosing_block_boundaries(options: EnclosingBoundaryOptions) -> Result<Option<Vec<u32>>> {
	reactor_ast::block::enclosing_block_boundaries(reactor_ast::block::EnclosingBoundaryOptions {
		code:   options.code,
		lang:   options.lang,
		path:   options.path,
		ranges: options
			.ranges
			.into_iter()
			.map(|range| reactor_ast::block::LineRange {
