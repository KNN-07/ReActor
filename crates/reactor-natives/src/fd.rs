...
fn fuzzy_find_sync(config: FuzzyFindConfig, ct: task::CancelToken) -> Result<FuzzyFindResult> {
	let root = reactor_walker::resolve_search_path(&config.path).map_err(iofs::map_walker_error)?;
	...
	let outcome = reactor_walker::WalkRequest::new(root)
		.hidden(include_hidden)
		.gitignore(respect_gitignore)
		.skip_git(true)
		.skip_node_modules(true)
		.follow_links(reactor_walker::FollowLinks::Always)
...
mod tests {
	...
	fn fuzzy_find_ranks_shallow_paths_first_on_score_tie() {
		...
		fs::create_dir_all(root.path().join(".reactor/skills/opt/scripts"))
