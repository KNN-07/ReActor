...
fn build_grep_walk_request(
	...
	order: reactor_walker::WalkOrder,
) -> Result<reactor_walker::WalkRequest> {
	let mut filter = reactor_walker::WalkFilter::files_only();
	if let Some(glob) = glob.map(str::trim).filter(|value| !value.is_empty()) {
		let pattern = glob_util::build_glob_pattern(glob, true);
		let compiled = reactor_walker::CompiledWalkGlob::new([pattern])
			.map_err(|err| Error::from_reason(format!("Invalid glob pattern: {err}")))?;
		filter = filter.glob(compiled);
	}

	Ok(reactor_walker::WalkRequest::new(search_path)
		.hidden(include_hidden)
		.gitignore(use_gitignore)
		.skip_git(true)
		.skip_node_modules(skip_node_modules)
		.follow_links(reactor_walker::FollowLinks::Never)
		.detail(reactor_walker::WalkDetail::Minimal)
		.size_hints(reactor_walker::SizeHintPolicy::WhenCheap)
		.order(order)
		.emit_root(false)
		.depth(1, usize::MAX)
		.directory_errors(reactor_walker::DirectoryErrorMode::SkipSkippable)
		.cache(false)
		.filter(filter))
}
...
fn collect_grep_candidates(
	...
	order: reactor_walker::WalkOrder,
	ct: &task::CancelToken,
) -> Result<Option<Vec<reactor_walker::FileCandidate>>> {
...
struct PassState {
	...
	deferred:          Mutex<Vec<reactor_walker::FileCandidate>>,
...
fn search_one_file<M: Matcher + Sync>(
	...
	matcher: &M,
	file: &reactor_walker::FileCandidate,
...
fn run_pass<M: Matcher + Sync>(
	candidates: &[reactor_walker::FileCandidate],
	matcher: &M,
	file_params: SearchParams,
	policy: ReadPolicy,
	...
	if parallel_allowed && reactor_walker::should_parallelize(candidates.len()) {
		reactor_walker::execute_candidates_init(
			candidates,
...
fn process_candidates<M: Matcher + Sync>(
	candidates: Vec<reactor_walker::FileCandidate>,
	matcher: &M,
	params: SearchParams,
	parallel_allowed: bool,
	stop_after_matches: Option<u64>,
	...
	request
		.for_each_file_candidate_parallel(
			|file| {
				...
				{
					return Ok(reactor_walker::ParallelWalkControl::Continue);
				}
				...
				Ok(reactor_walker::ParallelWalkControl::Continue)
			},
			|| ct.heartbeat(),
		)
...
fn flush_stream_window<M: Matcher + Sync>(
	window: &mut Vec<reactor_walker::FileCandidate>,
	...
	request
		.for_each_entry_with_heartbeat(
			...
			|entry| {
				...
				window.push(reactor_walker::FileCandidate {
			...
			|_| Ok(reactor_walker::WalkDecision::Include),
		)
		.map_err(iofs::map_walker_error)?;
