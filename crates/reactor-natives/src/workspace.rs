...
fn build_workspace_walk_request(config: &WorkspaceConfig) -> reactor_walker::WalkRequest {
	reactor_walker::WalkRequest::new(config.root.clone())
		.hidden(config.include_hidden)
		.gitignore(config.use_gitignore)
		.skip_git(true)
		.skip_node_modules(true)
		.follow_links(reactor_walker::FollowLinks::Never)
		.detail(reactor_walker::WalkDetail::Full)
...
fn glob_match_from_path(root: &Path, path: &Path) -> Option<GlobMatch> {
	let relative = reactor_walker::normalize_relative_path(root, path);
	if relative.is_empty() {
		return None;
	}
	let (file_type, mtime, size) = reactor_walker::classify_file_type(path)?;
	Some(GlobMatch {
