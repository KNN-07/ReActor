...
/// App name used as the XDG-root subdirectory (`$XDG_STATE_HOME/reactor/`),
/// matching `APP_NAME` in `packages/utils/src/dirs.ts`.
#[cfg(any(target_os = "linux", target_os = "macos"))]
const APP_NAME: &str = "reactor";
...
fn panic_disposition() -> PanicDisposition {
	if blocking_task_panic_scope_active() || reactor_uutils_ctx::is_active() {
		PanicDisposition::LoggedRecoverable
	} else {
		PanicDisposition::Fatal
	}
...
fn logs_dir() -> Option<PathBuf> {
	let home = home_dir()?;
	let config_override = std::env::var_os("REACTOR_CONFIG_DIR");
...
fn resolve_logs_dir(
	...
	// XDG takes precedence so users who migrated to `$XDG_STATE_HOME/reactor/logs/`
...
/// `$XDG_STATE_HOME` set, `$XDG_STATE_HOME/reactor` exists on disk, and
/// `REACTOR_CODING_AGENT_DIR` is unset or pointing at the default agent dir.
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn xdg_state_logs_from_env(home: &Path, config_dir_override: Option<&OsStr>) -> Option<PathBuf> {
	...
	let agent_override = std::env::var_os("REACTOR_CODING_AGENT_DIR");
...
/// reads, no fs reads. `reactor_dir_exists` decides whether the candidate
/// `<xdg_state_home>/reactor` actually lives on disk.
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn xdg_state_logs(
	xdg_state_home: Option<&OsStr>,
	agent_dir_override: Option<&OsStr>,
	default_agent_dir: &Path,
	reactor_dir_exists: impl FnOnce(&Path) -> bool,
) -> Option<PathBuf> {
	...
	let reactor_dir = Path::new(xdg).join(APP_NAME);
	if !reactor_dir_exists(&reactor_dir) {
		return None;
	}
	Some(reactor_dir.join("logs"))
}
#[cfg(any(target_os = "linux", target_os = "macos"))]
...
mod tests {
	...
	fn alloc_report_contains_size_alignment_and_backtrace() {
		...
		let report = format_alloc_report(layout);
		assert!(report.contains("reactor-natives alloc crash"), "report missing header: {report}");
	...
	#[test]
	fn resolve_logs_dir_defaults_under_dot_reactor() {
		let dir = resolve_logs_dir(Path::new("/tmp/reactor-natives-test-home"), None, None);
		assert_eq!(dir, PathBuf::from("/tmp/reactor-natives-test-home/.reactor/logs"));
	}
	...
	fn resolve_logs_dir_honors_relative_pi_config_dir() {
		...
		assert_eq!(dir, PathBuf::from("/tmp/reactor-natives-test-home/.reactor-dev/logs"));
	}
	...
	fn resolve_logs_dir_normalizes_parent_components_like_path_join() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/reactor-natives-test-home"),
			Some(OsStr::new("nested/../.reactor-dev")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/reactor-natives-test-home/.reactor-dev/logs"));
	}
	...
	fn xdg_state_logs_ignores_empty_agent_dir_override() {
		// An empty REACTOR_CODING_AGENT_DIR is "unset", not a divergent override; it
		// must not disable XDG resolution.
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			Some(OsStr::new("")),
			Path::new("/tmp/reactor-natives-test-home/.reactor/agent"),
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/reactor/logs")));
	}

	#[test]
	fn resolve_logs_dir_ignores_empty_pi_config_dir() {
		let dir =
			resolve_logs_dir(Path::new("/tmp/reactor-natives-test-home"), Some(OsStr::new("")), None);
		assert_eq!(dir, PathBuf::from("/tmp/reactor-natives-test-home/.reactor/logs"));
	}
	...
	fn resolve_logs_dir_prefers_xdg_when_provided() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/reactor-natives-test-home"),
			None,
			Some(PathBuf::from("/xdg/state/reactor/logs")),
		);
		assert_eq!(dir, PathBuf::from("/xdg/state/reactor/logs"));
	}
	...
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn xdg_state_logs_skipped_when_reactor_dir_missing() {
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			None,
			Path::new("/tmp/reactor-natives-test-home/.reactor/agent"),
	...
	fn xdg_state_logs_skipped_when_xdg_state_home_unset_or_empty() {
		let default_agent = Path::new("/tmp/reactor-natives-test-home/.reactor/agent");
	...
	fn xdg_state_logs_honored_when_agent_override_matches_default() {
		let default_agent = std::path::absolute(Path::new("./.reactor/agent")).unwrap();
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			Some(OsStr::new("./.reactor/agent")),
			&default_agent,
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/reactor/logs")));
	...
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn default_agent_dir_uses_dot_reactor_by_default() {
		let dir = default_agent_dir(Path::new("/tmp/reactor-natives-test-home"), None);
		assert_eq!(dir, PathBuf::from("/tmp/reactor-natives-test-home/.reactor/agent"));
	}
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	...
	fn build_crash_log_path_tags_kind_and_pid() {
		let dir = Path::new("/tmp/reactor-natives-test-home/.reactor/logs");
		...
		assert_eq!(
			alloc_log,
			PathBuf::from("/tmp/reactor-natives-test-home/.reactor/logs/native-alloc-99-1.log")
		);
	}
}
