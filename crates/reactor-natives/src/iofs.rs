...
//!
//! `reactor-walker` owns traversal and cache policy. This module keeps only the
...
fn walker_error_to_napi<E: std::fmt::Display>(err: reactor_walker::WalkError<E>) -> Error {
	match err {
		reactor_walker::WalkError::Interrupted(err) => Error::from_reason(err.to_string()),
		reactor_walker::WalkError::InvalidData { path, message } => Error::from_reason(format!(
...
pub(crate) const fn from_walker_file_type(file_type: reactor_walker::FileType) -> FileType {
	match file_type {
		reactor_walker::FileType::File => FileType::File,
		reactor_walker::FileType::Dir => FileType::Dir,
		reactor_walker::FileType::Symlink => FileType::Symlink,
	}
}

impl From<reactor_walker::CollectedEntry> for GlobMatch {
	fn from(entry: reactor_walker::CollectedEntry) -> Self {
		Self {
			path:      entry.path,
...
pub(crate) fn map_walker_error<E: std::fmt::Display>(err: reactor_walker::WalkError<E>) -> Error {
...
pub fn invalidate_fs_scan_cache(path: Option<String>) {
	match path {
		Some(path) => reactor_walker::invalidate_path_string(&path),
		None => reactor_walker::invalidate_all(),
	}
}
