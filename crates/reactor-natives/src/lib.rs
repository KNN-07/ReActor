...
//! # Example
//! ```ignore
//! use reactor_natives::text::visible_width;
//!
...
fn configure_rayon_pool() {
	...
	let result = match plan {
		RayonPoolPlan::WorkerThreads(threads) => rayon::ThreadPoolBuilder::new()
			...
			.build_global(),
		RayonPoolPlan::SkipGlobalPool => {
			reactor_uutils_ctx::set_rayon_global_pool_available(false);
			return;
		},
	};
	if result.is_ok() {
		reactor_uutils_ctx::set_rayon_global_pool_available(true);
	}
}
...
#[napi(js_name = "__piNativesV17_0_2")]
pub const fn reactor_natives_version_sentinel() {}
