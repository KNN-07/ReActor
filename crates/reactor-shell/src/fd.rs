//! In-process `fd` builtin backed by `reactor_walker`, `globset`, and `regex`.
...
use reactor_walker::CollectedEntry;
...
fn fd_walk_request(
	...
	reactor_walker::WalkRequest::new(root)
		...
		.order(reactor_walker::WalkOrder::Path)
		.emit_root(true)
...
fn process_walker_entry<W: Write>(
	...
	path: &Path,
	depth: usize,
	file_type: reactor_walker::FileType,
	out: &mut W,
	matches: &mut usize,
) -> io::Result<reactor_walker::WalkDecision> {
	let is_directory = file_type == reactor_walker::FileType::Dir;
	...
	if is_directory {
		if ignore_contains.iter().any(|name| path.join(name).exists()) {
			return Ok(reactor_walker::WalkDecision::SkipDescend);
		}
		if config.prune
			&& config
				.matcher
	...
	*matches = (*matches).saturating_add(1);
	if config.quiet {
		return Ok(reactor_walker::WalkDecision::Include);
	}
...
fn matches_walker_type_filter(
	...
	let kind_matches = if filter.has_kind() {
		(filter.regular && file_type == reactor_walker::FileType::File)
			|| (filter.directory && file_type == reactor_walker::FileType::Dir)
			|| (filter.symlink && file_type == reactor_walker::FileType::Symlink)
	} else {
		true
	};
	if !kind_matches {
...
/// closure, `reactor_walker`'s per-entry heartbeat never checks the flag and a
...
fn walker_error_to_io(err: reactor_walker::WalkError<io::Error>) -> io::Error {
	match err {
		reactor_walker::WalkError::Interrupted(err) => err,
		reactor_walker::WalkError::InvalidData { path, message } => {
			io::Error::other(format!("{}: {message}", path.display()))
		},
	}
}

fn walker_collect_error_to_io(err: reactor_walker::WalkError<String>) -> io::Error {
	match err {
		reactor_walker::WalkError::Interrupted(err) => io::Error::other(err),
		reactor_walker::WalkError::InvalidData { path, message } => {
			io::Error::other(format!("{}: {message}", path.display()))
		},
	}
...
fn process_collected_entry<W: Write>(
	...
	let is_directory = entry.file_type == reactor_walker::FileType::Dir;
	if depth == 0 && is_directory {
		return Ok(());
	}
...
mod tests {
	...
	fn walk_request(tree: &std::path::Path) -> reactor_walker::WalkRequest {
		reactor_walker::WalkRequest::new(tree)
			.hidden(true)
			.gitignore(false)
			.emit_root(true)
	}
	...
	fn cancel_heartbeat_aborts_collect_with_heartbeat() {
		...
		assert!(
			matches!(err, reactor_walker::WalkError::Interrupted(_)),
	...
	fn cancel_heartbeat_aborts_for_each_entry_with_heartbeat() {
		...
		let result = walk_request(&tree).for_each_entry_with_heartbeat(
			...
			|_entry| {
				...
				Ok::<_, std::io::Error>(reactor_walker::WalkDecision::Include)
			},
			|_error| Ok::<_, std::io::Error>(reactor_walker::WalkDecision::Include),
		);
