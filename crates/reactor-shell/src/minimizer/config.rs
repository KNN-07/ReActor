...
pub struct MinimizerOptions {
	...
	/// `REACTOR_MINIMIZER_LEGACY_FILTERS` environment variable (truthy = "1",
...
impl MinimizerConfig {
	...
	pub fn from_options(opts: &MinimizerOptions) -> Self {
		...
		let legacy_requested = resolve_legacy_filters(
			opts.legacy_filters,
			std::env::var("REACTOR_MINIMIZER_LEGACY_FILTERS")
				.ok()
				.as_deref(),
		);
			...
			&& !path.is_empty()
		{
			...
			if let Ok(contents) = fs::read_to_string(&expanded) {
				...
				match pipeline::parse_file(&contents, "user") {
					...
					Err(err) => {
						eprintln!("[reactor-natives minimizer] user filters: {err}");
					},
				}
			}
		}
...
/// and the raw `REACTOR_MINIMIZER_LEGACY_FILTERS` env value.
///
...
mod tests {
	...
	fn explicit_disabled_option_overrides_enabled_settings_file() {
		let path = std::env::temp_dir()
			.join(format!("reactor-minimizer-config-disabled-{}.toml", std::process::id()));
	...
	fn explicit_legacy_true_overrides_disabled_settings_file() {
		let path = std::env::temp_dir()
			.join(format!("reactor-minimizer-config-legacy-{}.toml", std::process::id()));
