...
mod tests {
	...
	fn bun_run_check_colon_compacts_workspace_success_noise() {
		...
		assert!(out.text.contains("@reactor/utils"));
		assert!(out.text.contains("@reactor/coding-agent"));
	...
	fn bun_check_failure_keeps_diagnostic_and_emits_failed_status() {
		...
		let input = concat!(
			...
			"@reactor/coding-agent check: $ tsgo -p tsconfig.json --noEmit\n",
			"src/tools/bash.ts(42,7): error TS2322: Type 'string' is not assignable to type \
			 'number'.\n",
			"@reactor/coding-agent check: Exited with code 1\n",
		);
	...
	fn bun_check_success_emits_passed_status_and_no_noise() {
		...
		let input = concat!(
			...
			"@reactor/utils check: $ tsgo -p tsconfig.json --noEmit\n",
			"@reactor/utils check: Exited with code 0\n",
			"@reactor/coding-agent check: Checked 1178 files in 287ms. No fixes applied.\n",
			"@reactor/coding-agent check: $ tsgo -p tsconfig.json --noEmit\n",
			"@reactor/coding-agent check: Exited with code 0\n",
		);
