...
";
	...
	fn fetch_output_counts_new_refs_as_updates() {
		...
		let out = filter(
			&ctx,
			"From github.com:KNN-07/ReActor\n * [new branch]      feature -> origin/feature\n",
			0,
		);
		assert!(out.changed);
		assert!(
	...
	fn push_output_keeps_deleted_refs() {
		...
		let out =
			filter(&ctx, "To github.com:KNN-07/ReActor.git\n - [deleted]         old-branch\n", 0);
		assert!(out.changed);
	...
	fn diff_stat_is_summarized() {
		...
		let input = "\
 crates/reactor-shell/src/minimizer/filters/git.rs     | 385 +++++++++++++++++------
...
";
		...
		assert!(
			out.text
				.contains("crates/reactor-shell/src/minimizer/filters/git.rs")
		);
		assert!(
			out.text
