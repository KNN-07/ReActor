...
async fn create_session_for_run(
	...
	if !uutils_env_disabled(config, "REACTOR_DISABLE_UUTILS_BUILTINS") {
		...
		if !uutils_env_disabled(config, "REACTOR_DISABLE_UUTILS_DESTRUCTIVE") {
			if !uutils_env_disabled(config, "REACTOR_DISABLE_RM_BUILTIN") {
				shell.register_builtin("rm", crate::coreutils::rm_builtin());
			}
			if !uutils_env_disabled(config, "REACTOR_DISABLE_MV_BUILTIN") {
	...
	let nohup_builtin_disabled = {
		let raw = config
			.session_env
			.as_ref()
			.and_then(|env| env.get("REACTOR_DISABLE_NOHUP_BUILTIN").cloned())
			.or_else(|| std::env::var("REACTOR_DISABLE_NOHUP_BUILTIN").ok());
...
async fn source_snapshot(
	...
	let source_info = SourceInfo::from("reactor-natives:snapshot");
...
async fn run_shell_command_single(
		...
		&& let Some(config) = options.minimizer.as_ref()
	{
		...
		if !buffered.exceeded {
			...
			// in `reactor-natives`. A supported filter that runs but leaves the output
...
async fn run_shell_command_once(
	...
	let source_info = SourceInfo::from("reactor-natives:command");
	let result = session
		.shell
		.run_string(command, &source_info, &params)
		.await;
...
async fn run_shell_command_streams(
	...
	ensure_trailing_newline_for_heredoc(&mut command);
	let source_info = SourceInfo::from("reactor-shell:streams");
	let result = session
		.shell
		.run_string(command, &source_info, &params)
		.await;
...
/// backpressure: on a bounded channel (the reactor-natives JS bridge) the send
...
impl builtins::Command for TimeoutCommand {
	...
	fn execute<SE: brush_core::ShellExtensions>(
		...
		async move {
			...
			let source_info = SourceInfo::from("reactor-natives:timeout");
...
impl builtins::Command for NohupCommand {
	...
	fn execute<SE: brush_core::ShellExtensions>(
		...
		async move {
			...
			let mut params = context.params.clone();
			params.process_group_policy = ProcessGroupPolicy::NewProcessGroup;
			let source_info = SourceInfo::from("reactor-natives:nohup");
			context
				.shell
				.run_string(command_line, &source_info, &params)
				.await
...
mod tests {
	...
	async fn uutils_mkdir_resolves_cwd_and_displays_operand() {
		...
		let source_info = SourceInfo::from("reactor-natives:test");
		let exec = session
			.shell
	...
	async fn uutils_filters_listing_find_grep() {
		let tmp = std::env::temp_dir().join(format!("utils-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		...
		params.set_fd(OpenFiles::STDOUT_FD, null_file().expect("null"));
		params.set_fd(OpenFiles::STDERR_FD, null_file().expect("null"));
		let si = SourceInfo::from("reactor-natives:test");
	...
	async fn uutils_builtins_respect_disable_env() {
		...
		let mut all_off = create_session(&session_with(&[("REACTOR_DISABLE_UUTILS_BUILTINS", "1")]))
			.await
			.expect("create_session");
		...
		let mut rm_off = create_session(&session_with(&[("REACTOR_DISABLE_RM_BUILTIN", "1")]))
			.await
			.expect("create_session");
	...
	fn unique_temp_dir(prefix: &str) -> std::path::PathBuf {
		...
		let nonce = std::time::SystemTime::now()
			...
			.expect("system time")
			.as_nanos();
		path.push(format!("reactor-shell-{prefix}-{}-{nonce}", std::process::id()));
...
"#,
	...
	async fn embedded_external_command_runs_in_its_own_session() {
		...
		// Build the same kind of session reactor-natives uses in production.
		let config = ShellConfig { session_env: None, snapshot_path: None, minimizer: None };
		let mut session = create_session(&config).await.expect("create_session");
