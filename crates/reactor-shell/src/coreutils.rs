...
/// Drives a patched uutils utility to completion under a [`reactor_uutils_ctx`]
...
fn run_caught(run: UutilRun, argv: Vec<OsString>) -> i32 {
	...
	if let Ok(code) = catch_unwind(|| run(argv)) {
		...
		let _ = writeln!(reactor_uutils_ctx::stderr(), "{name}: internal error");
		1
	}
}
...
uutil_builtin!(pub fn grep_builtin => reactor_uu_grep::run);
uutil_builtin!(pub fn rg_builtin => reactor_uu_grep::run_rg);
...
uutil_builtin!(pub fn diff_builtin => reactor_uu_diff::run);
...
mod tests {
	...
	fn scope_io(stderr: Box<dyn Write + Send>) -> reactor_uutils_ctx::ScopeIo {
		reactor_uutils_ctx::ScopeIo {
