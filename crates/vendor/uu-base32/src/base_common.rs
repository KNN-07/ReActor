...
use uucore::{
	...
	error::{FromIo, UResult, USimpleError, UUsageError, strip_errno},
	format_usage,
};
...
pub fn get_input(config: &Config) -> UResult<Box<dyn BufRead>> {
	match &config.to_read {
		Some(name) => {
			let file = File::open(reactor_uutils_ctx::resolve(Path::new(name)))
		...
		None => {
			...
			Ok(Box::new(BufReader::with_capacity(DEFAULT_BUF_SIZE, reactor_uutils_ctx::stdin())))
		},
	}
}
pub fn handle_input<R: BufRead>(input: &mut R, format: Format, config: Config) -> UResult<()> {
	...
	let mut stdout_lock = reactor_uutils_ctx::stdout().lock();
...
/// Determines if the input buffer contains any padding ('=') ignoring trailing
/// whitespace.
#[cfg(test)]
fn read_and_has_padding<R: io::Read>(input: &mut R) -> UResult<(bool, Vec<u8>)> {
	...
	// Treat the stream as padded if any '=' exists (GNU coreutils continues
	// decoding even when padding bytes are followed by more data).
