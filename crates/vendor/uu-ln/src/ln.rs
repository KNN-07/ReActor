...
fn show_error(msg: impl std::fmt::Display) {
	...
	if !rendered.is_empty() {
		let _ = writeln!(reactor_uutils_ctx::stderr(), "ln: {rendered}");
	}
}
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match ln_main(&matches) {
		...
		Err(err) => {
			...
			if !msg.is_empty() {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "ln: {msg}");
			}
...
fn exec(files: &[PathBuf], settings: &Settings) -> UResult<()> {
	...
	if !settings.no_target_dir {
		...
		if files.len() > 2 || reactor_uutils_ctx::resolve(last_file).is_dir() {
...
fn link_files_in_dir(files: &[PathBuf], target_dir: &Path, settings: &Settings) -> UResult<()> {
	...
	let target_dir_fs = reactor_uutils_ctx::resolve(target_dir);
...
fn link(src: &Path, dst: &Path, settings: &Settings) -> UResult<()> {
	...
	let src_fs = reactor_uutils_ctx::resolve(src);
	let dst_fs = reactor_uutils_ctx::resolve(dst);
...
pub fn symlink<P1: AsRef<Path>, P2: AsRef<Path>>(src: P1, dst: P2) -> std::io::Result<()> {
	...
	if reactor_uutils_ctx::resolve(src.as_ref()).is_dir() {
		symlink_dir(src, dst)
	} else {
...
mod tests {
	...
	fn symlink_relative_operands_create_in_scope_cwd_with_literal_content() {
		...
		// the call-site `reactor_uutils_ctx::resolve` patch places the link in the
