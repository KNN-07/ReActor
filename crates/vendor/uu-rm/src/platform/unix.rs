...
pub fn safe_remove_file(
	...
	let dir_fd = DirFd::open(&reactor_uutils_ctx::resolve(parent), SymlinkBehavior::Follow).ok()?;
...
fn handle_unlink(
	...
	if let Err(e) = dir_fd.unlink_at(entry_name, is_dir) {
		let e = e.map_err_context(|| format!("cannot remove {}", entry_path.quote()));
		show_error!("{e}");
		true
	} else {
		if is_dir {
...
pub fn remove_dir_with_special_cases(path: &Path, options: &Options, error_occurred: bool) -> bool {
	match fs::remove_dir(reactor_uutils_ctx::resolve(path)) {
		...
		Err(_) if !error_occurred && reactor_uutils_ctx::resolve(path).read_dir().is_err() => {
...
pub fn safe_remove_dir_recursive(
	...
	let initial_mode = match fs::symlink_metadata(reactor_uutils_ctx::resolve(path)) {
	...
	let dir_fd = match DirFd::open(&reactor_uutils_ctx::resolve(path), SymlinkBehavior::Follow) {
		Ok(fd) => fd,
		Err(e) => {
			...
			if e.kind() == std::io::ErrorKind::PermissionDenied {
				...
				if fs::remove_dir(reactor_uutils_ctx::resolve(path)).is_ok() {
					verbose_removed_directory(path, options);
					return false;
				}
