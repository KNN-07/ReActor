...
fn is_first_filename_timestamp(
	...
	timestamp.is_none()
		...
		&& reactor_uutils_ctx::var("_POSIX2_VERSION").as_deref() == Some("199209")
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match touch_main(&matches) {
		...
		Err(err) => {
			...
			if !msg.is_empty() {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "touch: {msg}");
			}
...
pub fn touch(files: &[InputFile], opts: &Options) -> Result<(), TouchError> {
	let (atime, mtime) = match &opts.source {
		Source::Reference(reference) => {
			...
			let (atime, mtime) = stat(&reactor_uutils_ctx::resolve(reference), !opts.no_deref)
...
fn touch_file(
	...
	if let Err(e) = metadata_result {
		...
		if opts.no_deref {
			...
			let _ = writeln!(reactor_uutils_ctx::stderr(), "touch: {e}");
...
mod tests {
	...
	fn relative_operand_creates_file_in_scope_cwd() {
		...
		// the call-site `reactor_uutils_ctx::resolve` patch makes the file land in
