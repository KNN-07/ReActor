...
pub fn run(args: Vec<OsString>) -> i32 {
	...
	match run_matches(&matches) {
		...
		Err(err) => {
			let code = err.code();
			let _ = writeln!(reactor_uutils_ctx::stderr(), "mkdir: {err}");
...
fn exec(dirs: ValuesRef<OsString>, config: &Config) {
	for dir in dirs {
		...
		if let Err(e) = mkdir(path, config) {
			let _ = writeln!(reactor_uutils_ctx::stderr(), "mkdir: {e}");
...
fn create_dir(path: &Path, is_parent: bool, config: &Config) -> UResult<()> {
	let path_exists = reactor_uutils_ctx::resolve(path).exists();
	...
	if config.recursive {
		...
		for dir in dirs_to_create.iter().rev() {
			if !reactor_uutils_ctx::resolve(dir).exists() {
...
fn create_single_dir(path: &Path, is_parent: bool, config: &Config) -> UResult<()> {
	...
	match create_dir_with_mode(&fs_path, create_mode) {
		Ok(()) => {
			if config.verbose {
				writeln!(reactor_uutils_ctx::stdout(), "mkdir: created directory {}", path.quote())?;
			}
