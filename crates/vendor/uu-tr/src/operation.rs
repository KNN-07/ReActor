...
// spell-checker:ignore (strings) anychar combinator Alnum Punct Xdigit alnum
// punct xdigit cntrl
...
impl Sequence {
	pub fn from_str(input: &[u8]) -> Result<Vec<Self>, BadSequence> {
		many0(alt((
			...
			map(Self::parse_backslash_or_char_with_warning, |s| Ok(Self::Char(s))),
		)))
		.parse(input)
		.map(|(_, r)| r)
		.unwrap()
	...
	fn parse_octal_with_warning(input: &[u8]) -> IResult<&[u8], u8> {
		preceded(
			...
			alt((
				...
				// Fallback for if the three digit octal escape is greater than \377 (0xFF), and
				// therefore can't be parsed as as a byte
		...
		map_opt(recognize(many_m_n(1, 3, one_of("01234567"))), |out: &[u8]| {
			...
			if result.is_none() {
				if let Ok(origin_octal) = std::str::from_utf8(input) {
					...
					let _ = writeln!(reactor_uutils_ctx::stderr(), "tr: warning: invalid utf8 sequence");
				}
			}
			result
		})
	...
	fn parse_backslash_or_char_with_warning(input: &[u8]) -> IResult<&[u8], u8> {
		alt((Self::parse_octal_with_warning, Self::parse_backslash, Self::single_char)).parse(input)
	}
