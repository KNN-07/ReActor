...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match realpath_main(&matches) {
		...
		// `reactor_uutils_ctx::set_exit_code` (upstream's `show!` machinery).
		Ok(()) => reactor_uutils_ctx::exit_code(),
		Err(err) => {
			let code = err.code();
			let msg = err.to_string();
			if !msg.is_empty() {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "realpath: {msg}");
			}
	...
	for path in &paths {
		...
		if !quiet {
			...
			if let Err(err) = result.map_err_context(|| path.maybe_quote().to_string()) {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "realpath: {err}");
...
fn canonicalize_relative(
	...
	// consulted.
	let abs = canonicalize(reactor_uutils_ctx::resolve(r), can_mode, resolve)?;
...
fn resolve_path(
	...
	// consulted.
	let abs = canonicalize(reactor_uutils_ctx::resolve(p), can_mode, resolve)?;
...
mod tests {
	...
	fn resolves_relative_operand_against_scope_cwd() {
		...
		// the call-site `reactor_uutils_ctx::resolve` patch makes this find the
