...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match paste(files, serial, delimiters, line_ending) {
		...
		Err(err) => {
			let code = err.code();
			let _ = writeln!(reactor_uutils_ctx::stderr(), "paste: {err}");
...
pub fn uu_app() -> Command {
	Command::new("paste")
		...
		.override_usage(reactor_uutils_ctx::format_usage("paste [OPTION]... [FILE]..."))
...
fn paste(
	...
	let stdin = Rc::new(RefCell::new(BufReader::new(reactor_uutils_ctx::stdin())));
	...
	for filename in filenames {
		if filename == "-" {
			...
			let file = File::open(reactor_uutils_ctx::resolve(&filename)).map_err(|err| {
	...
	let mut stdout = reactor_uutils_ctx::stdout();
...
enum InputSource {
	File(BufReader<File>),
	StandardInput(Rc<RefCell<BufReader<reactor_uutils_ctx::CtxStdin>>>),
}
