...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match tee(&opts) {
		Ok(()) => 0,
		Err(err) => {
			let _ = writeln!(reactor_uutils_ctx::stderr(), "tee: {err}");
			1
		},
	}
}
...
fn tee(options: &Options) -> Result<()> {
	...
	for name in &options.files {
		...
		match open(name, options.append) {
			...
			Err(err) => {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "tee: {}: {err}", name.maybe_quote());
	...
	let copy_result = copy(reactor_uutils_ctx::stdin(), &mut output);
...
fn open(name: &OsString, append: bool) -> Result<NamedWriter> {
	let path = reactor_uutils_ctx::resolve(name);
...
impl Write for Writer {
	fn write(&mut self, buf: &[u8]) -> Result<usize> {
		match self {
			...
			Self::Stdout => reactor_uutils_ctx::stdout().write(buf),
		}
	}
	...
	fn flush(&mut self) -> Result<()> {
		match self {
			...
			Self::Stdout => reactor_uutils_ctx::stdout().flush(),
		}
	}
}
