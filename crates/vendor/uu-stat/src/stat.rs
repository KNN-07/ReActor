...
pub fn run(_argv: Vec<std::ffi::OsString>) -> i32 {
	use std::io::Write;
	let _ = writeln!(reactor_uutils_ctx::stderr(), "stat: unsupported on this platform");
	1
}
...
pub fn uu_app() -> clap::Command {
	clap::Command::new("stat")
		...
		.override_usage(reactor_uutils_ctx::format_usage("stat [OPTION]... FILE..."))
}
...
for details about the options it supports.";
	...
	fn get_quoted_file_name(
		...
		let quoting_style = reactor_uutils_ctx::var("QUOTING_STYLE")
		...
		if file_type.is_symlink() {
			...
			match fs::read_link(resolved) {
				...
				Err(e) => {
					...
					let _ = writeln!(reactor_uutils_ctx::stderr(), "stat: {e}");
					Err(1)
				},
			}
		} else {
	...
	fn process_token_filesystem(t: &Token, meta: &StatFs, display_name: &str) {
		match *t {
			...
			// pi-uutils: context stdout instead of `print!`.
			Token::Char(c) => {
				let _ = write!(reactor_uutils_ctx::stdout(), "{c}");
			},
	...
	fn write_raw_byte(byte: u8) {
		...
		let _ = reactor_uutils_ctx::stdout().write_all(&[byte]);
	}
	...
	pub fn run(argv: Vec<OsString>) -> i32 {
		...
		let argv = match rewrite_bsd_invocation(&argv) {
			...
			Some(Err(msg)) => {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "stat: {msg}");
				return 1;
			},
		};
