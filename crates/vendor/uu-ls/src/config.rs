...
fn is_color_compatible_term() -> bool {
	let term = reactor_uutils_ctx::var("TERM").map(OsString::from);
	let colorterm = reactor_uutils_ctx::var("COLORTERM").map(OsString::from);
...
fn extract_quoting_style(
	...
	if let Some(style) = opt_quoting_style {
		...
		if let Some(style) = reactor_uutils_ctx::var("QUOTING_STYLE") {
...
fn parse_width(width_match: Option<&String>) -> Result<u16, LsError> {
	...
	let calculate_term_size = || DEFAULT_TERM_WIDTH;

	let ret = match width_match {
		Some(x) => parse_width_from_args(x)?,
		None => match reactor_uutils_ctx::var("COLUMNS").map(OsString::from) {
...
impl Config {
	...
	pub fn from(options: &clap::ArgMatches) -> UResult<Self> {
		...
		let env_var_blocksize = reactor_uutils_ctx::var("BLOCKSIZE").map(OsString::from);
		let env_var_block_size = reactor_uutils_ctx::var("BLOCK_SIZE").map(OsString::from);
		let env_var_ls_block_size = reactor_uutils_ctx::var("LS_BLOCK_SIZE").map(OsString::from);
		let env_var_posixly_correct = reactor_uutils_ctx::var("POSIXLY_CORRECT").map(OsString::from);
		...
		let (file_size_block_size, block_size) = if !opt_si && !opt_hr && !raw_block_size.is_empty() {
			if let Ok(size) = parse_size_non_zero_u64(&raw_block_size.to_string_lossy()) {
				...
				if let Some(invalid_block_size) = opt_block_size {
					return Err(Box::new(LsError::BlockSizeParseError(invalid_block_size.clone())));
				}
		...
		let color = if needs_color {
			Some(match reactor_uutils_ctx::var("LS_COLORS") {
		...
		if dired || is_dired_arg_present() {
			...
			// if we have --dired --hyperlink, we don't show dired but we still want to see
			// the long format
			format = Format::Long;
		}
		...
		let tab_size = if needs_color {
			...
			options
				...
				.or_else(|| reactor_uutils_ctx::var("TABSIZE").and_then(|s| s.parse().ok()))
		}
...
fn parse_time_style(options: &clap::ArgMatches) -> Result<(String, Option<String>), LsError> {
	...
	if let Some(field) = options
		...
		.map(Cow::from)
		.or_else(|| reactor_uutils_ctx::var("TIME_STYLE").map(Cow::from))
	{
				...
				> options.indices_of(options::TIME_STYLE).unwrap().next_back()
		{
			...
			let field = if let Some(field) = field.strip_prefix("posix-") {
				...
				if reactor_uutils_ctx::var("LC_TIME").as_deref() == Some("POSIX")
					|| reactor_uutils_ctx::var("LC_ALL").as_deref() == Some("POSIX")
				{
