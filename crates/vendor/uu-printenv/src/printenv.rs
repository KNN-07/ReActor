...
// the no-argument dump iterates `reactor_uutils_ctx::env_snapshot()` and named
// lookups go through `reactor_uutils_ctx::var`, because the embedding shell's
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match printenv_main(&matches) {
		...
		Err(err) => {
			...
			if !msg.is_empty() {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "printenv: {msg}");
			}
...
fn printenv_main(matches: &ArgMatches) -> UResult<()> {
	...
	if variables.is_empty() {
		...
		for (key, value) in reactor_uutils_ctx::env_snapshot() {
	...
	for env_var in variables {
		...
		if let Some(var) = reactor_uutils_ctx::var(&env_var) {
			let mut stdout = reactor_uutils_ctx::stdout();
