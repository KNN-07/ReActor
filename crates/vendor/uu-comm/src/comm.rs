...
fn compare(
	...
	let mut writer = BufWriter::new(reactor_uutils_ctx::stdout());
	...
	if should_check && (c1.has_error || c2.has_error) {
		if delayed_error {
			let _ = writeln!(reactor_uutils_ctx::stderr(), "comm: input is not in sorted order");
		}
		Ok(false)
	} else {
		Ok(true)
...
fn open_file(name: &OsStr, ending: LineEnding) -> io::Result<LineReader> {
	if name == "-" {
		return Ok(LineReader::new(Box::new(BufReader::new(reactor_uutils_ctx::stdin())), ending));
	}
	let resolved = reactor_uutils_ctx::resolve(name);
...
fn comm_main(matches: &ArgMatches) -> UResult<bool> {
	...
	let identical = if name1 == "-" || name2 == "-" {
		...
		files_identical(&reactor_uutils_ctx::resolve(name1), &reactor_uutils_ctx::resolve(name2))
			.unwrap_or(false)
	};
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match comm_main(&matches) {
		Ok(true) => reactor_uutils_ctx::exit_code(),
		Ok(false) => 1,
		Err(e) => {
			let code = e.code();
			let _ = writeln!(reactor_uutils_ctx::stderr(), "comm: {e}");
