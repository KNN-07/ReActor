		...
		match token {
			...
			"-r" => {
				if let Some(value) = toks.get(i + 1)
					...
					&& std::fs::symlink_metadata(reactor_uutils_ctx::resolve(Path::new(&argv[i + 1])))
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	let argv = match rewrite_bsd_invocation(&argv) {
		...
		Some(Err(msg)) => {
			let _ = writeln!(reactor_uutils_ctx::stderr(), "date: {msg}");
			return 1;
		},
	};
...
fn date_main(matches: &ArgMatches) -> UResult<()> {
	...
	let dates: Box<dyn Iterator<Item = _>> = match &settings.date_source {
		...
		DateSource::FileMtime(path) => {
			...
			// directory; `path` is kept for display.
			let metadata = std::fs::metadata(reactor_uutils_ctx::resolve(path))
	...
	for date in dates {
		...
		match date {
			...
			Err((input, _err)) => {
				...
				let _ = writeln!(reactor_uutils_ctx::stderr(), "date: invalid date '{input}'");
...
fn parse_date<S: AsRef<str> + Clone>(
	...
	if dbg_opts.debug {
		let _ = writeln!(reactor_uutils_ctx::stderr(), "date: input string: {input_str}");
	}
