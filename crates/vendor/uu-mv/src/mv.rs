...
fn show(err: &dyn UError) {
	reactor_uutils_ctx::set_exit_code(err.code());
	let _ = writeln!(reactor_uutils_ctx::stderr(), "mv: {err}");
}
...
fn run_matches(matches: &ArgMatches) -> UResult<()> {
	...
	if let Some(maybe_dir) = &target_dir {
		...
		if !reactor_uutils_ctx::resolve(Path::new(maybe_dir)).is_dir() {
...
fn handle_two_paths(source: &Path, target: &Path, opts: &Options) -> UResult<()> {
	...
	let source_fs = reactor_uutils_ctx::resolve(source);
	let target_fs = reactor_uutils_ctx::resolve(target);
...
fn assert_not_same_file(
	...
	let source_abs = reactor_uutils_ctx::resolve(source);
	let target_abs = reactor_uutils_ctx::resolve(target);
...
fn move_files_into_dir(files: &[PathBuf], target_dir: &Path, options: &Options) -> UResult<()> {
	...
	// pi-uutils: resolve against the shell working directory before probing.
	if !reactor_uutils_ctx::resolve(target_dir).is_dir() {
	...
	for sourcepath in files {
		...
		if reactor_uutils_ctx::resolve(sourcepath)
			.symlink_metadata()
			.is_err()
		{
		...
		match rename(
			...
			Err(e) if e.to_string().is_empty() => reactor_uutils_ctx::set_exit_code(1),
			Err(e) => {
...
fn rename(
	...
	let from_fs = reactor_uutils_ctx::resolve(from);
	let to_fs = reactor_uutils_ctx::resolve(to);

	if to_fs.exists() {
		if opts.update == UpdateMode::None {
			if opts.debug {
				let _ = writeln!(reactor_uutils_ctx::stdout(), "skipped {}", to.quote());
			}
			return Ok(());
		}
	...
	if let Some(context) = &opts.context {
		set_selinux_security_context(&reactor_uutils_ctx::resolve(to), Some(context))
	...
	if opts.verbose {
		...
		match display_manager {
			Some(pb) => pb.suspend(|| {
				let _ = writeln!(reactor_uutils_ctx::stdout(), "{message}");
			}),
			None => {
				let _ = writeln!(reactor_uutils_ctx::stdout(), "{message}");
			},
...
fn rename_fifo_fallback(from: &Path, to: &Path) -> io::Result<()> {
	...
	make_fifo(&to_fs).and_then(|_| fs::remove_file(reactor_uutils_ctx::resolve(from)))
}
...
fn rename_symlink_fallback(from: &Path, to: &Path) -> io::Result<()> {
	...
	let path_symlink_points_to = fs::read_link(reactor_uutils_ctx::resolve(from))?;
	unix::fs::symlink(path_symlink_points_to, reactor_uutils_ctx::resolve(to))?;
	#[cfg(not(any(target_os = "macos", target_os = "redox")))]
	{
		let _ = copy_xattrs_if_supported(from, to);
	}
	fs::remove_file(reactor_uutils_ctx::resolve(from))
}
...
fn rename_dir_fallback(
	...
	let total_size = dir_get_size(reactor_uutils_ctx::resolve(from)).ok();
	...
	#[cfg(all(unix, not(any(target_os = "macos", target_os = "redox"))))]
	let xattrs = fsxattr::retrieve_xattrs(reactor_uutils_ctx::resolve(from))
	...
	#[cfg(all(unix, not(any(target_os = "macos", target_os = "redox"))))]
	fsxattr::apply_xattrs(reactor_uutils_ctx::resolve(to), xattrs)?;

	result?;

	// Remove the source directory after successful copy
	fs::remove_dir_all(reactor_uutils_ctx::resolve(from))?;
...
fn copy_dir_contents(
	...
	fs::create_dir_all(reactor_uutils_ctx::resolve(to))?;
...
fn copy_dir_contents_recursive(
	...
	let entries = fs::read_dir(reactor_uutils_ctx::resolve(from_dir))?;
	...
	for entry in entries {
		...
		if reactor_uutils_ctx::resolve(&from_path).is_symlink() {
		...
		} else if reactor_uutils_ctx::resolve(&from_path).is_dir() {
			// Recursively copy subdirectory (only real directories, not symlinks)
			fs::create_dir_all(reactor_uutils_ctx::resolve(&to_path))?;
		...
		if let Some(pb) = progress_bar
			&& let Ok(metadata) = reactor_uutils_ctx::resolve(&from_path).metadata()
		{
			pb.inc(metadata.len());
		}
	}
...
fn copy_file_with_hardlinks_helper(
	...
	if reactor_uutils_ctx::resolve(from).is_symlink() {
		...
		// Copy a regular file.
		fs::copy(reactor_uutils_ctx::resolve(from), reactor_uutils_ctx::resolve(to))?;
...
fn rename_file_fallback(
	...
	#[cfg(unix)]
	{
		if let (Some(tracker), Some(scanner)) = (hardlink_tracker, hardlink_scanner) {
			...
			if let Some(existing_target) = tracker.check_hardlink(from, to, scanner, &hardlink_options)
			{
				...
				fs::hard_link(reactor_uutils_ctx::resolve(&existing_target), &to_fs)?;
				fs::remove_file(reactor_uutils_ctx::resolve(from))?;
				return Ok(());
			}
		}
	...
	// Regular file copy
	fs::copy(reactor_uutils_ctx::resolve(from), &to_fs)
...
fn copy_xattrs_if_supported(from: &Path, to: &Path) -> io::Result<()> {
	match fsxattr::copy_xattrs(reactor_uutils_ctx::resolve(from), reactor_uutils_ctx::resolve(to)) {
		Ok(()) => Ok(()),
...
fn is_writable(path: &Path) -> (bool, Option<u32>) {
	if let Ok(metadata) = reactor_uutils_ctx::resolve(path).metadata() {
...
fn read_yes() -> bool {
	...
	let mut stdin = reactor_uutils_ctx::stdin();
	let mut buf = [0u8; 1];
	let mut first = None;
	loop {
...
fn prompt_overwrite(to: &Path, cached_mode: Option<u32>) -> io::Result<()> {
	...
	let mut err = reactor_uutils_ctx::stderr();
...
fn can_delete_file(path: &Path) -> bool {
	...
	let resolved = reactor_uutils_ctx::resolve(path);
