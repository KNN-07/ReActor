...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match tac_main(&matches) {
		...
		Err(err) => {
			...
			if !msg.is_empty() {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "tac: {msg}");
			}
...
fn show(err: &TacError) {
	let _ = writeln!(reactor_uutils_ctx::stderr(), "tac: {err}");
...
fn tac(filenames: &[OsString], before: bool, regex: bool, separator: &OsStr) -> UResult<()> {
	...
	for filename in filenames {
		...
		let data: &[u8] = if filename == "-" {
			...
			match reactor_uutils_ctx::stdin().read_to_end(&mut contents) {
				Ok(_) => {
					buf = contents;
					&buf
				},
			...
			let path = reactor_uutils_ctx::resolve(filename);
