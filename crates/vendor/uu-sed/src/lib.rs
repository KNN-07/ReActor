...
//! shell builtin via [`reactor_uutils_ctx`].
//!
...
//! - all stdio goes through the `reactor_uutils_ctx` streams,
//! - every path operand resolves against the shell working directory via
//!   `reactor_uutils_ctx::resolve`,
//! - no `std::process::exit`: `q`/`Q` exit codes flow through
//!   `reactor_uutils_ctx::set_exit_code`, clap errors are rendered manually,
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	if !matches.args_present() {
		let _ = write!(reactor_uutils_ctx::stdout(), "{}", sed::uu_app().render_help());
		return 1;
	}
	...
	match sed::sed_main(&matches) {
		...
		Err(e) => {
			let code = e.code();
			let _ = writeln!(reactor_uutils_ctx::stderr(), "sed: {e}");
...
mod tests {
	...
	/// Drive `run()` under a reactor-uutils-ctx scope with `cwd` as the shell
	...
	fn run_sed_in(cwd: PathBuf, stdin: &[u8], args: &[&str]) -> (i32, String, String) {
		...
		let argv: Vec<OsString> = std::iter::once("sed")
			...
			.map(OsString::from)
			.collect();
		let code = reactor_uutils_ctx::scope(
			reactor_uutils_ctx::ScopeIo {
