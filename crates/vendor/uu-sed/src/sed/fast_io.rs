...
impl<'a> LineReader<'a> {
	...
	pub fn open(path: &PathBuf) -> io::Result<Self> {
		if path.as_os_str() == "-" {
			// Patched for reactor-uutils-ctx embedding: read the context stdin.
			let boxed: Box<dyn Read> = Box::new(reactor_uutils_ctx::stdin());
			let reader = BufReader::new(boxed);
			return Ok(LineReader::ReadInput(ReadLineCursor::new(reader)));
		}

		// Patched for reactor-uutils-ctx embedding: input file operands resolve
...
impl OutputBuffer {
	...
	#[cfg(unix)]
	pub fn new(w: Box<dyn OutputWrite + 'static>) -> Self {
		// Patched for reactor-uutils-ctx embedding: the writer is not fd-backed,
	...
	pub fn copy_file(&mut self, path: &PathBuf) -> io::Result<()> {
		...
		let Ok(file) = File::open(reactor_uutils_ctx::resolve(path)) else {
...
impl OutputBuffer {
	...
	// Patched for reactor-uutils-ctx embedding: the raw-fd write(2) and
