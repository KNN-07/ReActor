		...
		match &self.sources[next_index] {
			...
			ScriptValue::PathVal(p) => {
				if p.to_string_lossy() == "-" {
					self.state = State::Active {
						index:       next_index,
						reader:      Box::new(BufReader::new(reactor_uutils_ctx::stdin())),
						input_name:  "<stdin>".to_string(),
						line_number: 0,
					};
				} else {
					// Patched for reactor-uutils-ctx embedding: resolve `-f`
					// script files against the shell working directory.
					let file = File::open(reactor_uutils_ctx::resolve(p))
