...
impl InPlace {
	...
	pub fn new(context: ProcessingContext) -> Self {
		Self {
			output:          OutputBuffer::new(Box::new(reactor_uutils_ctx::stdout())),
	...
	pub fn begin(&mut self, file_name: &Path) -> UResult<&mut OutputBuffer> {
		// Patched for reactor-uutils-ctx embedding: resolve the operand against
		...
		let file_name = reactor_uutils_ctx::resolve(file_name);
	...
	fn begin_resolved(&mut self, file_name: &Path) -> UResult<&mut OutputBuffer> {
		if !self.in_place {
			self.output = OutputBuffer::new(Box::new(reactor_uutils_ctx::stdout()));
