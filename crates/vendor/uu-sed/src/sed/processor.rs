...
fn shell_command(cmd: &str) -> std::process::Command {
	...
	c.arg("/C").arg(cmd);
	// Patched for reactor-uutils-ctx embedding: see the unix variant above.
	c.current_dir(reactor_uutils_ctx::cwd());
	c
}
...
fn process_file(
	...
	'lines: while let Some(mut pattern) = reader.get_line()? {
		// Patched for reactor-uutils-ctx embedding: mmap-backed input never
		...
		while let Some(command_rc) = current.take() {
			...
			match command.code {
				...
				'q' => {
					...
					reactor_uutils_ctx::set_exit_code(*extract_variant!(command, Number) as i32);
...
pub fn process_all_files(
	...
	// Patched for reactor-uutils-ctx embedding: the context streams are never a
