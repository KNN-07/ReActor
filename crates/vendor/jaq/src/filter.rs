...
fn overrides() -> impl Iterator<Item = jaq_std::Filter<Native<Val>>> {
	...
	fn debug_msg(v: &Val) {
		...
		let _ = writeln!(reactor_uutils_ctx::stderr(), "[\"DEBUG:\", {v}]");
	}

	fn stderr_msg(v: &Val) {
		// like jq, print strings raw and everything else as JSON, no newline
		if let Some(s) = v.as_str() {
			let _ = write!(reactor_uutils_ctx::stderr(), "{s}");
		} else {
			let _ = write!(reactor_uutils_ctx::stderr(), "{v}");
		}
	}
			...
			match cv.0.pop_var().as_isize() {
				Some(code) => {
					...
					if let Some(s) = cv.1.as_str() {
						let _ = write!(reactor_uutils_ctx::stdout(), "{s}");
					} else {
						let _ = writeln!(reactor_uutils_ctx::stdout(), "{}", cv.1);
					}
