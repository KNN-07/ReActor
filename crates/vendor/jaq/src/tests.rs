...
//! [`reactor_uutils_ctx::scope`], the way the shell host invokes the builtin.
...
fn run_jq_in(
	...
	let out = Buf::default();
	let err = Buf::default();
	let io_ = reactor_uutils_ctx::ScopeIo {
	...
	let code = reactor_uutils_ctx::scope(io_, || crate::run(argv));
