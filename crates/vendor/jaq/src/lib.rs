...
//! in-process as a shell builtin via [`reactor_uutils_ctx`].
//!
...
pub fn run(argv: Vec<std::ffi::OsString>) -> i32 {
	...
	if cli.version {
		...
		let _ = writeln!(reactor_uutils_ctx::stdout(), "{}", include_str!("help.txt"));
		return 0;
	}
	...
	match res {
		...
		Err(e) => {
			...
			let _ = write!(reactor_uutils_ctx::stderr(), "{e}");
			e.report()
		},
	}
}
...
fn real_main(cli: &Cli) -> Result<i32, Error> {
	if let Some(test_files) = &cli.run_tests {
		return Ok(match test_files.last() {
			Some(file) => {
				run_tests(io::BufReader::new(std::fs::File::open(reactor_uutils_ctx::resolve(file))?))
			},
			None => run_tests(io::BufReader::new(reactor_uutils_ctx::stdin())),
		});
	}
		...
		Some(filter) => {
			let (path, code) = match filter {
				cli::Filter::FromFile(path) => {
					(path.into(), std::fs::read_to_string(reactor_uutils_ctx::resolve(path))?)
				},
	...
	let last = if cli.files.is_empty() {
		let inputs = read::buffered(cli, io::BufReader::new(reactor_uutils_ctx::stdin()));
		...
		for file in &cli.files {
			...
			let resolved = reactor_uutils_ctx::resolve(file);
...
fn binds(cli: &Cli) -> Result<Vec<(String, Val)>, Error> {
	...
	let rawfile = cli.rawfile.iter().map(|(k, path)| {
		let s = std::fs::read_to_string(reactor_uutils_ctx::resolve(path))
	...
	let env = reactor_uutils_ctx::env_snapshot()
		.into_iter()
...
fn run_tests(read: impl BufRead) -> i32 {
	...
	for test in tests {
		...
		let _ = writeln!(reactor_uutils_ctx::stdout(), "Testing {}", test.filter);
		match run_test(test) {
			Err(e) => {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "{e:?}");
			},
			Ok((expect, obtain)) if expect != obtain => {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "expected {expect}, obtained {obtain}",);
			},
			Ok(_) => passed += 1,
		}
		total += 1;
	...
	let _ = writeln!(reactor_uutils_ctx::stdout(), "{passed} out of {total} tests passed");
