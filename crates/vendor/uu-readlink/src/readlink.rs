...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match readlink_main(&matches) {
		...
		Err(err) => {
			...
			if !msg.is_empty() {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "readlink: {msg}");
			}
...
fn readlink_main(matches: &ArgMatches) -> UResult<()> {
	...
	for p in &files {
		...
		let resolved = reactor_uutils_ctx::resolve(p);
		...
		match path_result {
			...
			Err(err) => {
				...
				let _ = writeln!(reactor_uutils_ctx::stderr(), "readlink: {message}");
				return Err(1.into());
			},
		}
	}
...
mod tests {
	...
	fn resolves_relative_operand_against_scope_cwd() {
		...
		// call-site `reactor_uutils_ctx::resolve` patch makes this find the link.
