...
impl CommandBuilderOptions {
	fn new(
		...
		Ok(Self { action, env, limiters, verbose: false, replace })
	}
}
...
impl CommandBuilder<'_> {
	...
	fn execute(self) -> Result<CommandResult, CommandExecutionError> {
		...
		if self.options.verbose {
			...
			// Upstream printed `Command`'s Debug representation; with the
			// scope environment attached via `env_clear().envs(..)` that
			// Debug form would dump the whole environment, so format the
			// program and arguments directly instead.
			let mut line = entry_point.to_string_lossy().into_owned();
			for arg in &final_args {
				line.push(' ');
				line.push_str(&arg.to_string_lossy());
			}
			let _ = writeln!(reactor_uutils_ctx::stderr(), "{line}");
		}

		match &self.options.action {
			ExecAction::Command(_) => {
				// Children must run in the scope's working directory (the
				// host process cwd belongs to the host shell, not this
				// command) and see the shell's exported environment, which
				// `do_xargs` snapshots into `self.options.env`.
				let mut command = Command::new(entry_point);
				command
					.args(&final_args)
					.current_dir(reactor_uutils_ctx::cwd())
					.env_clear()
...
fn run_command_captured(command: &mut Command) -> io::Result<ExitStatus> {
	...
	if let Some(mut out) = child.stdout.take() {
		let _ = io::copy(&mut out, &mut reactor_uutils_ctx::stdout());
	}
	let status = child.wait();
	if let Ok(buf) = stderr_thread.join() {
		let _ = reactor_uutils_ctx::stderr().write_all(&buf);
	}
	status
}
	...
	R: Read,
{
	fn new(rd: R, delimiter: u8) -> Self {
		Self { rd: BufReader::new(rd), delimiter }
	}
}
...
fn do_xargs(args: &[&str]) -> Result<CommandResult, XargsError> {
	...
	let matches = match matches {
		...
		Err(e) => match e.kind() {
			ErrorKind::DisplayHelp | ErrorKind::DisplayVersion => {
				...
				// newline here.
				let _ = write!(reactor_uutils_ctx::stdout(), "{e}");
	...
	// process environment.
	let env: HashMap<OsString, OsString> = reactor_uutils_ctx::env_snapshot()
		.into_iter()
	...
	let args_file: Box<dyn Read> = if let Some(path) = &options.arg_file {
		Box::new(
			fs::File::open(reactor_uutils_ctx::resolve(path))
				.map_err(|e| format!("Failed to open {path}: {e}"))?,
		)
	} else {
		Box::new(reactor_uutils_ctx::stdin())
	};
...
pub fn xargs_main(args: &[&str]) -> i32 {
	match do_xargs(args) {
		...
		Err(e) => {
			let _ = writeln!(reactor_uutils_ctx::stderr(), "Error: {e}");
