		...
		if filename == "-" {
			...
			let result = match mode {
				Mode::Bytes(ranges, opts) | Mode::Characters(ranges, opts) => {
					cut_bytes(reactor_uutils_ctx::stdin(), &mut out, ranges, opts)
				},
				Mode::Fields(ranges, opts) => {
					cut_fields(reactor_uutils_ctx::stdin(), &mut out, ranges, opts)
				},
			};
			if let Err(err) = result {
				let _ = writeln!(reactor_uutils_ctx::stderr(), "cut: {err}");
				reactor_uutils_ctx::set_exit_code(1);
			}
			stdin_read = true;
		} else {
			let result = File::open(reactor_uutils_ctx::resolve(Path::new(filename)))
...
/// Get delimiter and output delimiter from `-d`/`--delimiter` and
/// `--output-delimiter` options respectively Allow either delimiter to have a
/// value that is neither UTF-8 nor ASCII to align with GNU behavior
	...
	let delim = match delim_opt {
		...
		Some(os_string) => {
			if os_string.is_empty() {
				...
				// For delimiter `-d` option value - allow both UTF-8 (possibly multi-byte)
				// characters and Non UTF-8 (and not ASCII) single byte "characters", like
				// `b"\xAD"` to align with GNU behavior
				let bytes = os_str_as_bytes(os_string)?;
				if os_string.to_str().is_some_and(|s| s.chars().count() > 1)
					|| os_string.to_str().is_none() && bytes.len() > 1
				{
					return Err(USimpleError::new(1, "the delimiter must be a single character"));
				}
...
fn cut_main(matches: &ArgMatches) -> UResult<()> {
	...
	let mode_parse = match mode_parse {
		Err(_) => mode_parse,
		Ok(mode) => match mode {
			Mode::Bytes(..) | Mode::Characters(..) if matches.contains_id(options::DELIMITER) => Err(
				"invalid input: The '--delimiter' ('-d') option can only be used when printing a \
				 sequence of fields"
					.to_owned(),
			),
			...
			Mode::Bytes(..) | Mode::Characters(..) if matches.get_flag(options::ONLY_DELIMITED) => {
				Err(
					"invalid input: The '--only-delimited' ('-s') option can only be used when \
