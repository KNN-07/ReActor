...
// spell-checker:ignore rsplit hexdigit bitlen invalidchecksum inva idchecksum
// xffname
...
fn get_file_to_check(
	...
	if filename == "-" {
		Ok(Box::new(reactor_uutils_ctx::stdin())) // Use stdin if "-" is specified in the checksum file
	} else {
		let failed_open = || {
			write_file_report(
		...
		let print_error = |err: io::Error| {
			report_error(&err.map_err_context(|| {
				locale_aware_escape_name(filename, QuotingStyle::SHELL_ESCAPE)
					.to_string_lossy()
					.to_string()
			}));
		};
		match File::open(reactor_uutils_ctx::resolve(filename)) {
			Ok(f) => {
				if f
					.metadata()
					...
					.is_dir()
				{
					print_error(io::Error::new(io::ErrorKind::IsADirectory, "Is a directory"));
...
fn get_input_file(filename: &OsStr) -> UResult<Box<dyn Read>> {
	match File::open(reactor_uutils_ctx::resolve(filename)) {
		...
		Err(_) => Err(
			io::Error::other(format!("{}: {}", filename.maybe_quote(), "No such file or directory"))
				.into(),
		),
	}
}
...
fn process_checksum_file(
	...
	let file: Box<dyn Read> = if input_is_stdin {
		...
		match get_input_file(filename_input) {
			...
			Err(e) => {
				...
				let _ = writeln!(reactor_uutils_ctx::stderr(), "{}: {e}", command_name());
	...
	// cached_line_format is used to ensure that several non algo-based checksum
	// line will use the same parser.
