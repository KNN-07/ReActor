...
fn print_legacy_checksum(
	...
	match (options.algo_kind, sum) {
		...
		(SizedAlgoKind::Crc | SizedAlgoKind::Crc32b, DigestOutput::Crc(sum)) => {
			let _ = write!(reactor_uutils_ctx::stdout(), "{prefix}{sum} {size}");
		},
	...
	if escaped_filename != "-" {
		...
		let _dropped_result = reactor_uutils_ctx::stdout().write_all(escaped_filename.as_bytes());
	}
}
...
fn print_tagged_checksum(options: &ChecksumComputeOptions, filename: &OsStr, sum: &String) {
	...
	let _ = write!(reactor_uutils_ctx::stdout(), "{prefix}{} (", options.algo_kind.to_tag());

	// Print filename
	let _dropped_result = reactor_uutils_ctx::stdout().write_all(escaped_filename.as_bytes());

	// Print closing parenthesis and sum
	let _ = write!(reactor_uutils_ctx::stdout(), ") = {sum}");
}
...
fn print_untagged_checksum(
	...
	let _ = write!(reactor_uutils_ctx::stdout(), "{prefix}{sum} {}", match reading_mode {
		ReadingMode::Binary => '*',
		ReadingMode::Text => ' ',
	});
	...
	I: Iterator<Item = &'a OsStr>,
{
	...
	while let Some(filename) = files.next() {
		...
		let resolved_filepath = reactor_uutils_ctx::resolve(filepath);
		let stdin_buf;
		let file_buf;
		...
		let mut file = BufReader::with_capacity(
			READ_BUFFER_SIZE,
			if filename == "-" {
				stdin_buf = reactor_uutils_ctx::stdin();
		...
		match options.output_format {
			OutputFormat::Raw => {
				...
				digest_output.write_raw(reactor_uutils_ctx::stdout())?;
				return Ok(());
			},
			OutputFormat::Legacy => {
		...
		let _ = write!(reactor_uutils_ctx::stdout(), "{}", options.line_ending);
	}
	Ok(())
}
