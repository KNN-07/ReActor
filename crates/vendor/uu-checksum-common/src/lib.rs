...
pub(crate) fn report_error(error: &dyn std::fmt::Display) {
	let _ = writeln!(reactor_uutils_ctx::stderr(), "{}: {error}", command_name());
	reactor_uutils_ctx::set_exit_code(1);
}

pub(crate) fn report_warning(message: &str) {
	let _ = writeln!(reactor_uutils_ctx::stderr(), "{}: {message}", command_name());
}
...
fn finish_error(bin: &str, err: Box<dyn UError>) -> i32 {
	...
	if !message.is_empty() {
		let _ = writeln!(reactor_uutils_ctx::stderr(), "{bin}: {message}");
	}
