...
pub fn run(args: Vec<OsString>) -> i32 {
	...
	let args = match rewrite_bsd_invocation(&args) {
		...
		Some(Err(msg)) => {
			let _ = writeln!(reactor_uutils_ctx::stderr(), "tail: {msg}");
			return 1;
		},
	};
	let settings = match parse_settings(args) {
		...
		Err(ArgsError::Other(err)) => {
			let code = err.code();
			let _ = writeln!(reactor_uutils_ctx::stderr(), "tail: {err}");
...
fn uu_tail(settings: &Settings) -> UResult<()> {
	...
	if settings.debug && settings.follow.is_some() {
		if observer.use_polling {
			let _ = writeln!(reactor_uutils_ctx::stderr(), "tail: using polling mode");
		} else {
			let _ = writeln!(reactor_uutils_ctx::stderr(), "tail: using notification mode");
		}
	}
...
fn tail_stdin(
	...
	// streaming (pipe) path.
	header_printer.print_input(input);
	let mut reader = BufReader::new(reactor_uutils_ctx::stdin());
	unbounded_tail(&mut reader, settings)?;
	Ok(())
}
...
fn unbounded_tail<T: Read>(reader: &mut BufReader<T>, settings: &Settings) -> UResult<()> {
	let mut writer = BufWriter::new(reactor_uutils_ctx::stdout().lock());
	match &settings.mode {
...
mod tests {
	...
	fn run_in(cwd: PathBuf, args: Vec<&str>) -> (i32, String, String) {
		...
		let io = reactor_uutils_ctx::ScopeIo {
	...
	fn bounded_tail_broken_pipe_does_not_abort() {
		...
		let code = reactor_uutils_ctx::scope(io, || {
