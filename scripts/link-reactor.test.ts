...
const linkScript = path.join(repoRoot, "scripts", "link-reactor.sh");
const targetWrapper = path.join(repoRoot, "packages", "coding-agent", "scripts", "reactor");
const tempDirs: string[] = [];

function makeTempDir() {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-link-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("scripts/link-reactor.sh", () => {
	it("falls back to BUN_INSTALL/bin when bun cannot resolve the global bin", () => {
		...
		expect(fs.readlinkSync(path.join(bunInstall, "bin", "reactor"))).toBe(targetWrapper);
	});
	...
	it("uses bun pm -g bin when it succeeds", () => {
		...
		expect(result.status).toBe(0);
		expect(result.stderr).toBe("");
		expect(fs.readlinkSync(path.join(globalBin, "reactor"))).toBe(targetWrapper);
		expect(fs.existsSync(path.join(dir, "unused-bun-install", "bin", "reactor"))).toBe(false);
	});
});
