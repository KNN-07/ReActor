...
# ReActor Coding Agent Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/KNN-07/ReActor/main/scripts/install.sh | sh
#
# Options:
...
INSTALL_DIR="${REACTOR_INSTALL_DIR:-$HOME/.local/bin}"
...
install_via_bun() {
    ...
    echo ""
    echo "✓ Installed reactor via bun"
    echo "Run 'reactor' to get started!"
}
...
install_binary() {
    ...
    curl -fsSL --connect-timeout 10 --speed-limit 1024 --speed-time 30 "$BINARY_URL" -o "${INSTALL_DIR}/reactor"
    chmod +x "${INSTALL_DIR}/reactor"
    echo ""
    echo "✓ Installed reactor to ${INSTALL_DIR}/reactor"

    # Check if in PATH
    case ":$PATH:" in
        *":$INSTALL_DIR:"*) echo "Run 'reactor' to get started!" ;;
        *) echo "Add ${INSTALL_DIR} to your PATH, then run 'reactor'" ;;
    esac
}
