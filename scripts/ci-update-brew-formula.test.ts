...
const SUMS = {
	"reactor-darwin-arm64": "darwin_arm64_sha",
	"reactor-darwin-x64": "darwin_x64_sha",
	"reactor-linux-arm64": "linux_arm64_sha",
	"reactor-linux-x64": "linux_x64_sha",
};
...
describe("renderFormula", () => {
	...
	// `bin.install nil => "reactor"` raises (issue #2398).
	it("attaches `using: :nounzip` to every per-platform url stanza", () => {
		const matches = formula.match(/using: :nounzip/g) ?? [];
		expect(matches).toHaveLength(4);
		for (const arch of ["reactor-darwin-arm64", "reactor-darwin-x64", "reactor-linux-arm64", "reactor-linux-x64"]) {
	...
	// popened binary doesn't touch the real `~/.reactor` (denied by Homebrew's
	...
	it("wraps `generate_completions_from_executable` with a HOME redirect to buildpath", () => {
		expect(formula).toMatch(
			/with_env\(HOME: buildpath\) do\n\s+generate_completions_from_executable\(bin\/"reactor", "completions", shells: \[:bash, :zsh, :fish\]\)\n\s+end/,
		);
