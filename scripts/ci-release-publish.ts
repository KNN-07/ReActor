 ...
 *
 * The default mode publishes public JS packages and the `@reactor/natives`
...
export const packages: PublishPackage[] = [
	...
	{ dir: "packages/coding-agent", kind: "typescript", publishBin: { reactor: "dist/cli.js" } },
];
...
function buildNativeOptionalDependencies(version: string): JsonObject {
	...
	for (const target of LEAF_TARGETS) {
		optionalDependencies[`@reactor/natives-${target.tag}`] = version;
	}
...
async function packAndPublish(dir: string, name: string): Promise<void> {
	...
	const packDir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-pack-"));
	try {
