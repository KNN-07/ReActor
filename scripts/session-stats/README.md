...
(`~/.reactor/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.reactor/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.reactor/agent/sessions/ and populates ss_* tables
...
The LLM phase (default `anthropic/claude-sonnet-4-6` via `@reactor/ai`,
credentials resolved through reactor's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.reactor/stats-audit-cache.json` (keyed by session id +
