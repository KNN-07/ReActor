...
from reactor_rpc import (  # noqa: E402
    AgentEndEvent,
    AutoRetryEndEvent,
    AutoRetryStartEvent,
    ExtensionUiRequest,
...
def resolve_reactor_bin(raw: str | None) -> str:
    ...
    if not found:
        raise SystemExit("Could not find `reactor` on PATH. Set --reactor-bin or REACTOR_BIN.")
    return found
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run OpenRouter fixture evaluations through reactor RPC mode."
    )
    parser.add_argument("--reactor-bin", default=os.environ.get("REACTOR_BIN"))
...
async def run_all(args: argparse.Namespace) -> int:
    ...
    results_dir = (
        ...
        if args.results_dir
        else tmp_root / f"reactor-fixture-runs-{timestamp}"
    )
