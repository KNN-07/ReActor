# ReActor Coding Agent Installer for Windows
# Usage: irm https://raw.githubusercontent.com/KNN-07/ReActor/main/scripts/install.ps1 | iex
#
# Or with options:
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/KNN-07/ReActor/main/scripts/install.ps1))) -Source
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/KNN-07/ReActor/main/scripts/install.ps1))) -Binary
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/KNN-07/ReActor/main/scripts/install.ps1))) -Source -Ref v3.20.1
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/KNN-07/ReActor/main/scripts/install.ps1))) -Source -Ref main
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/KNN-07/ReActor/main/scripts/install.ps1))) -Binary -Ref v3.20.1
...
$Repo = "KNN-07/ReActor"
$Package = "@reactor/coding-agent"
$InstallDir = if ($env:REACTOR_INSTALL_DIR) { $env:REACTOR_INSTALL_DIR } else { "$env:LOCALAPPDATA\reactor" }
$BinaryName = "reactor-windows-x64.exe"
...
function Configure-BashShell {
    try {
        $settingsDir = Join-Path $env:USERPROFILE ".reactor\agent"
        ...
        if ($bashPath) {
            ...
            Write-Host "  ReActor requires a bash shell on Windows. Options:" -ForegroundColor Yellow
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("reactor-install-" + [System.Guid]::NewGuid().ToString("N"))
    ...
    Write-Host ""
    Write-Host "✓ Installed reactor via bun" -ForegroundColor Green

    Configure-BashShell

    Write-Host "Run 'reactor' to get started!"
}
...
function Install-Binary {
    ...
    $OutPath = Join-Path $InstallDir "reactor.exe"
    Invoke-WebRequest -Uri $BinaryUrl -OutFile $OutPath -TimeoutSec 900

    Write-Host ""
    Write-Host "✓ Installed reactor to $OutPath" -ForegroundColor Green
    ...
    if ($needsRestart) {
        Write-Host "Restart your terminal, then run 'reactor' to get started!"
    } else {
        Write-Host "Run 'reactor' to get started!"
    }
}
