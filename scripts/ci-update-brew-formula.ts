#!/usr/bin/env bun
//
// Render the Homebrew formula for `reactor` from a published GitHub release and write
// it to a tap checkout. The release publishes per-platform bare binaries
// (reactor-<platform>-<arch>); this reads their sha256 digests straight from the
// release metadata so the formula never drifts from the shipped assets.
//
// Usage:
//   bun scripts/ci-update-brew-formula.ts <tag> --out <path/to/Formula/reactor.rb>
...
export function renderFormula(version: string, sums: Record<string, string>): string {
	...
	// which nests the file outside the staging CWD; `Dir["reactor-*"].first` then
	// returns `nil` and `bin.install nil => "reactor"` raises.
	//
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/reactor-darwin-arm64",
          using: :nounzip
      sha256 "${sums["reactor-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/reactor-darwin-x64",
          using: :nounzip
      sha256 "${sums["reactor-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/reactor-linux-arm64",
          using: :nounzip
      sha256 "${sums["reactor-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/reactor-linux-x64",
          using: :nounzip
      sha256 "${sums["reactor-linux-x64"]}"
    end
  end

  def install
    bin.install Dir["reactor-*"].first => "reactor"
    (bin/"reactor").chmod 0555
    with_env(HOME: buildpath) do
      generate_completions_from_executable(bin/"reactor", "completions", shells: [:bash, :zsh, :fish])
    end
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/reactor --version")
  end
end
`;
}
...
async function main(): Promise<void> {
	...
	const targets = ["reactor-darwin-arm64", "reactor-darwin-x64", "reactor-linux-arm64", "reactor-linux-x64"];
