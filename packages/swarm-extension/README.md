...
Multi-agent orchestration for ReActor. Define agent workflows in YAML — pipelines, parallel fan-outs, sequential chains, or any DAG — and run them unattended until completion.

Each agent is a full ReActor subagent with access to every tool: bash, python, read, write, edit, grep, find, fetch, web_search, browser. The orchestrator manages lifecycle and ordering; agents communicate through the shared workspace filesystem.
...
reactor-swarm path/to/swarm.yaml

# Background — survives terminal close:
nohup reactor-swarm path/to/swarm.yaml \
...
Register the extension in your config (`~/.reactor/config.json` or `.reactor/config.json`):
...
Each agent is a full ReActor session. It can:
...
Any model configured in reactor works. Set a swarm default and optionally override per agent:
...
src/swarm/
  ...
  executor.ts         Spawns agents via ReActor's runSubprocess
