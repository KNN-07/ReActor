...
- Removed `harmony-leak` exports from the `@reactor/agent-core` package entrypoint
...
- Remote-compaction and summarization failures now throw ai's typed `ProviderHttpError` instead of mutating plain `Error`s with a `.status` property; the generic `requestRemoteCompaction` error now carries `.status` (and response headers) too.
...
- Removed `compaction/index.ts` re-export of snapcompact helpers, so snapcompact utilities are no longer available from the agent compaction barrel and should be imported from `@reactor/snapcompact`
...
- Added the `snapcompact` compaction strategy via `@reactor/snapcompact`: instead of an LLM summary, discarded history is printed onto dense bitmap frames and re-attached to the compaction summary message as image blocks. `CompactionSummaryMessage` gains an optional `images` field, `estimateTokens()` charges per attached frame, and frames persist under `preserveData.snapcompact` with an 8-frame middle-out eviction budget.
...
- Catalog imports moved to the new `@reactor/catalog` package: subpath imports (`calculateCost`, Codex wire constants) plus catalog values previously taken from the `@reactor/ai` root (`getBundledModel`, `clampThinkingLevelForModel`), which ai no longer re-exports; type-only `Model`/`Api`/`Effort` imports from ai are unchanged
...
- Added `getReadToolPath(context)` to `@reactor/agent-core/compaction/tool-protection` to extract a paired `read` tool call's `path` for embedders building read-targeted protection matchers
- Added `getReadToolPath(context)` to `@reactor/agent-core/compaction/tool-protection`: the shared primitive that extracts a paired `read` tool call's `path` argument, so embedders can build their own read-targeted compaction protection matchers (e.g. plan-file reads) the same way `isSkillReadToolResult` does.
...
- Removed the `@reactor/agent-core/compaction/handoff` exports from the package surface, including `extractHandoffDocument`, `createHandoffContext`, and `createHandoffFileName`
...
- Added `generateHandoff(messages, model, apiKey, options)` to `@reactor/agent-core/compaction` to generate a handoff document by calling the model directly, using live system/tool context and optional metadata
...
- Added compaction APIs under `@reactor/agent-core/compaction`, including context compaction, branch summarization, handoff prompt/context helpers, pruning, token budgeting, prompt templates, and OpenAI `/responses/compact` helpers.
...
- Switched from local `@reactor/ai` to upstream `@mariozechner/ai` package
...
- **Agent loop moved from `@reactor/ai`**: The `agentLoop`, `agentLoopContinue`, and related types have moved to this package. Import from `@reactor/pi-agent` instead.

### Added

- `streamFn` option on `Agent` for custom stream implementations. Default uses `streamSimple` from ai.
