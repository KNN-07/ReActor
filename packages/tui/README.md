...
```typescript
import { TUI, Text, Editor, ProcessTerminal } from "@reactor/tui";
...
```typescript
import { CombinedAutocompleteProvider } from "@reactor/tui";
...
All updates are wrapped in **synchronized output** (`\x1b[?2026h` ... `\x1b[?2026l`) for atomic, flicker-free rendering unless `REACTOR_NO_SYNC_OUTPUT=1` is set. The opt-out removes only the DEC 2026 wrapper; paint writes still guard terminal autowrap to avoid pending-wrap cursor artifacts.
...
```typescript
import { Ellipsis, visibleWidth, truncateToWidth, wrapTextWithAnsi } from "@reactor/tui";
...
```typescript
import { isEnter, isEscape, isArrowUp, isArrowDown, isCtrlC, isTab, isBackspace } from "@reactor/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@reactor/tui";
import type { Component } from "@reactor/tui";
