...
describe("synchronizedOutputUserOverride", () => {
	...
	it("returns false for either opt-out flag", () => {
		expect(synchronizedOutputUserOverride({ REACTOR_NO_SYNC_OUTPUT: "1" })).toBe(false);
		expect(synchronizedOutputUserOverride({ REACTOR_TUI_SYNC_OUTPUT: "0" })).toBe(false);
	});

	it("returns true for either force-on flag", () => {
		expect(synchronizedOutputUserOverride({ REACTOR_FORCE_SYNC_OUTPUT: "1" })).toBe(true);
		expect(synchronizedOutputUserOverride({ REACTOR_TUI_SYNC_OUTPUT: "1" })).toBe(true);
	});

	it("resolves opt-out ahead of force-on when both are set", () => {
		expect(synchronizedOutputUserOverride({ REACTOR_NO_SYNC_OUTPUT: "1", REACTOR_FORCE_SYNC_OUTPUT: "1" })).toBe(
			false,
		);
		expect(synchronizedOutputUserOverride({ REACTOR_TUI_SYNC_OUTPUT: "0", REACTOR_FORCE_SYNC_OUTPUT: "1" })).toBe(
			false,
		);
	});
});
...
describe("shouldEnableSynchronizedOutputByDefault", () => {
	...
	it("lets a user opt-out beat every positive heuristic", () => {
		expect(shouldEnableSynchronizedOutputByDefault({ REACTOR_NO_SYNC_OUTPUT: "1" }, "kitty")).toBe(false);
		expect(shouldEnableSynchronizedOutputByDefault({ REACTOR_TUI_SYNC_OUTPUT: "0" }, "ghostty")).toBe(false);
		expect(
			shouldEnableSynchronizedOutputByDefault(
				{ REACTOR_NO_SYNC_OUTPUT: "1", WT_SESSION: "abc", TERM_FEATURES: "Sy" },
				"kitty",
			),
		).toBe(false);
	});
	...
	it("lets a user force-on beat the conservative defaults", () => {
		expect(shouldEnableSynchronizedOutputByDefault({ REACTOR_FORCE_SYNC_OUTPUT: "1" }, "base")).toBe(true);
		expect(shouldEnableSynchronizedOutputByDefault({ REACTOR_TUI_SYNC_OUTPUT: "1", TMUX: "1" }, "base")).toBe(true);
		expect(
			shouldEnableSynchronizedOutputByDefault({ REACTOR_FORCE_SYNC_OUTPUT: "1", SSH_CONNECTION: "1 2 3 4" }, "base"),
		).toBe(true);
	});
});
	...
	it("resolves the process-wide Warp terminal id and image protocol from TERM_PROGRAM", async () => {
		...
		const proc = Bun.spawn({
			cmd: [
				process.execPath,
				"--eval",
				`import { ImageProtocol, TERMINAL, TERMINAL_ID } from "@reactor/tui/terminal-capabilities";
console.log(JSON.stringify({ id: TERMINAL_ID, imageProtocol: TERMINAL.imageProtocol, expected: ImageProtocol.Kitty }));`,
	...
	it("leaves synchronized output off by default and honors hyperlink force-on", () => {
		...
		expect(shouldEnableHyperlinksByDefault({ REACTOR_FORCE_HYPERLINKS: "1" }, "warp")).toBe(true);
	});
});
...
describe("hyperlinksUserOverride", () => {
	...
	it("returns true for the force-on flag", () => {
		expect(hyperlinksUserOverride({ REACTOR_FORCE_HYPERLINKS: "1" })).toBe(true);
	});

	it("returns false for the opt-out flag", () => {
		expect(hyperlinksUserOverride({ REACTOR_NO_HYPERLINKS: "1" })).toBe(false);
	});

	it("resolves opt-out ahead of force-on when both are set", () => {
		expect(hyperlinksUserOverride({ REACTOR_NO_HYPERLINKS: "1", REACTOR_FORCE_HYPERLINKS: "1" })).toBe(false);
	});
	...
	it("ignores values other than the literal '1'", () => {
		...
		expect(hyperlinksUserOverride({ REACTOR_FORCE_HYPERLINKS: "true" })).toBeNull();
		expect(hyperlinksUserOverride({ REACTOR_FORCE_HYPERLINKS: "0" })).toBeNull();
		expect(hyperlinksUserOverride({ REACTOR_NO_HYPERLINKS: "0" })).toBeNull();
	});
});
...
describe("shouldEnableHyperlinksByDefault", () => {
	...
	it("lets REACTOR_NO_HYPERLINKS beat every positive heuristic", () => {
		expect(shouldEnableHyperlinksByDefault({ REACTOR_NO_HYPERLINKS: "1" }, "kitty")).toBe(false);
		expect(
			shouldEnableHyperlinksByDefault(
				{ REACTOR_NO_HYPERLINKS: "1", TERM_PROGRAM: "tmux", TERM_PROGRAM_VERSION: "3.5a", TMUX: "1" },
				"wezterm",
			),
		).toBe(false);
	});

	it("lets REACTOR_FORCE_HYPERLINKS override the conservative defaults (old tmux, screen, base terminal)", () => {
		expect(shouldEnableHyperlinksByDefault({ REACTOR_FORCE_HYPERLINKS: "1" }, "base")).toBe(true);
		expect(shouldEnableHyperlinksByDefault({ REACTOR_FORCE_HYPERLINKS: "1", TMUX: "1" }, "wezterm")).toBe(true);
		expect(shouldEnableHyperlinksByDefault({ REACTOR_FORCE_HYPERLINKS: "1", STY: "1.pts-0" }, "kitty")).toBe(true);
	});
});
