	...
	#consumePrivateModeSequence(
		...
		if (params.includes(2026)) {
			if (this.#traits.syncOutputDisabled) {
				this.#fail(
					final === "h"
						? "synchronized-output begin emitted while REACTOR_NO_SYNC_OUTPUT is set"
						: "synchronized-output end emitted while REACTOR_NO_SYNC_OUTPUT is set",
					op,
					before,
					after,
					index,
...
function scenarioEnv(envMode: EnvMode): Record<EnvKey, string | undefined> {
	return {
		...
		REACTOR_NO_SYNC_OUTPUT: envMode === "vteNoSync" ? "1" : undefined,
...
function writeReplayLog(scenario: Scenario, operations: readonly OperationLogEntry[]): string {
	const filePath = path.join(
		os.tmpdir(),
		`reactor-tui-stress-${scenario.name}-${(scenario.seed >>> 0).toString(16)}-${Date.now().toString(36)}.json`,
	);
