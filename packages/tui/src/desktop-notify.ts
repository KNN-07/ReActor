 ...
 * session, and (3) the user has not opted out via `REACTOR_NO_DESKTOP_NOTIFY=1`.
...
export function shouldDeliverDesktopNotification(
	...
	if (env.REACTOR_NO_DESKTOP_NOTIFY === "1") return false;
	return true;
}
...
export function sendDesktopNotification(message: string | TerminalNotification): void {
	...
	try {
		...
		// `gdbus` waiting on a stalled session bus) would keep `reactor` alive past
