...
import type { Api, Context, ImageContent, Model, OptionsForApi, UserMessage } from "@reactor/ai/types";
import { getBundledModel } from "@reactor/catalog/models";
import { $which, removeSyncWithRetries } from "@reactor/utils";
import { e2eApiKey } from "./oauth";

const TEMP_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-temp-images-"));
