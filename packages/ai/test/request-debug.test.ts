...
import { clearCustomApis, registerCustomApi } from "@reactor/ai/api-registry";
import { stream } from "@reactor/ai/stream";
import type { AssistantMessage, FetchImpl, Model, ModelSpec } from "@reactor/ai/types";
import { AssistantMessageEventStream } from "@reactor/ai/utils/event-stream";
import { wrapFetchForRequestDebug } from "@reactor/ai/utils/request-debug";
...
beforeEach(async () => {
	previousDebugFlag = Bun.env.REACTOR_REQ_DEBUG;
...
afterEach(async () => {
	...
	if (previousDebugFlag === undefined) delete Bun.env.REACTOR_REQ_DEBUG;
	else Bun.env.REACTOR_REQ_DEBUG = previousDebugFlag;
...
/** Find the latest rr-session-*.json written to the temp dir by REACTOR_REQ_DEBUG
...
describe("REACTOR_REQ_DEBUG request/response recording", () => {
	it("leaves fetch untouched when the flag is disabled", () => {
		delete Bun.env.REACTOR_REQ_DEBUG;
