...
import { __providerInFlightForTesting, configureProviderMaxInFlightRequests, streamSimple } from "@reactor/ai/stream";
import type { Context } from "@reactor/ai/types";
...
async function useIsolatedLimiterRoot(): Promise<void> {
	limiterRoot = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-provider-inflight-test-"));
