 ...
 *     → anthropic-messages parser → reactor Context (cacheRetention derived)
 *     → ai openai-codex-responses provider
 ...
 * directly — caching only works if ai's codex transport reaches Codex
 ...
 * (override via `REACTOR_E2E_GATEWAY_URL`) AND the bearer token file exists at
...
const MODEL = Bun.env.REACTOR_E2E_CODEX_MODEL ?? "gpt-5.3-codex";
...
the reactor auth-gateway's cross-protocol prompt-caching pipeline. The request
...
describe.skipIf(!gateway.ok)("auth-gateway: anthropic-messages → openai-codex caching e2e", () => {
	...
	it("caches the system prefix across a cross-protocol translate (messages→codex)", async () => {
		...
		//   - the anthropic-messages encoder forgot to map ai's
