 ...
 *     → openai-responses parser → reactor Context
 *     → ai anthropic provider (auto cache_control via cacheRetention)
 ...
 * with the gateway live (`reactor auth-gateway serve` or pm2).
 */
...
const MODEL = Bun.env.REACTOR_E2E_ANTHROPIC_MODEL ?? "claude-sonnet-4-5";
