...
describe("streamPiNative request shape", () => {
	...
	it("forwards `model.headers` and lets a caller-supplied Authorization win", async () => {
		...
		const headers = captured.init?.headers as Record<string, string>;
		expect(headers["x-reactor-slot"]).toBe("reactor-worker-1");
...
describe("streamPiNative event flow", () => {
	...
	it("uses REACTOR_STREAM_FIRST_EVENT_TIMEOUT_MS for silent reactor-native streams", async () => {
		const previous = Bun.env.REACTOR_STREAM_FIRST_EVENT_TIMEOUT_MS;
		...
		try {
			...
			if (previous === undefined) {
				...
				Bun.env.REACTOR_STREAM_FIRST_EVENT_TIMEOUT_MS = previous;
			}
		}
	});

	it("rejects when a reactor-native stream stalls after semantic progress", async () => {
		const partial = baseAssistant({ content: [{ type: "text", text: "hi" }] });
		const chunks = [
	...
	it("does not time out a healthy reactor-native stream that keeps making semantic progress", async () => {
