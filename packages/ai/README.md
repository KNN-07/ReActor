...
```bash
npm install @reactor/ai
```

## Quick Start

```typescript
import { z, getModel, stream, complete, Context, Tool } from "@reactor/ai";
...
```typescript
import { stream, validateToolCall, Tool } from "@reactor/ai";
...
```typescript
import * as fs from "node:fs";
import { getModel, complete } from "@reactor/ai";
...
```typescript
import { getModel, streamSimple, completeSimple } from "@reactor/ai";
...
```typescript
import { getModel, stream } from "@reactor/ai";
...
```typescript
import { getProviders, getModels, getModel } from "@reactor/ai";
...
```typescript
import { Model, stream } from "@reactor/ai";
...
```typescript
import { getModel, complete, Context } from "@reactor/ai";
...
```typescript
import { Context, getModel, complete } from "@reactor/ai";
...
bundles. Bun's `fetch` does not consume the env var natively, so reactor injects
...
```typescript
import { getEnvApiKey } from "@reactor/ai";
...
Authenticate via the [`reactor`](https://reactor.norman.id.vn) coding-agent CLI, which drives this library's OAuth/API-key flows in-process and persists into `agent.db`:

```bash
reactor auth-broker login              # interactive provider selection
reactor auth-broker login anthropic    # login to a specific provider
reactor auth-broker login vllm         # store vLLM API key (or placeholder for local no-auth)
reactor auth-broker list               # list supported providers
reactor auth-broker logout             # interactive — pick a stored credential to remove
```
...
```typescript
import { loginGitHubCopilot } from "@reactor/ai";
import * as fs from "node:fs";
...
```typescript
import { getModel, complete, getOAuthApiKey } from "@reactor/ai";
import * as fs from "node:fs";
