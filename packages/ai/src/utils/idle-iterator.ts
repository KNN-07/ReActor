 ...
 *
 * `REACTOR_OPENAI_STREAM_IDLE_TIMEOUT_MS` is accepted as a backward-compatible alias.
 * Set `REACTOR_STREAM_IDLE_TIMEOUT_MS=0` to disable the watchdog.
 *
...
export function getStreamIdleTimeoutMs(fallbackMs: number = DEFAULT_STREAM_IDLE_TIMEOUT_MS): number | undefined {
	return normalizeIdleTimeoutMs(
		$env.REACTOR_STREAM_IDLE_TIMEOUT_MS ?? $env.REACTOR_OPENAI_STREAM_IDLE_TIMEOUT_MS,
		fallbackMs,
	);
}
 ...
 *
 * `REACTOR_OPENAI_STREAM_IDLE_TIMEOUT_MS` takes precedence over the generic
 * `REACTOR_STREAM_IDLE_TIMEOUT_MS` because some deployments tune OpenAI-compatible
 * backends separately from Anthropic/Gemini-style transports.
 *
 * Set `REACTOR_OPENAI_STREAM_IDLE_TIMEOUT_MS=0` to disable the watchdog.
 */
export function getOpenAIStreamIdleTimeoutMs(fallbackMs: number = DEFAULT_STREAM_IDLE_TIMEOUT_MS): number | undefined {
	return normalizeIdleTimeoutMs(
		$env.REACTOR_OPENAI_STREAM_IDLE_TIMEOUT_MS ?? $env.REACTOR_STREAM_IDLE_TIMEOUT_MS,
		fallbackMs,
	);
}
 ...
 *
 * Set `REACTOR_STREAM_FIRST_EVENT_TIMEOUT_MS=0` to disable the watchdog.
 *
...
export function getStreamFirstEventTimeoutMs(
	...
	return normalizeIdleTimeoutMs($env.REACTOR_STREAM_FIRST_EVENT_TIMEOUT_MS, fallback);
}

/**
 * Returns the first-event timeout used for OpenAI-family streaming transports.
 *
 * Precedence: explicit `REACTOR_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS` (including a
 ...
 * `streamIdleTimeoutMs` or `REACTOR_OPENAI_STREAM_IDLE_TIMEOUT_MS` resolved
 * upstream) floors the first-event budget so slow local OpenAI-compatible
 * servers are not undercut by a shorter `REACTOR_STREAM_FIRST_EVENT_TIMEOUT_MS`
...
export function getOpenAIStreamFirstEventTimeoutMs(
	...
	const openAIFirstEventRaw = $env.REACTOR_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS;
	if (openAIFirstEventRaw !== undefined) {
		return normalizeIdleTimeoutMs(openAIFirstEventRaw, fallbackMs);
	}
	const base = normalizeIdleTimeoutMs($env.REACTOR_STREAM_FIRST_EVENT_TIMEOUT_MS, fallbackMs);
