...
/**
 * Normalizes provider id (e.g. github-copilot -> REACTOR_PROXY_GITHUB_COPILOT) and looks it up.
 * If not found, falls back to REACTOR_PROXY. Results are memoized because env values are static
...
export function getProxyForProvider(provider: string): string | undefined {
	...
	const envKey = `REACTOR_PROXY_${normalized}`;
	const value = Bun.env[envKey] || Bun.env.REACTOR_PROXY;
