 ...
 * wire shapes through ai's canonical {@link Context}, this module accepts
 * the canonical shape *directly* — for clients that already speak ai
 * (containerized reactor, the swarm extension, reactor-worker's sidecar auth-gateway).
 ...
 * translations impose on first-class ai fields (service tier, cache
 * markers, thinking budgets, tool-choice variants, …).
 *
 * The streaming wire is {@link AssistantMessageEvent} serialized verbatim and
 * SSE-framed. Same type ai already produces internally; the client feeds
 ...
 * `SimpleStreamOptions` from older / newer reactor builds without per-version
 * conditionals.
 */
...
/**
 * Parse a reactor-native request body. Validation is intentionally minimal — only
 * the shape the gateway itself reads is checked (`modelId`, `context.messages`
 * array, options is an object). Everything downstream is the canonical ai
...
export function parseRequest(body: unknown, _headers?: Headers): PiNativeParsedRequest {
	...
	// `stream` defaults to true — reactor-native clients overwhelmingly stream, and
 ...
 *
 * No per-event re-shaping: the reactor-native client is ai itself, so the
 ...
 * targets (containerized reactor → host gateway, reactor-worker slot → reactor-auth-gateway
 ...
 * provider-specific status taxonomy — reactor-native callers consume `type`
 * directly.
 */
