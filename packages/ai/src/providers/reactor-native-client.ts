 ...
 * way.
 *
 * Activated when a {@link Model} has `transport: "reactor-native"` set; the
 * dispatch hook lives in `streamSimple()` (see `../stream.ts`). Used by
 * containerized reactor deployments (reactor-worker slots, the swarm extension) that
 * route every LLM call through a credential-holding sidecar so the slot
 * itself stays credential-free.
 */
import { readSseJson } from "@reactor/utils";
...
const REACTOR_NATIVE_STREAM_IDLE_TIMEOUT_ERROR = "reactor-native stream stalled while waiting for the next event";
...
/**
 * Stream a turn through an `reactor auth-gateway` over the reactor-native protocol.
 *
	...
	void (async () => {
		...
		try {
			...
			const watchedSource = iterateWithIdleTimeout(source, {
				...
				errorMessage: REACTOR_NATIVE_STREAM_IDLE_TIMEOUT_ERROR,
				firstItemErrorMessage: REACTOR_NATIVE_STREAM_FIRST_EVENT_TIMEOUT_ERROR,
				onIdle: () =>
					abortTracker.abortLocally(new AIError.StreamTimeoutError(REACTOR_NATIVE_STREAM_IDLE_TIMEOUT_ERROR)),
				onFirstItemTimeout: () =>
			...
			if (!sawTerminal) {
				...
				// `reactor-native-server.encodeStream`.
