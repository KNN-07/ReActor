...
- Added the `ToolCallSyntax` union and `FALLBACK_TOOL_SYNTAX` constant to `@reactor/catalog/identity` (re-exported from `@reactor/ai/grammar`).
- Added `preferredToolSyntax(modelId)` to `@reactor/catalog/identity`, resolving a model's native tool-call syntax affinity from its family token (Claude→`anthropic`, GLM→`glm`, Kimi→`kimi`, Qwen→`qwen3`, DeepSeek→`deepseek`, OpenAI/gpt-oss→`harmony`, else the `xml` fallback).
...
- Added `modelFamilyToken(modelId)` to `@reactor/catalog/identity`: a coarse vendor-lineage token (`anthropic`/`openai`/`gemini`/`kimi`/…) for "are two models the same family?" comparisons, backed by `parseKnownModel` canonical-id normalization. Opaque and comparison-only; kind/variant collapsed onto the vendor token ([#2406](https://github.com/can1357/oh-my-pi/issues/2406))
...
- Wired `@reactor/catalog` into the release publish package list, tarball install smoke test, and root `bun generate-models` script.
