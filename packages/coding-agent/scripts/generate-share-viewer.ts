#!/usr/bin/env bun
/**
 * Build the standalone share-viewer page the reactor relay serves at `GET /s/<id>`.
 *
 ...
 * hands the JSON to template.js via `window.__REACTOR_SESSION_DATA__`.
 *
...
// Pin the reactor brand palette (collab-web pink/purple identity) — the viewer is
// a public artifact matching the live reactor.norman.id.vn client, not a per-user export
...
const html = getTemplate()
	...
	.replace("<title>Session Export</title>", () => "<title>reactor session</title>")
...
if (!html.includes("__REACTOR_SESSION_DATA__")) throw new Error("share loader not injected");
