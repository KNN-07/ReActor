/**
 * Bun `--preload` shim for the reactor dev launcher (`scripts/reactor`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the reactor CLI: Bun reads
 ...
 * import-time snapshots (e.g. `getProjectDir()` in `@reactor/utils/dirs`).
 */
const launchCwd = process.env.REACTOR_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.REACTOR_LAUNCH_CWD;
	try {
