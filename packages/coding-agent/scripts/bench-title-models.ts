 ...
 *   bun scripts/bench-title-models.ts --db ~/.reactor/agent/history.db --out bench.json
 */
...
function parseArgs(argv: string[]): BenchConfig {
	...
	return {
		dbPath: (get("--db") ?? path.join(os.homedir(), ".reactor/agent/history.db")).replace(/^~/, os.homedir()),
