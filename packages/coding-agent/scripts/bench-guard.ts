 ...
 *
 * Re-runs the `REACTOR_TIMING=x` cold-boot benchmark under hyperfine and fails when
 * the median regresses past `baseline * THRESHOLD`. `REACTOR_TIMING=x` runs the full
...
const BENCH_COMMAND = "REACTOR_TIMING=x REACTOR_STRICT_EDIT_MODE=1 bun src/cli.ts";
