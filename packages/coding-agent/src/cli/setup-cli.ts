 ...
 *
 * Handles `reactor setup` for onboarding and `reactor setup <component>` for optional dependencies.
 */
import * as path from "node:path";
import { $which, APP_NAME, getProjectDir, getPythonEnvDir } from "@reactor/utils";
import { $ } from "bun";
...
// package dependencies beyond a working interpreter. `reactor setup python --check`
...
/**
 * Unified `reactor setup speech` flow. Drives every {@link SpeechComponent} through
