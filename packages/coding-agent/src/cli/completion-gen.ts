 ...
 */
import type { ArgDescriptor, CliConfig, CommandCtor, FlagDescriptor } from "@reactor/utils/cli";
...
function bashValueBranch(bin: string, v: ValueSource): string {
	switch (v.kind) {
		...
		case "list":
			return `_reactor_comma "${bashWords(v.values)}"; return 0`;
		case "models":
			return v.multiple
				? `_reactor_comma "$(command ${bin} __complete models 2>/dev/null | cut -f1)"; return 0`
	...
	for (const c of spec.commands) {
		...
		const elseBranch = argWords
			...
			: fileArg
				...
				: ":";
		parts.push(`_reactor_cmd_${bashFn(c.name)}() {
	case "$prev" in
${bashFlagCase(bin, c.flags)}
	...
	for (const c of spec.commands) {
		dispatch.push(`\t\t${commandTokens(c).join("|")})\n\t\t\t_reactor_cmd_${bashFn(c.name)}\n\t\t\t;;`);
	}
	parts.push(`_reactor() {
	local cur prev cmd i
	cur="\${CPI_WORDS[CPI_CWORD]}"
	prev="\${CPI_WORDS[CPI_CWORD-1]}"
	cmd=""
	for (( i=1; i < CPI_CWORD; i++ )); do
		case "\${CPI_WORDS[i]}" in
			-*) ;;
			*) cmd="\${CPI_WORDS[i]}"; break ;;
		esac
	done
	case "$cmd" in
${dispatch.join("\n")}
...
function zshAction(v: ValueSource): string {
	switch (v.kind) {
		...
		case "list":
			return ":value:_reactor_tools";
		case "models":
			return v.multiple ? ":models:_reactor_models_list" : ":model:_reactor_call models";
		case "sessions":
			return ":session:_reactor_call sessions";
		case "file":
			return ":file:_files";
		case "dir":
			return ":dir:_files -/";
...
function generateZsh(spec: CompletionSpec): string {
	const { bin } = spec;
	// The `:value:_reactor_tools` action references this helper; bake its candidates
...
${cmdRows}
	...
	const aliasArms = spec.commands
		.map(c => `\t\t\t${commandTokens(c).join("|")}) _reactor_cmd_${bashFn(c.name)} ;;`)
		.join("\n");
	const rootSpecs = [
...
# Works both ways: autoloaded from $fpath (file named _reactor) or eval'd from a
# startup file. When autoloaded, funcstack[1] is _reactor and we invoke it; when
# sourced/eval'd we register it with compdef instead.
if [ "$funcstack[1]" = "_reactor" ]; then
	_reactor "$@"
else
	compdef _reactor ${bin}
fi`);
	parts.push("");
	return `${parts.join("\n")}\n`;
}
...
function generateFish(spec: CompletionSpec): string {
	...
	lines.push(`function __fish_reactor_no_subcommand`);
	...
	const rootCond = "__fish_reactor_no_subcommand";
