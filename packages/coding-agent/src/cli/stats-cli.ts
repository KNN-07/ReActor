 ...
 *
 * Handles `reactor stats` subcommand for viewing AI usage statistics.
 */

import { truncateToWidth } from "@reactor/tui/utils";
import { APP_NAME, formatDuration, formatNumber, formatPercent } from "@reactor/utils";
...
async function printStatsSummary(): Promise<void> {
	const { getDashboardStats } = await import("@reactor/stats");
	const stats = await getDashboardStats();
