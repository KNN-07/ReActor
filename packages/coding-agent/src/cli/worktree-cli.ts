/**
 * CLI handler for `reactor worktree` — list and clean up agent-managed worktrees.
 *
 * Layout under `~/.reactor/wt/`:
 *
...
import { getWorktreesDir, isEnoent } from "@reactor/utils";
...
export interface WorktreeEntry {
	/** Absolute path to the worktree dir (or stray container) under `~/.reactor/wt/`. */
	path: string;
	...
	branch?: string;
	/** When set, the entry is unhealthy and `reactor worktree clear` will remove it. */
	orphanReason?: string;
}
...
async function scanWorktrees(): Promise<WorktreeEntry[]> {
	...
	for (const name of topLevel) {
		...
		// Legacy nesting: ~/.reactor/wt/<encoded-project>/<branch-or-id>
		let children: string[];
		try {
