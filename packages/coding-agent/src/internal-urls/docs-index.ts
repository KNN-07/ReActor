/**
 * Harness documentation index for the `reactor://` protocol.
 *
 * Compiled binaries and the prepacked npm bundle inline a compressed index of the
 * docs (injected via `process.env.REACTOR_DOCS_EMBED` at build time). The format is two lines:
...
const docsEmbed = process.env.REACTOR_DOCS_EMBED ?? "";
