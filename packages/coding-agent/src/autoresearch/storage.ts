...
import { getAutoresearchDbPath, getAutoresearchProjectDir, logger } from "@reactor/utils";
 ...
 *
 * Used to key per-project autoresearch state under `~/.reactor/autoresearch/`.
...
async function resolveAutoresearchPaths(cwd: string): Promise<{ dbPath: string; projectDir: string }> {
	const override = process.env.REACTOR_AUTORESEARCH_DB_DIR;
