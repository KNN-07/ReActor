...
export interface PythonExecutorOptions {
	 ...
	 * dir. When present, exported to the kernel as `REACTOR_ARTIFACTS_DIR` and
	 * preferred over `REACTOR_SESSION_FILE`-derived paths.
	 */
	artifactsDir?: string;
	 ...
	 * the kernel as `REACTOR_EVAL_LOCAL_ROOTS` (JSON) so `write("local://x")` lands
