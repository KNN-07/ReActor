...
# ReActor prelude helpers (loaded once into the runner namespace)
if "__reactor_prelude_loaded__" not in globals():
    __reactor_prelude_loaded__ = True
    ...
    # __reactor_display is injected by runner.py before the prelude executes; it
    # mirrors IPython's display() semantics with the same MIME bundle output.
    _reactor_display = __reactor_display  # type: ignore[name-defined]
    ...
    def _emit_status(op: str, **data):
        ...
        _reactor_display({"application/x-reactor-status": {"op": op, **data}}, raw=True)
    ...
    _REACTOR_INTERNAL_URL_RE = re.compile(r"^([a-z][a-z0-9+.-]*)://(.*)$", re.IGNORECASE)

    def _should_delegate_read(path: str | Path) -> bool:
        return (
            isinstance(path, str)
            and _REACTOR_INTERNAL_URL_RE.match(path) is not None
    ...
    def _resolve_reactor_path(path: str | Path) -> Path:
        """Map a helper path to a real filesystem Path.

        A `scheme://…` whose scheme has an injected on-disk root (e.g.
        `local://`, via REACTOR_EVAL_LOCAL_ROOTS) is rewritten under that root so it
        ...
        if not isinstance(path, str):
            return Path(path)
        match = _REACTOR_INTERNAL_URL_RE.match(path)
        if not match:
            return Path(path)
        scheme = match.group(1).lower()
        try:
            roots = json.loads(os.environ.get("REACTOR_EVAL_LOCAL_ROOTS") or "{}")
    ...
    def output(
        *ids: str,
        ...
        """
        # Prefer REACTOR_ARTIFACTS_DIR so subagents resolve through the parent's
        # shared artifacts dir; fall back to deriving from REACTOR_SESSION_FILE
        # for legacy callers / top-level sessions where the two coincide.
        artifacts_dir = os.environ.get("REACTOR_ARTIFACTS_DIR")
        if not artifacts_dir:
            session_file = os.environ.get("REACTOR_SESSION_FILE")
            if not session_file:
    ...
    def _tool_proxy_from_env() -> tuple[str, str, str]:
        base = os.environ.get("REACTOR_TOOL_BRIDGE_URL")
        token = os.environ.get("REACTOR_TOOL_BRIDGE_TOKEN")
        session = os.environ.get("REACTOR_TOOL_BRIDGE_SESSION")
    ...
    def _bridge_call(name: str, args: dict):
        ...
        _run_id_getter = globals().get("__reactor_current_run_id__")
        _run_id = (
            _run_id_getter()
            if callable(_run_id_getter)
            else globals().get("__reactor_run_id__")
        )
        payload = json.dumps(
    ...
    def phase(title):
        ...
        globals()["__reactor_current_phase__"] = str(title)
