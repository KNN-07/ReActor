...
# ReActor Ruby prelude helpers (loaded once into the runner's TOPLEVEL_BINDING).
#
...
# uses (REACTOR_TOOL_BRIDGE_URL/TOKEN/SESSION). Path helpers honor REACTOR_EVAL_LOCAL_ROOTS
# so `write("local://x")` lands where `read local://x` resolves.
#
# `__reactor_*` primitives (emit/present/status/scrub/run_id) are provided by
# runner.rb; this file only consumes them.

unless defined?($__reactor_prelude_loaded) && $__reactor_prelude_loaded
  $__reactor_prelude_loaded = true
  ...
  # an injected on-disk root (REACTOR_EVAL_LOCAL_ROOTS, e.g. `local://`) is rewritten
  # under that root; plain paths pass through; any other `scheme://` is rejected.
  def __reactor_resolve_path(path)
    ...
    roots =
      begin
        raw = ENV["REACTOR_EVAL_LOCAL_ROOTS"]
    ...
    relative = __reactor_url_decode(m[2].tr("\\", "/"))
  ...
  def display(value)
    __reactor_present(value, "display")
    nil
  end
  ...
  def display_image(base64, mime_type: "image/png")
    __reactor_emit_display({ mime_type.to_s => base64.to_s })
    nil
  end
  ...
  def env(key = nil, value = nil)
    if key.nil?
      ...
      __reactor_emit_status("env", "count" => items.size, "keys" => items.keys.first(20))
      return items
    end
    unless value.nil?
      ENV[key.to_s] = value.to_s
      __reactor_emit_status("env", "key" => key.to_s, "value" => value.to_s, "action" => "set")
      return value
    end
    val = ENV[key.to_s]
    __reactor_emit_status("env", "key" => key.to_s, "value" => val, "action" => "get")
    val
  end
  ...
  def read(path, offset = 1, limit = nil)
    resolved = __reactor_resolve_path(path)
    ...
    __reactor_emit_status("read", "path" => resolved.to_s, "chars" => data.length, "preview" => __reactor_scrub(data[0, 500].to_s))
    data
  end
  ...
  def write(path, content)
    ...
    __reactor_emit_status("write", "path" => resolved.to_s, "chars" => content.to_s.length)
    resolved.to_s
  end
  ...
  def __reactor_apply_query(data, query)
  ...
  def output(*ids, format: "raw", query: nil, offset: nil, limit: nil)
    artifacts_dir = ENV["REACTOR_ARTIFACTS_DIR"]
    if artifacts_dir.nil? || artifacts_dir.empty?
      session_file = ENV["REACTOR_SESSION_FILE"]
      if session_file.nil? || session_file.empty?
        __reactor_emit_status("output", "error" => "No session file available")
        raise "No session - output artifacts unavailable"
      end
      artifacts_dir = session_file.sub(/\.[^.]*\z/, "")
    end
    unless File.directory?(artifacts_dir)
      __reactor_emit_status("output", "error" => "Artifacts directory not found", "path" => artifacts_dir)
    ...
    if query && (!offset.nil? || !limit.nil?)
      __reactor_emit_status("output", "error" => "query cannot be combined with offset/limit")
      ...
      if query
        json_value =
          begin
            ...
            __reactor_emit_status("output", "id" => output_id, "error" => "Not valid JSON: #{e.message}")
        ...
        if start_line > total_lines
          __reactor_emit_status("output", "id" => output_id, "error" => "Offset #{start_line} beyond end (#{total_lines} lines)")
    ...
    unless not_found.empty?
      ...
      __reactor_emit_status("output", "not_found" => not_found, "available_count" => available.length)
      raise msg
    end

    if ids.length == 1
      if format == "json"
        __reactor_emit_status("output", "id" => ids[0], "chars" => results[0]["char_count"])
        return results[0]
      end
      __reactor_emit_status("output", "id" => ids[0], "chars" => results[0]["content"].length)
      return results[0]["content"]
    end

    if format == "json"
      __reactor_emit_status("output", "count" => results.length, "total_chars" => results.sum { |r| r["char_count"] })
      return results
    end
    combined = results.map { |r| { "id" => r["id"], "content" => r["content"] } }
    __reactor_emit_status("output", "count" => combined.length, "total_chars" => combined.sum { |r| r["content"].length })
    combined
  end
  ...
  module OmpBridge
    ...
    def proxy_env
      base = ENV["REACTOR_TOOL_BRIDGE_URL"]
      token = ENV["REACTOR_TOOL_BRIDGE_TOKEN"]
      session = ENV["REACTOR_TOOL_BRIDGE_SESSION"]
    ...
    def call(name, args)
      ...
      payload = JSON.generate("session" => session, "run" => $__reactor_current_rid, "name" => name, "args" => args)
  ...
  def __reactor_pool_map(items)
    arr = items.to_a
    return [] if arr.empty?
    limit = __reactor_concurrency_limit
  ...
  def parallel(thunks)
    ...
    __reactor_pool_map(list) { |t| t.call }
  end
  ...
  def pipeline(items, *stages)
    ...
    stages.each do |stage|
      ...
      current = __reactor_pool_map(current) { |item| stage.call(item) }
    end
    current
  end
  ...
  def log(message)
    __reactor_emit_status("log", "message" => message.to_s)
    nil
  end

  def phase(title)
    $__reactor_current_phase = title.to_s
    __reactor_emit_status("phase", "title" => title.to_s)
    nil
  end
