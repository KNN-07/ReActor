...
const TRACE_IPC = $flag("REACTOR_RUBY_IPC_TRACE");
...
const RUNNER_CACHE_DIR = path.join(os.tmpdir(), "reactor-ruby-runner");
...
export async function checkRubyKernelAvailability(cwd: string, interpreter?: string): Promise<RubyKernelAvailability> {
	if (isBunTestRuntime() || $flag("REACTOR_RUBY_SKIP_CHECK")) {
		return { ok: true };
	}
...
export class RubyKernel extends BaseKernel<KernelExecuteOptions> {
	...
	static async start(options: KernelStartOptions): Promise<RubyKernel> {
		...
		// REACTOR_RUBY_SKIP_CHECK), where no candidate was probed.
...
function buildInitScript(cwd: string, env?: Record<string, string | undefined>): string {
	...
	const lines = [`__reactor_init_cwd = ${JSON.stringify(cwd)}`, "Dir.chdir(__reactor_init_cwd) rescue nil"];
	for (const key in envPayload) {
		lines.push(`ENV[${JSON.stringify(key)}] = ${JSON.stringify(envPayload[key])}`);
	}
	lines.push("$LOAD_PATH.delete(__reactor_init_cwd)", "$LOAD_PATH.unshift(__reactor_init_cwd)");
	return lines.join("\n");
}
