...
it("imports the JS process entry without loading dotenv before profile bootstrap", async () => {
	using tempDir = TempDir.createSync("@reactor-js-process-import-");
	await Bun.write(path.join(tempDir.path(), ".env"), "REACTOR_PROCESS_ENTRY_ENV_PROBE=loaded-too-early\n");
	const env = Object.fromEntries(
		Object.entries(process.env).filter((entry): entry is [string, string] => typeof entry[1] === "string"),
	);
	delete env.REACTOR_PROCESS_ENTRY_ENV_PROBE;
