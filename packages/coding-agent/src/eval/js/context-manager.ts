import { logger, postmortem, Snowflake, workerHostEntry } from "@reactor/utils";
import {
	createWorkerHandle,
 ...
 * `reactor --smoke-test` so binary / source / tarball installs all exercise it.
 */
...
function spawnBunWorker(): WorkerHandle {
	try {
		...
		const worker = hostEntry
			? new Worker(hostEntry, { type: "module", argv: ["__reactor_worker_js_eval"] })
