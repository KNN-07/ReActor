...
// `__reactor_import__` helper, which resolves the specifier against the active session cwd. A real
...
// worker-injected `__reactor_import__` global does not exist. The swap therefore guards on the
...
function buildReactorImportCall(sourceLiteral: string, optionsLiteral: string | undefined): string {
	...
	return optionsLiteral
		? `__reactor_import__(${sourceLiteral}, ${optionsLiteral})`
		: `__reactor_import__(${sourceLiteral})`;
}
...
function buildOptionsLiteral(node: BabelImportDeclaration): string | undefined {
	...
	// Native dynamic import takes options as `{ with: { ... } }`. `__reactor_import__` forwards the
...
export async function rewriteImports(code: string): Promise<string> {
	...
	// Top-level static `import` declarations become `await __reactor_import__(...)` calls.
	...
	// Dynamic `import(...)` expressions (anywhere) get their callee swapped for `__reactor_import__`
...
export async function rewriteDynamicImports(code: string, callee = "__reactor_import__"): Promise<string> {
	if (!code.includes("import")) return code;
	const ast = await parseProgram(code);
	if (!ast) return code;
...
async function returnFinalExpression(code: string): Promise<{ source: string; returned: boolean }> {
	...
	if (last?.type === "ExpressionStatement") {
		...
		return { source: `${prefix}__reactor_set_final_expr__((${trimmedStatement}));${suffix}`, returned: true };
	}
	if (last?.type === "ReturnStatement") {
		// Top-level `return value;` is otherwise swallowed: it forces the cell into an async IIFE
		// wrapper that discards the returned value. Rewrite into `__reactor_set_final_expr__((expr))`
		...
		return { source: `${prefix}__reactor_set_final_expr__((${expr}));${suffix}`, returned: true };
	}
