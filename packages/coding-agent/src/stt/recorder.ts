...
import { $which, logger, Snowflake } from "@reactor/utils";
...
$r = Mci "open new type waveaudio alias reactor_rec"
if ($r -ne 0) { exit 1 }

Mci "set reactor_rec channels 1 samplespersec 16000 bitspersample 16"

$r = Mci "record reactor_rec"
if ($r -ne 0) {
    Mci "close reactor_rec"
    exit 1
}
...
# Stop and save
Mci "stop reactor_rec"
$saveCmd = 'save reactor_rec "' + $outPath + '"'
$r = Mci $saveCmd
if ($r -ne 0) {
...
async function startPowerShellRecording(outputPath: string): Promise<RecordingHandle> {
	...
	const scriptPath = path.join(os.tmpdir(), `reactor-stt-record-${Snowflake.next()}.ps1`);
...
export async function startRecording(outputPath: string): Promise<RecordingHandle> {
	...
	if (recorders.length === 0) {
		throw new Error("No audio recorder available — run `reactor setup speech`");
	}
	...
	throw new Error(`No audio recorder could start — run \`reactor setup speech\`.\n${failures.join("\n")}`);
}
