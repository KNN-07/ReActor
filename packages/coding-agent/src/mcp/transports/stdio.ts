...
import { getProjectDir, readJsonl, Snowflake } from "@reactor/utils";
...
export interface StdioSpawnCommand {
	 ...
	 *
	 * Windows uses this only when the ReActor host has no console to share. When
	 ...
	 * wrapper chains must stay in the ReActor console session so nested console
...
export async function resolveStdioSpawnCommand(
	...
	// same console session. Only hide the child when ReActor itself has no console
