...
import type { OAuthCallbackFlowOptions } from "@reactor/ai/oauth/callback-server";
import { OAuthCallbackFlow } from "@reactor/ai/oauth/callback-server";
import type { OAuthController, OAuthCredentials } from "@reactor/ai/oauth/types";
import type { FetchImpl } from "@reactor/ai/types";
import { getActiveProfile } from "@reactor/utils/dirs";
import type { OAuthCredential } from "../session/auth-storage";

/** Credential-id prefix for ReActor-managed MCP OAuth credentials keyed by profile and server URL. */
...
/**
 * Deterministic credential id for an MCP server URL scoped to an ReActor profile.
 *
...
/** Whether a credential id was minted by ReActor's MCP OAuth flows (either era). */
 ...
 *
 * Plane is stricter for ReActor-synthesized fallback resources (e.g. using the
...
export class MCPOAuthFlow extends OAuthCallbackFlow {
	...
	async generateAuthUrl(state: string, redirectUri: string): Promise<{ url: string; instructions?: string }> {
		...
		if (existingResource) {
			...
			// provider-authored, not ReActor's server-URL fallback. Preserve same-host
