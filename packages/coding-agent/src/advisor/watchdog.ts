...
import { getAgentDir, isEnoent, logger, prompt } from "@reactor/utils";
 ...
 * `.reactor/<F>` for each given filename — and return the readable candidates with
...
export async function collectConfigCandidates(
	...
	// 1. User level: ~/.reactor/<F> (or active profile agent dir)
	if (resolvedAgentDir) {
	...
	// 2. Project levels (both standalone and native config .reactor/): walk up from cwd to repoRoot / home
	let current = cwd;
	while (true) {
		for (const filename of filenames) {
			candidates.add(path.resolve(current, ".reactor", filename));
	...
	for (const candidate of candidates) {
		try {
			...
			const ownerDir = baseName === ".reactor" ? path.dirname(parent) : parent;
			const ownerBaseName = ownerDir.split(path.sep).pop() ?? "";
			if (isUser || !ownerBaseName.startsWith(".") || baseName === ".reactor") {
...
/**
 * Discover and load WATCHDOG.md files walking up from cwd, project .reactor folder, and user agent dir.
