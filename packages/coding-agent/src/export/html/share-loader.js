    (function() {
      ...
      // ============================================================
      //
      // Served by the reactor relay at /s/<id>; the AES-256-GCM key rides in the
      // URL fragment and never leaves the browser. Resolves the session JSON
      // and hands it to template.js via `window.__REACTOR_SESSION_DATA__`:
      ...
      async function fetchGistBlob(id) {
        ...
        var file = files.find(function(f) { return /\.reactorshare\.txt$/.test(f.filename); }) || files[0];
      ...
      async function load() {
        ...
        if (data && data.header && data.header.title) {
          document.title = data.header.title + ' — reactor session';
        }
        return data;
      }
      ...
      window.__REACTOR_SESSION_DATA__ = pending;
    })();
