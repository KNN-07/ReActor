/**
 * ReActor extension package roots.
 *
...
import { getAgentDir, isEnoent, logger, tryParseJson } from "@reactor/utils";
...
function scopeDirs(ctx: LoadContext): ScopeDirs {
	return {
		project: path.join(ctx.cwd, ".reactor"),
		user: getAgentDir(),
	};
}
 ...
 *    `reactor install <pkg>` / `reactor plugin install` / `reactor plugin link`). Marketplace
 ...
 * "Claude Code Marketplace" and "ReActor Extension Packages".
 */
