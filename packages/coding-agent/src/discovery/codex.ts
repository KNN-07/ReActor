...
import { logger, parseFrontmatter } from "@reactor/utils";
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	...
	// ReActor hooks must be named `pre-<tool>.<ts|js>` or `post-<tool>.<ts|js>`.
	// Files without that prefix are not ReActor hooks (e.g. the standalone Codex
