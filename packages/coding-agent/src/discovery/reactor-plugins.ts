/**
 * ReActor extension-package sub-discovery provider.
 *
 ...
 * `rules/`, `prompts/`, and `.mcp.json` — are picked up by reactor's standard
 * discovery surfaces. The native `reactor` provider in `builtin.ts` only walks
 * `.reactor/` and `~/.reactor/agent/`, so without this provider those sub-trees are
 * silently ignored.
 *
 * Provider priority is set below the native `reactor` provider (100) so an
 * extension package never shadows the user's own `.reactor/` configuration on
 * dedup.
 *
 * @see ./reactor-extension-roots.ts
 * @see ../../docs/extension-loading.md
 */
import * as path from "node:path";
import { logger, parseFrontmatter, tryParseJson } from "@reactor/utils";
...
const DISPLAY_NAME = "ReActor Extension Packages";
const DESCRIPTION =
...
async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
	...
	for (let i = 0; i < tasks.length; i++) {
		...
		if (!parsed) {
			warnings.push(`[reactor-plugins] Invalid JSON in ${mcpPath}`);
			logger.warn(`[reactor-plugins] Invalid JSON in ${mcpPath}`);
			continue;
		}
		...
		for (const [serverName, serverCfg] of Object.entries(servers)) {
			...
			if (typeof cfg.command !== "string" && typeof cfg.url !== "string") {
				warnings.push(
					`[reactor-plugins] Skipping MCP server "${serverName}" in ${mcpPath}: missing command or url`,
				);
				continue;
			}
