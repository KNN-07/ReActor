/**
 * Builtin Provider (.reactor)
 *
 * Primary provider for ReActor native configs. Supports all capabilities.
 */
import * as path from "node:path";
import { getAgentDir, logger, parseFrontmatter, tryParseJson } from "@reactor/utils";
...
const DESCRIPTION = "Native ReActor configuration from ~/.reactor and .reactor/";
const PRIORITY = 100;
...
async function getConfigDirs(ctx: LoadContext): Promise<Array<{ dir: string; level: "user" | "project" }>> {
	...
	// profile's agent dir (~/.reactor/profiles/<name>/agent), like sessions and MCP.
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	// Walk up from cwd finding .reactor/skills/ in ancestors (closest first)
	...
	// User-level scan from ~/.reactor/agent/skills/
...
registerProvider<Skill>(skillCapability.id, {
	...
	description: "Auto-generated managed skills from ~/.reactor/agent/managed-skills",
...
async function loadRules(ctx: LoadContext): Promise<LoadResult<Rule>> {
	...
	// https://reactor.norman.id.vn/docs/context-files as the file that gets "re-injected near
	// the current turn so they keep hold across long conversations".
	// User scope:    ~/.reactor/agent/RULES.md
	// Project scope: nearest .reactor/RULES.md walking up from cwd to repoRoot
...
registerProvider<ContextFile>(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load AGENTS.md from .reactor/ directories",
	priority: PRIORITY,
