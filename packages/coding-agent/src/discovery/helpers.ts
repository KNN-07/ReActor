...
import { FileType, glob } from "@reactor/natives";
import {
	CONFIG_DIR_NAME,
	getAgentDir,
	getConfigDirName,
...
async function readExtensionModuleManifest(
	...
	const pkg = tryParseJson<{ reactor?: ExtensionModuleManifest; pi?: ExtensionModuleManifest }>(content);
	const manifest = pkg?.reactor ?? pkg?.pi;
 ...
 * 3. Subdirectory with package.json: `extensions/<ext>/package.json` with "reactor"/"pi" field → load declared paths
 *
 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.reactor/`.
 *    The first match returns `<dir>/.reactor/plugins/installed_plugins.json`.
 * 2. If no `.reactor/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.reactor/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .reactor/ directory (nearest wins).
	// Stop before os.homedir() — ~/.reactor/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.reactor/plugins/installed_plugins.json`
 * when no project anchor (.reactor/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .reactor/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
 ...
 * Reads ~/.claude/plugins/installed_plugins.json and ~/.reactor/plugins/installed_plugins.json,
...
export async function listClaudePluginRoots(
	...
	// ── ReActor installed plugins registry ───────────────────────────────────────
	// ReActor registry is authoritative: its entries replace Claude's entries for the same plugin ID.
	...
	const reactorRegistryPath = path.join(getPluginsDir(home), "installed_plugins.json");
	const reactorContent = await readFile(reactorRegistryPath);
	if (reactorContent) {
		const reactorRegistry = parseClaudePluginsRegistry(reactorContent);
		if (reactorRegistry) {
			for (const [pluginId, entries] of Object.entries(reactorRegistry.plugins)) {
				if (!Array.isArray(entries) || entries.length === 0) continue;
				...
				// ReActor is authoritative: drop all Claude-sourced entries for this plugin ID
			...
			warnings.push(`Failed to parse ReActor plugin registry: ${reactorRegistryPath}`);
		}
	}

	// ── Project-scoped ReActor registry ────────────────────────────────────────
	// Loaded from the nearest .reactor/plugins/installed_plugins.json relative to cwd.
 ...
 * These are prepended to the cache with highest precedence (before ReActor/Claude entries).
