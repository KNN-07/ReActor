import type { Model, ProviderResponseMetadata, RawSseEvent } from "@reactor/ai";
...
function trimRawLines(raw: string[]): TrimResult {
	...
	const tail = `: reactor-debug-truncated originalChars=${originalChars}`;
	trimmed.push(tail);
	chars += tail.length + 1;
...
export class RawSseDebugBuffer {
	...
	toRawText(): string {
		...
		const dropped = `: reactor-debug-dropped records=${this.#droppedRecords} chars=${this.#droppedChars}\n\n`;
