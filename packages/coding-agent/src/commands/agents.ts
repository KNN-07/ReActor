...
export default class Agents extends Command {
	...
	static flags = {
		...
		user: Flags.boolean({ description: "Write to ~/.reactor/agent/agents (default)" }),
		project: Flags.boolean({ description: "Write to ./.reactor/agents" }),
	};

	static examples = [
		"# Export bundled agents into user config (default)\n  reactor agents unpack",
		"# Export bundled agents into project config\n  reactor agents unpack --project",
		"# Overwrite existing local agent files\n  reactor agents unpack --project --force",
		"# Export into a custom directory\n  reactor agents unpack --dir ./tmp/agents --json",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Agents);
		if (!args.action) {
			renderCommandHelp("reactor", "agents", Agents);
			return;
		}
