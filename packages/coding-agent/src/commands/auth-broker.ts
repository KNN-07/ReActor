/**
 * `reactor auth-broker` — manage the reactor credential vault.
 */
...
export default class AuthBroker extends Command {
	static description = "Manage the reactor auth-broker (credential vault)";
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  reactor auth-broker serve",
		"# Boot on a non-default port\n  reactor auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  reactor auth-broker token",
		"# Rotate the bearer token\n  reactor auth-broker token --regenerate",
		"# List supported OAuth providers\n  reactor auth-broker list",
		"# Local login (run on the broker host)\n  reactor auth-broker login anthropic",
		"# Interactive provider selection\n  reactor auth-broker login",
		"# Remote login over SSH tunnel\n  reactor auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  reactor auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  reactor auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  reactor auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  reactor auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  reactor auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  reactor auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("reactor", "auth-broker", AuthBroker);
			return;
		}
