...
#
# `__reactor_emit_referenced_exports` reads captured function bodies from stdin,
...
__reactor_sq_quote() {
	...
	__reactor_qout=
	__reactor_sq=\'
	while case "$__reactor_qbuf" in *$__reactor_sq*) true ;; *) false ;; esac; do
		__reactor_qout=$__reactor_qout${__reactor_qbuf%%$__reactor_sq*}"'\\''"
		__reactor_qbuf=${__reactor_qbuf#*$__reactor_sq}
	done
	__reactor_qout=$__reactor_qout$__reactor_qbuf
	printf "'%s'" "$__reactor_qout"
}
...
__reactor_emit_export_for() {
	...
	eval "__reactor_xv=\"\${$1}\"" 2>/dev/null || return
	printf 'export %s=%s\n' "$1" "$(__reactor_sq_quote "$__reactor_xv")"
}

# Read function bodies on stdin, emit dedup'd export lines on stdout.
__reactor_emit_referenced_exports() {
	grep -oE '\$\{?[A-Za-z_][A-Za-z0-9_]*' \
		| sed -E 's/^\$\{?//' \
		| sort -u \
		| while IFS= read -r __reactor_name; do
			__reactor_emit_export_for "$__reactor_name"
		done
}
