...
echo "# Shell snapshot - generated by reactor agent" >| "$SNAPSHOT_FILE"
...
${fnEnvHelper}
printf '%s\\n' "$__reactor_funcs" | __reactor_emit_referenced_exports >> "$SNAPSHOT_FILE"
unset -f __reactor_sq_quote __reactor_emit_export_for __reactor_emit_referenced_exports 2>/dev/null
unset __reactor_funcs __reactor_qbuf __reactor_qout __reactor_sq __reactor_xv __reactor_name 2>/dev/null
...
export async function getOrCreateSnapshot(
	...
	const snapshotDir = path.join(os.tmpdir(), "reactor-shell-snapshots");
