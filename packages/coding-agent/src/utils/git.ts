...
import { $which, hasFsCode, isEisdir, isEnoent, isEnotdir, Snowflake } from "@reactor/utils";
...
async function writeTempPatch(content: string): Promise<string> {
	const tempPath = path.join(os.tmpdir(), `reactor-git-patch-${Snowflake.next()}.patch`);
