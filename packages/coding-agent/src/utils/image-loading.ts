...
import type { ImageContent, Model } from "@reactor/ai";
import { formatBytes, readImageMetadata, SUPPORTED_IMAGE_MIME_TYPES } from "@reactor/utils";
 ...
 * to PNG/JPEG instead — the automatic equivalent of `REACTOR_NO_WEBP=1`.
 */
 ...
 * `REACTOR_NO_WEBP` env fallback in {@link resizeImage} still applies. Feed straight
...
export interface LoadImageInputOptions {
	...
	/** Force non-WebP output (e.g. for Ollama). Leave unset to honor `REACTOR_NO_WEBP`. */
	excludeWebP?: boolean;
}
