...
import { type GrepMatch, GrepOutputMode, type GrepResult, grep } from "@reactor/natives";
...
/** Mirrors `MAX_FILE_BYTES` in `crates/reactor-natives/src/grep.rs`. Native grep
...
async function resolveArchiveSearchPaths(
	...
	for (let idx = 0; idx < pathSpecs.length; idx++) {
		...
		if (!tempDir) {
			tempDir = await mkdtemp(path.join(tmpdir(), "reactor-search-archive-"));
		}
...
const REACTOR_ROOT_URL_RE = /^reactor:\/\/(?:\/?|docs\/?)$/i;
...
async function searchVirtualResources(
	...
	const dir = await mkdtemp(path.join(tmpdir(), "reactor-search-virtual-"));
	try {
...
async function expandVirtualInternalResource(
	...
	if (REACTOR_ROOT_URL_RE.test(rawPath)) {
		const completions = await internalRouter.complete("reactor", "");
		if (completions && completions.length > 0) {
			...
			for (const completion of completions) {
				...
				const docUrl = `reactor://${completion.value}`;
