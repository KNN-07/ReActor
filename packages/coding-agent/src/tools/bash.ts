...
import { ImageProtocol, TERMINAL } from "@reactor/tui";
import { getProjectDir, isEnoent, logger, prompt } from "@reactor/utils";
 ...
 * ACP deployment shape of an editor spawning reactor as a co-hosted subprocess.
 */
...
export class BashTool implements AgentTool<typeof bashSchemaBase | typeof bashSchemaWithAsync, BashToolDetails> {
	...
	async execute(
		...
		// must still cancel the call or job, but ReActor does not impose a deadline.
