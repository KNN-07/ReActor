...
import type { FetchImpl, ImageContent, TextContent } from "@reactor/ai";
import { htmlToMarkdown } from "@reactor/natives";
import { type Component, Text } from "@reactor/tui";
import { $which, ptree, truncate } from "@reactor/utils";
...
async function renderNotebookPayload(bytes: Uint8Array, displayUrl: string): Promise<string> {
	return withTempBinaryFile("reactor-url-notebook-", ".ipynb", bytes, tempPath =>
		readEditableNotebookText(tempPath, displayUrl),
	);
}

async function renderSqlitePayload(bytes: Uint8Array): Promise<string> {
	return withTempBinaryFile("reactor-url-sqlite-", ".sqlite", bytes, async tempPath => {
