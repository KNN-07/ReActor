...
import { Process, type PtyRunResult, PtySession } from "@reactor/natives";
import { isEexist, isEnoent, logger, postmortem, procmgr, sanitizeText } from "@reactor/utils";
...
export async function startDaemonBrokerFromEnvironment(): Promise<void> {
	...
	if (!lease) return;
	process.title = "reactor daemon broker";
