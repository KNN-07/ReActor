...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@reactor/utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.reactor/, .pi/, .claude/)
	...
	// User config directories (~/.reactor/agent/, ~/.pi/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .reactor/lsp.*, .pi/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.reactor/agent/lsp.*, ~/.pi/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
