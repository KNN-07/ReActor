...
import { $env, prompt, Snowflake } from "@reactor/utils";
...
import * as taskExecutor from "./executor";
import * as isolationRunner from "./isolation-runner";
import { type IsolationContext, makeIsolationCommitMessage } from "./isolation-runner";
...
function assertDepthAndSpawnAllowed(request: StructuredSubagentRequest, agentName: string): void {
	...
	const blockedAgent = request.blockedAgent ?? $env.REACTOR_BLOCKED_AGENT;
...
export async function resolveEffectiveSubagentPolicy(
	...
	const discovery = await taskDiscovery.discoverAgents(request.session.cwd);
	const agent = taskDiscovery.getAgent(discovery.agents, agentName);
	if (!agent) {
...
async function leaseArtifacts(
	...
	const artifactsDir = path.join(
		os.tmpdir(),
		`${invocationKind === "eval" ? "reactor-eval-agent" : "reactor-task"}-${Snowflake.next()}`,
	);
	await fs.mkdir(artifactsDir, { recursive: true });
...
export async function runStructuredSubagent(request: StructuredSubagentRequest): Promise<StructuredSubagentResult> {
	...
	try {
		...
		if (policy.isIsolated) {
			try {
				isolationContext = await isolationRunner.prepareIsolationContext(request.session.cwd);
			} catch (error) {
				const message = error instanceof Error ? error.message : String(error);
				throw new StructuredSubagentError(
					"isolation",
		...
		const result = !isolationContext
			? await taskExecutor.runSubprocess(baseOptions)
			: await isolationRunner.runIsolatedSubprocess({
					baseOptions,
		...
		if (
			...
			const outcome = await isolationRunner.mergeIsolatedChanges({
				result,
			...
			if (outcome.changesApplied !== false) {
				const nestedPatchSummary = await isolationRunner.applyEligibleNestedPatches({
					result,
					repoRoot: isolationContext.repoRoot,
					mergeMode: policy.mergeMode,
					changesApplied: outcome.changesApplied,
