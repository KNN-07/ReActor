...
import { getAgentDir, getProjectDir, isEnoent } from "@reactor/utils";
...
export async function installPlugin(packageName: string): Promise<InstalledPlugin> {
	...
	if (!(await pkgJson.exists())) {
		await pkgJson.write(JSON.stringify({ name: "reactor-plugins", private: true, dependencies: {} }, null, 2));
	}
	...
	return {
		...
		manifest: pkg.reactor || pkg.pi || { version: pkg.version },
