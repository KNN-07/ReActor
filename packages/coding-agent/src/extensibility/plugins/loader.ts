...
import { getPluginsDir, getPluginsLockfile, isEnoent } from "@reactor/utils";
 ...
 *
 * `home` controls which `<plugins>/reactor-plugins.lock.json` is read — pass it
...
/**
 * Load project-local plugin overrides (checks .reactor and .pi directories).
 */
 ...
 * `<root>/package.json#dependencies`, and `<root>/reactor-plugins.lock.json#plugins`.
...
async function collectPluginsAtRoot(
	...
	const lockPath = path.join(root, "reactor-plugins.lock.json");
	...
	for (const name of names) {
		...
		let pluginPkg: { version: string; reactor?: PluginManifest; pi?: PluginManifest };
		try {
		...
		const manifest: PluginManifest | undefined = pluginPkg.reactor || pluginPkg.pi;
		if (!manifest) {
			// Not an reactor plugin, skip
			continue;
		}
 ...
 * (`getPluginsDir(home)`) and, when a project anchor (`.reactor/` or `.git/`)
 * exists at or above `cwd`, the project root
 * (`<projectAnchor>/.reactor/plugins`). Each root contributes the union of its
 * `package.json#dependencies` and `reactor-plugins.lock.json#plugins`. Project
...
interface DeclaredManifestEntries {
	/** True when the directory's package.json declares a non-empty `reactor`/`pi` `extensions` array. */
	declared: boolean;
...
/**
 * Read the extension entries declared by `dir`'s own package.json `reactor`/`pi`
...
function readDeclaredManifestEntries(dir: string): DeclaredManifestEntries {
	...
	let pkg: { reactor?: { extensions?: unknown }; pi?: { extensions?: unknown } };
	try {
		pkg = JSON.parse(raw) as { reactor?: { extensions?: unknown }; pi?: { extensions?: unknown } };
	} catch {
		return { declared: false, files: [] };
	}
	const declared = (pkg.reactor ?? pkg.pi)?.extensions;
 ...
 *   1. the directory's own package.json `reactor`/`pi` `extensions` entries —
 ...
 *     {@link resolveDirectoryEntries} — its own package.json `reactor`/`pi`
 ...
 *     and ReActor's configured-directory (`-e`) extension loader
 *   - otherwise (tools/hooks/commands) only a direct index.{ts,js,mjs,cjs}.
 *     The sub-extension scan and the `reactor`/`pi` `extensions` manifest are
