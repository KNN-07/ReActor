 ...
 *
 * Dependencies (the arktype validation and coding-agent) are injected via the
 * CustomCommandAPI to avoid import resolution issues with custom commands loaded from user directories.
 */
import * as fs from "node:fs";
import * as path from "node:path";
import { getAgentDir, getProjectDir, isEnoent, logger } from "@reactor/utils";
...
/**
 * Load bundled commands (shipped with coding-agent).
 */
