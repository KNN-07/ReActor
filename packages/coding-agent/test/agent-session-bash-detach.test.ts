 ...
 * verified at the unit level by `reactor-natives::shell::tests::child_session_action`
 * (truth-table) and `embedded_external_command_runs_in_its_own_session` (real
 * brush spawn). This test pulls the fix end-to-end through the ReActor coding
 * agent stack:
 *
 *   AgentSession.prompt
 *     → Agent.prompt (real)
 ...
 *         → BashTool.execute
 *           → executeBash
 *             → reactor-natives `Shell.run` (real native binding)
 ...
 * SIGTTIN/SIGTTOU the ReActor host process.
 */
...
import { Agent, type AgentMessage, type AgentTool } from "@reactor/agent-core";
import { createMockModel, type MockResponse } from "@reactor/ai/providers/mock";
...
import { BashTool, type ToolSession } from "@reactor/coding-agent/tools";
