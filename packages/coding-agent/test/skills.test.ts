...
import { type Skill as CapabilitySkill, skillCapability } from "@reactor/coding-agent/capability/skill";
import { getCapability } from "@reactor/coding-agent/discovery";
import {
	loadSkills,
	loadSkillsFromDir,
	parseSkillInvocation,
	type Skill,
} from "@reactor/coding-agent/extensibility/skills";
...
describe("skills", () => {
	...
	describe("loadSkills with options", () => {
		...
		// canonical ReActor-native `~/.agent[s]/skills` (the `agents` provider).
		...
		// MUST NOT consider the ReActor-native `enableAgentsUser`/`...Project`
		...
		it("should skip skills disabled via frontmatter", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-disabled-skill-"));
...
`,
		...
		it("should hide skills with disable-model-invocation frontmatter (Agent Skills spec)", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-dmi-skill-"));
