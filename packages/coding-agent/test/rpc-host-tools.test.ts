...
import type { AgentEvent } from "@reactor/agent-core";
import { defineRpcClientTool, RpcClient } from "@reactor/coding-agent/modes";
import { RpcHostToolBridge } from "@reactor/coding-agent/modes/rpc/host-tools";
import type {
	RpcHostToolCallRequest,
	RpcHostToolCancelRequest,
	RpcHostToolUpdate,
} from "@reactor/coding-agent/modes/rpc/rpc-types";
...
describe("RpcClient custom tools", () => {
	it("registers host custom tools and serves tool calls over the RPC transport", async () => {
		const scriptPath = path.join(os.tmpdir(), `reactor-rpc-host-tools-${Date.now()}.js`);
