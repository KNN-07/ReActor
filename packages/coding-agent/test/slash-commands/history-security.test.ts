...
import { shouldSkipHistory } from "@reactor/coding-agent/modes/controllers/input-controller";
...
describe("shouldSkipHistory — security filter for slash command history", () => {
	...
	it("skips /join with a link argument (carries 32-byte room key and write token)", () => {
		expect(shouldSkipHistory("/join reactor://share/abc123def456...")).toBe(true);
		expect(shouldSkipHistory("/join reactor:abc123def456...")).toBe(true);
	});
