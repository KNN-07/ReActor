...
import { BUILTIN_SLASH_COMMANDS } from "@reactor/coding-agent/slash-commands/builtin-registry";
...
describe("/move directory completion", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-move-completion-"));
	...
	it("completes parent directory paths", async () => {
		...
		const siblingName = `reactor-move-sibling-${path.basename(tempDir)}`;
