/**
 * ACP stdout-hygiene smoke: launching `reactor acp` must not leak any banner,
 ...
 * unwind, but force-kill quickly if it hasn't reaped — `reactor acp` blocks on
...
describe("ACP stdout hygiene", () => {
	it("emits a JSON-RPC initialize response as the first bytes on stdout", async () => {
		const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "reactor-acp-stdout-"));
		...
		// run (~12s cold vs ~0.4s warm). XDG_* and REACTOR_CODING_AGENT_DIR
