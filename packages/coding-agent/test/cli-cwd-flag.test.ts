...
import { applyStartupCwd } from "@reactor/coding-agent/cli/startup-cwd";
import { getProjectDir, normalizePathForComparison, setProjectDir } from "@reactor/utils";
...
describe("parseArgs — --cwd flag", () => {
	...
	it("applies --cwd before session lookup callers read the project directory", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-cwd-launch-"));
		const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-cwd-target-"));
	...
	it("normalizes a relative --cwd target to the resolved absolute path", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-cwd-rel-"));
