...
import { type EditMode, type EditModeSessionLike, resolveEditMode } from "@reactor/coding-agent/utils/edit-mode";

const originalEditVariant = Bun.env.REACTOR_EDIT_VARIANT;
const originalStrictEditMode = Bun.env.REACTOR_STRICT_EDIT_MODE;
...
function restoreEnv(): void {
	...
	if (originalStrictEditMode === undefined) {
		delete Bun.env.REACTOR_STRICT_EDIT_MODE;
	} else {
		Bun.env.REACTOR_STRICT_EDIT_MODE = originalStrictEditMode;
	}
}
...
describe("resolveEditMode", () => {
	...
	test("keeps REACTOR_EDIT_VARIANT ahead of the Kimi fallback", () => {
		Bun.env.REACTOR_EDIT_VARIANT = "hashline";
	...
	test("keeps strict edit mode ahead of the Kimi fallback", () => {
		delete Bun.env.REACTOR_EDIT_VARIANT;
		Bun.env.REACTOR_STRICT_EDIT_MODE = "1";
