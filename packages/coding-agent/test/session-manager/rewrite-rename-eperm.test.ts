...
import { recoverOrphanedBackups } from "@reactor/coding-agent/session/session-listing";
import { SessionManager } from "@reactor/coding-agent/session/session-manager";
import { FileSessionStorage, MemorySessionStorage } from "@reactor/coding-agent/session/session-storage";
...
describe("SessionManager rewrite EPERM replacement fallback", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "reactor-eperm-"));
	});
...
describe("FileSessionStorage.writeTextAtomic commitGuard cleanup", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "reactor-guard-cleanup-"));
	});
