...
import { listSessions } from "@reactor/coding-agent/session/session-listing";
...
describe("large session memory guards", () => {
	...
	it("streams large session files and keeps only the latest compaction summary", async () => {
		const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "reactor-large-session-"));
	...
	it("only elides loaded compactions on the active branch", async () => {
		const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "reactor-branch-load-"));
		tempDirs.push(tempDir);
