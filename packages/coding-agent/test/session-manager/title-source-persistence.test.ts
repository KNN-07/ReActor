...
import { FileSessionStorage, type WriteTextAtomicOptions } from "@reactor/coding-agent/session/session-storage";
import type { SessionTitleUpdate } from "@reactor/coding-agent/session/session-title-slot";
...
describe("session title source persistence", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-title-source-"));
