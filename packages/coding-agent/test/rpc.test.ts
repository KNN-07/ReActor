...
import type { AgentEvent, AgentMessage } from "@reactor/agent-core";
import { type AssistantMessage, Effort, type TextContent } from "@reactor/ai";
import {
...
import type { BashExecutionMessage } from "@reactor/coding-agent/session/messages";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("RPC mode", () => {
	...
	beforeEach(() => {
		sessionDir = path.join(os.tmpdir(), `reactor-rpc-test-${Snowflake.next()}`);
		client = new RpcClient({
			cliPath: path.join(import.meta.dir, "..", "dist", "cli.js"),
			cwd: path.join(import.meta.dir, ".."),
			env: { REACTOR_CODING_AGENT_DIR: sessionDir },
			provider: "anthropic",
