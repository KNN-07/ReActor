...
describe("fetchMarketplace", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-fetcher-test-"));
	});
	...
	it("loads catalog from .reactor-plugin/marketplace.json when present", async () => {
		const root = path.join(tmpDir, "reactor-only");
		fs.mkdirSync(path.join(root, ".reactor-plugin"), { recursive: true });
		const catalog = {
			name: "reactor-only-marketplace",
			owner: { name: "Test" },
			plugins: [{ name: "reactor-plugin", source: "./plugins/reactor-plugin", description: "x" }],
		};
		fs.writeFileSync(path.join(root, ".reactor-plugin", "marketplace.json"), JSON.stringify(catalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("reactor-only-marketplace");
		expect(result.catalog.plugins[0].name).toBe("reactor-plugin");
	});

	it("prefers .reactor-plugin/marketplace.json over .claude-plugin/marketplace.json when both exist", async () => {
		const root = path.join(tmpDir, "both-catalogs");
		fs.mkdirSync(path.join(root, ".reactor-plugin"), { recursive: true });
		fs.mkdirSync(path.join(root, ".claude-plugin"), { recursive: true });
		const reactorCatalog = {
			name: "from-reactor-plugin",
			owner: { name: "Test" },
		...
		fs.writeFileSync(path.join(root, ".reactor-plugin", "marketplace.json"), JSON.stringify(reactorCatalog));
		fs.writeFileSync(path.join(root, ".claude-plugin", "marketplace.json"), JSON.stringify(claudeCatalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("from-reactor-plugin");
	});

	it("falls back to .claude-plugin/marketplace.json when .reactor-plugin is absent", async () => {
	...
	it("error message names both candidate paths when neither exists", async () => {
		...
		await expect(fetchMarketplace(empty, tmpDir)).rejects.toThrow(
			/\.reactor-plugin[\\/]marketplace\.json.*\.claude-plugin[\\/]marketplace\.json/,
		);
	});
