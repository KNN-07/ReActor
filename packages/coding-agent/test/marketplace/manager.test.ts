...
function buildMinimalFixture(): string {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-mgr-fixture-"));
...
function createTestContext(): TestContext {
	const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-mgr-test-"));
...
function mockPluginManagerPaths(root: string) {
	return [
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(root, "reactor-plugins.lock.json")),
...
describe("MarketplaceManager", () => {
	...
	it("installPlugin → plugin in cache + in registry", async () => {
		...
		const runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "reactor-plugins.lock.json")).json();
	...
	it("installPlugin exposes marketplace package to the runtime loader", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-mgr-home-"));
		try {
			const pluginsDir = path.join(tmpHome, ".reactor", "plugins");
			const manager = new MarketplaceManager({
				marketplacesRegistryPath: path.join(tmpHome, ".reactor", "marketplaces.json"),
	...
	it("installPlugin keeps marketplace packages out of ReActor extension roots", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-mgr-home-"));
		try {
			const pluginsDir = path.join(tmpHome, ".reactor", "plugins");
			const manager = new MarketplaceManager({
	...
	it("installPlugin with scope:project exposes the marketplace package to the runtime loader", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-mgr-home-"));
		const projectAnchor = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-mgr-project-"));
		try {
			const userPluginsDir = path.join(tmpHome, ".reactor", "plugins");
			const projectPluginsDir = path.join(projectAnchor, ".reactor", "plugins");
			...
			const projectLock = await Bun.file(path.join(projectPluginsDir, "reactor-plugins.lock.json")).json();
			...
			expect(fs.existsSync(path.join(userPluginsDir, "reactor-plugins.lock.json"))).toBe(false);
		} finally {
			fs.rmSync(tmpHome, { recursive: true, force: true });
	...
	it("setPluginEnabled → persisted in registry", async () => {
		...
		let runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "reactor-plugins.lock.json")).json();
		expect(runtimeConfig.plugins["hello-plugin"].enabled).toBe(false);
		...
		runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "reactor-plugins.lock.json")).json();
	...
	it("installPlugin scope:project when no projectInstalledRegistryPath → throws", async () => {
		const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-mgr-noproj-"));
		try {
