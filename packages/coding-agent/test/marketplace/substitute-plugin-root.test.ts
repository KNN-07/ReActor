...
import { substitutePluginRoot } from "@reactor/coding-agent/discovery/substitute-plugin-root";
...
describe("substitutePluginRoot", () => {
	...
	it("replaces REACTOR_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${REACTOR_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${REACTOR_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
