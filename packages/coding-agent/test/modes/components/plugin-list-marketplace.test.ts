...
import { PluginManager } from "@reactor/coding-agent/extensibility/plugins";
import {
...
} from "@reactor/coding-agent/modes/components/plugin-settings";
...
describe("PluginListComponent", () => {
	...
	it("empty-state mentions both npm and marketplace install commands", () => {
		...
		expect(text).toContain("reactor plugin install <package>");
		expect(text).toContain("reactor plugin install <name>@<marketplace>");
	});
...
describe("MarketplacePluginDetailComponent", () => {
	...
	it("shortens home-relative install paths to ~ before rendering", () => {
		...
		const installPath = `${home}/.reactor/cache/plugins/sample@mkt`;
		...
		expect(text).toContain("~/.reactor/cache/plugins/sample@mkt");
