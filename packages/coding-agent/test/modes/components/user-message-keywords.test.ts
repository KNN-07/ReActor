...
import { CustomEditor } from "@reactor/coding-agent/modes/components/custom-editor";
import { UserMessageComponent } from "@reactor/coding-agent/modes/components/user-message";
import { getEditorTheme, initTheme } from "@reactor/coding-agent/modes/theme/theme";
...
describe("UserMessageComponent magic-keyword highlighting", () => {
	...
	it("wraps image references in file hyperlinks when a blob path is available", () => {
		const imagePath = path.resolve("/tmp/reactor-image.png");
