...
import type { AssistantMessage, Usage } from "@reactor/ai";
import type { Rule } from "@reactor/coding-agent/capability/rule";
import { OmfgController } from "@reactor/coding-agent/modes/controllers/omfg-controller";
...
import { Container, type TUI } from "@reactor/tui";
import { removeWithRetries } from "@reactor/utils";

const PROJECT_OPTION = "This project (.reactor/rules)";
const GLOBAL_OPTION = "Global — all projects (~/.reactor/agent/rules)";
...
describe("OmfgController", () => {
	it("saves a matching generated rule under project rules and registers it live", async () => {
		...
		const savedPath = path.join(harness.projectDir, ".reactor", "rules", "ts-no-any.md");
		...
		expect(rendered).toContain(path.join(".reactor", "rules", "ts-no-any.md"));
	...
	it("reiterates when the first valid rule does not match history", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".reactor", "rules", "ts-no-any.md")).exists()).toBe(true);
	});
	...
	it("asks before saving when validation never confirms a match", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".reactor", "rules", "no-match.md")).exists()).toBe(false);
	});
	...
	it("lets the user amend from the save selector before writing the rule", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".reactor", "rules", "ts-any-broad.md")).exists()).toBe(
			false,
		);
		expect(await Bun.file(path.join(harness.projectDir, ".reactor", "rules", "ts-no-explicit-any.md")).exists()).toBe(
			true,
		);
	});
	...
	it("clears the panel and aborts the inner request on Escape", async () => {
		...
		expect(controller.hasActiveRequest()).toBe(false);
		expect(await Bun.file(path.join(harness.projectDir, ".reactor", "rules", "ts-no-any.md")).exists()).toBe(false);
	});
});
