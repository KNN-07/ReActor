...
import { rewriteImports, wrapCode } from "@reactor/coding-agent/eval/js/context-manager";
import { indirectEval } from "@reactor/coding-agent/eval/js/shared/indirect-eval";
...
describe("rewriteImports", () => {
	it("rewrites a top-level default import", async () => {
		...
		expect(out).toContain('await __reactor_import__("bar")');
	...
	it("rewrites destructured named imports with renames", async () => {
		...
		expect(out).toContain('await __reactor_import__("pkg")');
	...
	it("rewrites namespace imports", async () => {
		...
		expect(out).toContain('const ns = await __reactor_import__("pkg")');
	});
	...
	it("rewrites side-effect-only imports", async () => {
		...
		expect(out).toContain('await __reactor_import__("polyfill")');
	});
	...
	it("preserves import attributes via the dynamic import options bag", async () => {
		...
		expect(out).toContain('await __reactor_import__("./d.json", { with: { type: "json" } })');
		expect(out).toContain("const data =");
	});

	// Dynamic `import(...)` callees are swapped for a shim that prefers the worker-injected
	// `__reactor_import__` helper but falls back to native dynamic import. The fallback matters:
	...
	const SHIM = '(typeof __reactor_import__ === "function" ? __reactor_import__ : (s, o) => import(s, o))';
	...
	it("routes dynamic import through the helper when present and native import when serialized into a foreign realm", async () => {
		...
		expect("__reactor_import__" in globals).toBe(false);

		// Worker realm: helper global exists, call must route through it.
		const seen: string[] = [];
		globals.__reactor_import__ = async (source: string) => {
			seen.push(source);
			return { stubbed: true };
		};
		try {
	...
	it("does not rewrite import statements embedded in template literals (the bug)", async () => {
		...
		expect(out).not.toContain("await __reactor_import__(");
	});
	...
	it("does not rewrite import statements inside block comments", async () => {
		...
		expect(out).not.toContain('await __reactor_import__("bar")');
	});
	...
	it("rewrites real top-level imports while leaving template-embedded look-alikes alone", async () => {
		...
		const out = await rewriteImports(code);
		expect(out).toContain('await __reactor_import__("alpha")');
		expect(out).toContain('await __reactor_import__("gamma")');
		expect(out).not.toContain('await __reactor_import__("beta")');
	...
	it("captures the final expression even when trailing empty statements follow", async () => {
		...
		expect(wrapped.source).toContain("__reactor_set_final_expr__((await Promise.resolve(1)))");
	});
	...
	it("strips type-only imports before rewriting imports and top-level return", async () => {
		...
		expect(wrapped.finalExpressionReturned).toBe(true);
		expect(wrapped.source).toContain("__reactor_set_final_expr__(42)");
...
describe("wrapCode cross-cell persistence", () => {
	it("publishes top-level function declarations and their sibling consts from async-wrapped cells", async () => {
		...
		const wrapped = await wrapCode(
			"async function reactorPersistedFn(n) { return (await Promise.resolve(n)) + 1; }\nconst reactorPersistedTotal = await reactorPersistedFn(41);",
		);
		expect(wrapped.asyncWrapped).toBe(true);
		try {
			await indirectEval(wrapped.source);
			const fn = globals.reactorPersistedFn as (n: number) => Promise<number>;
			expect(typeof fn).toBe("function");
			expect(await fn(1)).toBe(2);
			expect(globals.reactorPersistedTotal).toBe(42);
		} finally {
			delete globals.reactorPersistedFn;
			delete globals.reactorPersistedTotal;
		}
	});

	it("publishes explicit top-level var declarations from async-wrapped cells", async () => {
		const globals = globalThis as Record<string, unknown>;
		const wrapped = await wrapCode("await Promise.resolve();\nvar reactorPersistedVar = 5;");
		expect(wrapped.asyncWrapped).toBe(true);
		try {
			await indirectEval(wrapped.source);
			expect(globals.reactorPersistedVar).toBe(5);
		} finally {
			delete globals.reactorPersistedVar;
		}
	});
});
