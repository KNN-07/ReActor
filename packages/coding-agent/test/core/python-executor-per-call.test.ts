...
import { executePython } from "@reactor/coding-agent/eval/py/executor";
import type { KernelExecuteOptions, KernelExecuteResult } from "@reactor/coding-agent/eval/py/kernel";
...
describe("executePython (per-call)", () => {
	...
	it("returns a cancelled timeout result when kernel startup exceeds the deadline", async () => {
		...
		using tempDir = TempDir.createSync("@reactor-python-executor-per-call-");
