...
} from "@reactor/coding-agent/mcp/startup-events";
...
describe("mcp/startup-events — connection-status cross-module contract", () => {
	...
	it("sanitizes failure errors before rendering them in status text", () => {
		const homePath = `${os.homedir()}/.reactor/mcp.log`;
		...
		expect(message).not.toContain("\t");
		expect(message).toContain("broken: failed at   ~/.reactor/mcp.log");
	});

	it("sanitizes server names before rendering them in status text", () => {
		const homePath = `${os.homedir()}/.reactor`;
		const message = formatMCPConnectionStatusMessage({
		...
		expect(message).not.toContain("\n");
		expect(message).not.toContain("\t");
		expect(message).toContain("Connected: ~/.reactor/connected   server.");
		expect(message).toContain("Failed: ~/.reactor/broken server: missing command.");
		expect(message).toContain("Still connecting: ~/.reactor/pending");
	});
