...
describe("normalizeShareServerUrl", () => {
	test("strips trailing slashes and falls back to the default", () => {
		expect(normalizeShareServerUrl("https://reactor.norman.id.vn/s/")).toBe("https://reactor.norman.id.vn/s");
		expect(normalizeShareServerUrl("https://example.com/s///")).toBe("https://example.com/s");
		expect(normalizeShareServerUrl(undefined)).toBe("https://reactor.norman.id.vn/s");
		expect(normalizeShareServerUrl("   ")).toBe("https://reactor.norman.id.vn/s");
	});
});
