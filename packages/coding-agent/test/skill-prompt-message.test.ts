...
import { buildSkillPromptMessage, type Skill } from "@reactor/coding-agent/extensibility/skills";
...
async function createSkill(body: string): Promise<{ dir: string; skill: Skill }> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), `reactor-skill-prompt-${Snowflake.next()}-`));
