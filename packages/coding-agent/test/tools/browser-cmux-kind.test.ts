...
import { resolveCmuxKind } from "@reactor/coding-agent/tools/browser";
...
describe("resolveCmuxKind", () => {
	...
	it("REACTOR_BROWSER_CMUX=0 disables cmux even when the socket environment is present", () => {
		expect(resolveCmuxKind(null, { CMUX_SOCKET_PATH: "/tmp/cmux.sock", REACTOR_BROWSER_CMUX: "0" })).toBeNull();
	});

	it("REACTOR_BROWSER_CMUX=1 enables cmux over a disabled setting", () => {
		expect(
			resolveCmuxKind({ settingEnabled: false }, { CMUX_SOCKET_PATH: "/tmp/cmux.sock", REACTOR_BROWSER_CMUX: "1" }),
		).toEqual({
			kind: "cmux",
			socketPath: "/tmp/cmux.sock",
			password: undefined,
