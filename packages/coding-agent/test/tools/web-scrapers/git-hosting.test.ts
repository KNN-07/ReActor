...
import { handleGitHub, parseGitHubUrl, stripActionsLogTimestamps } from "@reactor/coding-agent/web/scrapers/github";
import { handleGitHubGist } from "@reactor/coding-agent/web/scrapers/github-gist";
...
describe("parseGitHubUrl — Actions", () => {
	...
	it("classifies a job URL using the API-form plural `jobs` segment", () => {
		const gh = parseGitHubUrl("https://github.com/KNN-07/ReActor/actions/runs/27070071296/jobs/79897931171");
	...
	it("does not treat non-run Actions URLs (e.g. workflow files) as runs/jobs", () => {
		expect(parseGitHubUrl("https://github.com/KNN-07/ReActor/actions/workflows/ci.yml")?.type).toBe("other");
		expect(parseGitHubUrl("https://github.com/KNN-07/ReActor/actions")?.type).toBe("other");
	});

	it("does not misparse a run URL with a non-numeric id", () => {
		expect(parseGitHubUrl("https://github.com/KNN-07/ReActor/actions/runs/latest")?.type).toBe("other");
	});
...
describe("parseGitHubUrl — commit", () => {
	...
	it("falls back to `other` for a bare /commit segment with no SHA", () => {
		expect(parseGitHubUrl("https://github.com/KNN-07/ReActor/commit")?.type).toBe("other");
	});
});
