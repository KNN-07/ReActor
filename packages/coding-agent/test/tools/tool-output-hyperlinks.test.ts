...
import { resetSettingsForTest, Settings, settings } from "@reactor/coding-agent/config/settings";
import { editToolRenderer } from "@reactor/coding-agent/edit/renderer";
import { getThemeByName, initTheme } from "@reactor/coding-agent/modes/theme/theme";
import type { ToolSession } from "@reactor/coding-agent/tools";
import { astGrepToolRenderer } from "@reactor/coding-agent/tools/ast-grep";
import { ReadTool, readToolRenderer } from "@reactor/coding-agent/tools/read";
import { WriteTool, writeToolRenderer } from "@reactor/coding-agent/tools/write";
...
describe("tool output OSC 8 file:// hyperlinks", () => {
	it("links plain text and image read titles to the resolved filesystem path", async () => {
		...
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-link-read-"));
		try {
	...
	it("links the write header to the absolute path it wrote", async () => {
		settings.override("tui.hyperlinks", "always");
		const theme = (await getThemeByName("dark"))!;
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-link-write-"));
		try {
	...
	it("resolves scoped search links against cwd, not the (sub)scope path", async () => {
		...
		const projectRoot = path.resolve("/tmp/reactor-project");
	...
	it("links the edit header to the absolute details.path even when the arg path is relative", async () => {
		settings.override("tui.hyperlinks", "always");
		const theme = (await getThemeByName("dark"))!;
		const editPath = path.resolve("/tmp/reactor-project/src/a.ts");
