...
import {
	...
	parseDefaultBranch,
	parseGitHubRepo,
} from "@reactor/coding-agent/modes/components/status-line/git-utils";

describe("parseGitHubRepo", () => {
	test("parses HTTPS URL", () => {
		expect(parseGitHubRepo("https://github.com/KNN-07/ReActor.git")).toBe("KNN-07/ReActor");
	});

	test("parses HTTPS URL without .git suffix", () => {
		expect(parseGitHubRepo("https://github.com/KNN-07/ReActor")).toBe("KNN-07/ReActor");
	});

	test("parses SSH scp-style URL", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/ReActor.git")).toBe("loftiskg/ReActor");
	});

	test("parses SSH scp-style URL without .git suffix", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/ReActor")).toBe("loftiskg/ReActor");
	});
