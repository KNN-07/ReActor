...
import { JsRuntime, type RuntimeHooks } from "@reactor/coding-agent/eval/js/shared/runtime";

const GLOBAL_KEYS = ["__reactor_import__", "read"] as const;
...
function snapshotGlobals(): Record<GlobalKey, GlobalSnapshot> {
	...
	return {
		__reactor_import__: { exists: "__reactor_import__" in globals, value: globals.__reactor_import__ },
...
describe("JsRuntime global disposal", () => {
	it("keeps newer same-realm runtime globals after disposing an older runtime", () => {
		...
		try {
			...
			expect(globals.__reactor_import__).toBe(secondImport);
			expect(globals.__reactor_helpers__).toBe(second.helpers);
			expect(typeof globals.read).toBe("function");
	...
	it("reactivates older same-realm runtime globals when no other run is active", async () => {
		...
		try {
			...
			expect(globals.__reactor_helpers__).toBe(first.helpers);
	...
	it("defers cross-runtime setCwd while another same-realm runtime is running", async () => {
		...
		try {
			...
			// setCwd: the saved __reactor_session__ stack entry carries the new value.
			expect(await first.run("__reactor_session__.cwd", undefined, hooks)).toBe(pendingCwd);
			expect(first.cwd).toBe(pendingCwd);
			expect(globals.__reactor_helpers__).toBe(first.helpers);
			expect(globals.__reactor_session__).toMatchObject({ cwd: pendingCwd });
		} finally {
			gate.resolve();
