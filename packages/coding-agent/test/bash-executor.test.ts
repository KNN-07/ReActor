...
import { resetSettingsForTest, Settings, type ShellMinimizerSettings } from "@reactor/coding-agent/config/settings";
import { buildMinimizerOptions, executeBash } from "@reactor/coding-agent/exec/bash-executor";
import { DEFAULT_MAX_BYTES } from "@reactor/coding-agent/session/streaming-output";
import * as shellSnapshot from "@reactor/coding-agent/utils/shell-snapshot";
import type { Shell, ShellRunResult } from "@reactor/natives";
import * as piNatives from "@reactor/natives";
...
function makeTempDir(): string {
	return fs.mkdtempSync(path.join(os.tmpdir(), "reactor-bash-exec-"));
}
...
describe("executeBash", () => {
	...
	it("passes env vars", async () => {
		const result = await executeBash("echo $REACTOR_TEST_ENV", {
			cwd: tempDir,
			timeout: 5000,
			env: { REACTOR_TEST_ENV: "hello" },
		});
		expect(result.output.trim()).toBe("hello");
	});

	it("applies non-interactive environment defaults", async () => {
		const result = await executeBash('echo "$GIT_TERMINAL_PROMPT:$REACTOR_TEST_ENV"', {
			cwd: tempDir,
			timeout: 5000,
			env: { REACTOR_TEST_ENV: "hello" },
		});
	...
	it("runs non-bash shellPath commands through the configured shell", async () => {
		...
		const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-shellpath-"));
...
`,
	...
	it("uses executable SHELL for user-shell shortcut commands", async () => {
		...
		const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-env-shell-"));
...
`,
	...
	it("loads zshrc aliases for user-shell shortcut commands", async () => {
		...
		const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-zsh-shellpath-"));
		fs.writeFileSync(path.join(shellDir, ".zshrc"), "alias reactor_shell_alias='printf zsh-alias-ok\\\\n'\n");
		...
		try {
			const result = await executeBash("reactor_shell_alias", {
				cwd: tempDir,
				timeout: 5000,
	...
	it("restores persistent sessions after native abort cleanup settles", async () => {
		...
		await executeBash("export REACTOR_AFTER_ABORT=still_persistent", {
			cwd: tempDir,
			timeout: 5000,
			sessionKey: "settled-native-abort",
		});
	...
	it("resets persistent session state after abort", async () => {
		...
		await executeBash("export REACTOR_RESET_VAR=alive", { cwd: tempDir, timeout: 5000, sessionKey });
		const beforeAbort = await executeBash("echo $REACTOR_RESET_VAR", { cwd: tempDir, timeout: 5000, sessionKey });
		...
		const afterAbort = await executeBash("echo ${REACTOR_RESET_VAR:-unset}", {
			cwd: tempDir,
			timeout: 5000,
			sessionKey,
		});
	...
	it("sources snapshot env vars across session commands", async () => {
		...
		fs.writeFileSync(snapshotPath, "export REACTOR_SNAPSHOT_TEST=from_snapshot\n");
		vi.spyOn(Settings.prototype, "getShellConfig").mockReturnValue({
			shell: bashPath,
			args: ["-l", "-c"],
			env: {
		...
		const result = await executeBash("echo $REACTOR_SNAPSHOT_TEST", { cwd: tempDir, timeout: 5000, sessionKey });
