...
import {
	...
	type TtsrScanArgs,
	type TtsrTestArgs,
} from "@reactor/coding-agent/cli/ttsr-cli";
import { resetSettingsForTest } from "@reactor/coding-agent/config/settings";
import { getProjectAgentDir, getProjectDir, removeSyncWithRetries, setProjectDir } from "@reactor/utils";

let testTmpDir: string;

beforeAll(() => {
	testTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-ttsr-tests-"));
});
...
describe("reactor ttsr", () => {
	...
	describe("test — context inference and matching", () => {
		it("infers tool/edit context when a positional resolves to a .ts file and --source is omitted", async () => {
			...
			// Simulate `reactor ttsr test --rule <rule> src/foo.ts`: the command layer
