...
import { Settings, settings } from "@reactor/coding-agent/config/settings";
import * as downloader from "@reactor/coding-agent/stt/downloader";
import * as recorder from "@reactor/coding-agent/stt/recorder";
import { STTController } from "@reactor/coding-agent/stt/stt-controller";
import { getTinyModelsCacheDir, removeWithRetries, setAgentDir } from "@reactor/utils";
...
describe("isSttModelCached completeness", () => {
	...
	beforeEach(async () => {
		...
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-stt-cache-"));
		setAgentDir(tmp);
