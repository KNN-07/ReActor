...
import { type SkillPromptDetails, stripInternalDetailsFields } from "@reactor/coding-agent/session/messages";
import type { CustomMessageEntry } from "@reactor/coding-agent/session/session-entries";
	...
	it("F1: strips __queueChipText from persisted details while preserving all other SkillPromptDetails fields", () => {
		...
		const id = session.appendCustomMessageEntry<SkillPromptDetails>(
			...
			true,
			{
				...
				args: "bar",
				lineCount: 10,
				__queueChipText: "reactor-cmd-1-0",
			},
			"user",
		);
