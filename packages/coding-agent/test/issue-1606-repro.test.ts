/**
 * Regression for https://github.com/KNN-07/ReActor/issues/1606
 *
 ...
 * `process.execPath … __reactor_tiny_inference`, `cli.ts` dispatches that flag into
...
import { createTinyTitleSubprocess } from "@reactor/coding-agent/tiny/title-client";
...
describe("issue #1606 — tiny model lives in an isolated subprocess", () => {
	it("ping/pongs through the spawned worker subprocess and tears it down cleanly", async () => {
		...
		const script =
			'const { smokeTestTinyTitleWorker } = await import("@reactor/coding-agent/tiny/title-client"); await smokeTestTinyTitleWorker({ timeoutMs: 15000 });';
