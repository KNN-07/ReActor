...
describe("profile alias installer", () => {
	it("writes a bash-compatible function that forwards subcommands through reactor", async () => {
		const files = new Map<string, string>();

		const result = await installProfileAlias({
			profile: "work",
			aliasName: "reactor-work",
			shellPath: "/bin/bash",
			platform: "linux",
			homeDir: "/home/me",
		...
		expect(result.command).toBe("reactor --profile=work");
		expect(files.get("/home/me/.bashrc")).toContain("reactor-work() {");
		expect(files.get("/home/me/.bashrc")).toContain('command reactor --profile=work "$@"');
	});
	...
	it("can target the current source invocation instead of the installed reactor binary", async () => {
		const files = new Map<string, string>();
		...
		expect(files.get("/Users/me/.zshrc")).toContain("reactor-work() {");
	...
	it("installs the zsh alias under ZDOTDIR when set", async () => {
		...
		expect(files.get(result.configPath)).toContain("reactor-work() {");
	});
	...
	it("writes a fish function that forwards argv", async () => {
		...
		const content = files.get("/Users/me/.config/fish/conf.d/reactor-profiles.fish") ?? "";
		expect(content).toContain("function reactor-work --wraps reactor");
		expect(content).toContain("command reactor --profile=work $argv");
	});
	...
	it("installs the fish alias under XDG_CONFIG_HOME when set", async () => {
		...
		expect(result.configPath).toBe("/home/me/.dotfiles/config/fish/conf.d/reactor-profiles.fish");
		expect(files.get(result.configPath)).toContain("function reactor-work --wraps reactor");
	});
	...
	it("writes a PowerShell function because aliases cannot carry arguments", async () => {
		...
		expect(content).toContain("function reactor-work");
		expect(content).toContain("& reactor --profile=work @args");
	});
	...
	it("detects pwsh from PSModulePath when SHELL is unset on Windows", async () => {
		...
		expect(files.get(result.configPath)).toContain("& reactor --profile=work @args");
	});
	...
	it("replaces a previous block for the same alias", async () => {
		const files = new Map<string, string>([
			[
				...
				[
					"before",
					"# >>> reactor profile alias: reactor-work >>>",
					"alias reactor-work='command reactor --profile=old'",
					"# <<< reactor profile alias: reactor-work <<<",
					"after",
				].join("\n"),
			],
		...
		expect(content).toContain('command reactor --profile=work "$@"');
	...
	it("refuses to shadow the base reactor command case-insensitively", async () => {
		for (const aliasName of ["reactor", "ReActor"]) {
			await expect(
				installProfileAlias({
					profile: "work",
					aliasName,
	...
	it("validates profile names before rendering shell code", async () => {
		...
		).rejects.toThrow("Invalid ReActor profile");
	...
	it("normalizes backslashes in XDG_CONFIG_HOME for fish config paths on Windows", async () => {
		...
		expect(result.configPath).toBe("D:/xdg/fish/conf.d/reactor-profiles.fish");
		expect(result.reloadedWith).toBe("source 'D:/xdg/fish/conf.d/reactor-profiles.fish'");
	});
