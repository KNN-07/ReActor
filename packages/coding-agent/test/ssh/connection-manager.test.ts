...
import * as connectionManager from "@reactor/coding-agent/ssh/connection-manager";
import { removeWithRetries } from "@reactor/utils";

async function withLooseKey<T>(run: (keyPath: string) => Promise<T>): Promise<T> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-ssh-key-"));
...
describe("buildRemoteCommand", () => {
	...
	it("still rejects directory identity paths on Windows args", async () => {
		const keyPath = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-ssh-key-"));
		try {
			await expect(
				connectionManager.buildRemoteCommand(
					{
