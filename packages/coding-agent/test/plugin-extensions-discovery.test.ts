...
import { discoverAndLoadExtensions } from "@reactor/coding-agent/extensibility/extensions/loader";
import { getAgentDir, getPluginsDir, removeSyncWithRetries, setAgentDir, TempDir } from "@reactor/utils";

const currentPiCodingAgentPath = Bun.resolveSync("@reactor/coding-agent", import.meta.dir);
const currentPiExtensionsPath = Bun.resolveSync("@reactor/coding-agent/extensibility/extensions", import.meta.dir);
...
describe("plugin extension discovery", () => {
	...
	beforeEach(() => {
		...
		// resolves into `<tempHome>/.reactor/plugins` on every platform. Two things are needed:
		//  - mock os.homedir() so configRoot = `<tempHome>/.reactor` (the previous
		//    XDG_DATA_HOME redirect was a no-op on Windows, where these tests then wrote
		//    into and rm'd the developer's real `~/.reactor/plugins`);
		//  - clear the XDG_* vars, because on Linux/macOS the resolver prefers
		//    `$XDG_DATA_HOME/reactor` over the home config root when that dir exists, so an
		...
		setAgentDir(path.join(tempHome, ".reactor", "agent"));
		...
		// ~/.reactor must fail loudly here instead of clobbering the developer's plugins.
	...
	it("loads installed legacy Pi plugin extensions from Windows drive-letter paths", async () => {
		...
		fs.writeFileSync(
			...
			[
				...
				'import { isToolCallEventType as legacyRoot } from "@mariozechner/coding-agent";',
				'import { isToolCallEventType as legacyExtensions } from "@mariozechner/coding-agent/extensibility/extensions";',
	...
	it("loads installed legacy Pi plugin extensions that use package imports", async () => {
		...
		fs.writeFileSync(
			...
			[
				'import { isToolCallEventType as legacyExtensions } from "@earendil-works/coding-agent/extensibility/extensions";',
				`import { isToolCallEventType as modernExtensions } from ${JSON.stringify(currentPiExtensionsPath)};`,
				"",
				'if (legacyExtensions !== modernExtensions) throw new Error("legacy extension import did not remap");',
	...
	it("resolves a sub-extension directory via its own package.json manifest over a decoy index", async () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ name: "feature-ext", version: "1.0.0", reactor: { extensions: ["./dist/real-ext.ts"] } }),
		);
		fs.writeFileSync(
			realEntry,
			[
