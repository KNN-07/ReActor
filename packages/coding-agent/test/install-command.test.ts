/**
 * Regression test for #1496 (bug 2): `reactor install ./my-extension` used to be
...
import { commands, isSubcommand, resolveCliArgv } from "@reactor/coding-agent/cli-commands";
import { looksLikeLocalPath } from "@reactor/coding-agent/commands/install";
...
describe("install command is registered as a top-level subcommand", () => {
	...
	test("CLI runner rejects only bare reserved management words", () => {
		expect(resolveCliArgv(["extensions"])).toEqual({
			error: '`reactor extensions` is not a management command. Use `reactor plugin list` / `reactor plugin install`, or run `reactor launch extensions` if you meant to send "extensions" as a prompt.',
		});
...
describe("looksLikeLocalPath", () => {
	...
	test("npm specs and marketplace refs are remote", () => {
		expect(looksLikeLocalPath("@reactor/exa")).toBe(false);
	...
	test("bare names that exist as a local directory are treated as local", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "reactor-install-test-"));
		try {
