...
} from "@reactor/coding-agent/internal-urls/registry-helpers";
import * as planHandoff from "@reactor/coding-agent/plan-mode/plan-handoff";
...
import * as isolationRunner from "@reactor/coding-agent/task/isolation-runner";
import {
...
} from "@reactor/coding-agent/task/structured-subagent";
...
describe("structured subagent primitive", () => {
	...
	it("gives task and eval invocations identical blocked-agent preflight errors", async () => {
		const previous = Bun.env.REACTOR_BLOCKED_AGENT;
		Bun.env.REACTOR_BLOCKED_AGENT = "worker";
		try {
			...
			if (previous === undefined) delete Bun.env.REACTOR_BLOCKED_AGENT;
			else Bun.env.REACTOR_BLOCKED_AGENT = previous;
		}
	});
	...
	it("leases temporary artifacts for a retained invocation and registers them for agent URLs", async () => {
		...
		expect(path.basename(settled.artifactsDir)).toStartWith("reactor-task-");
	...
	it("persists nested patch text with the compatible recovery path and wording", async () => {
		const artifactsDir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-structured-subagent-"));
		const completed = result();
		completed.patchPath = "/recovery/Worker.patch";
		completed.branchName = "reactor/task/Worker";
		...
		expect(hint).toContain("Captured branch preserved as reactor/task/Worker.");
