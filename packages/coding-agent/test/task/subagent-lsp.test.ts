...
import type { AgentDefinition, TaskParams } from "@reactor/coding-agent/task/types";
import type { IsolationHandle, WorktreeBaseline } from "@reactor/coding-agent/task/worktree";
...
import "@reactor/coding-agent/tools/yield";
...
describe("subagent LSP availability", () => {
	...
	it("opens isolated persisted subagent sessions with the worktree cwd", async () => {
		...
		mockIsolation();
		const { getOptions } = mockCreateAgentSession();
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-isolated-session-cwd-"));
		try {
