 ...
 *
 * The native `reactor` discovery provider only walks `.reactor/` and `~/.reactor/agent/`.
 * Extension packages registered via `extensions:` in settings or
 * `--extension` on the CLI ship their own `skills/`, `hooks/`, `tools/`,
 * `commands/`, `rules/`, `prompts/`, and `.mcp.json`. The `reactor-plugins`
 * provider (`src/discovery/reactor-plugins.ts`) is what wires those sub-trees
...
import { getCapability } from "@reactor/coding-agent/capability";
import { clearCache } from "@reactor/coding-agent/capability/fs";
import { hookCapability } from "@reactor/coding-agent/capability/hook";
import { mcpCapability } from "@reactor/coding-agent/capability/mcp";
import { promptCapability } from "@reactor/coding-agent/capability/prompt";
import { ruleCapability } from "@reactor/coding-agent/capability/rule";
import { skillCapability } from "@reactor/coding-agent/capability/skill";
import { slashCommandCapability } from "@reactor/coding-agent/capability/slash-command";
import { toolCapability } from "@reactor/coding-agent/capability/tool";
import type { LoadContext, Provider } from "@reactor/coding-agent/capability/types";
...
function buildExtensionPackage(packageDir: string): void {
	writeFile(
		...
		JSON.stringify({ name: path.basename(packageDir), reactor: { extensions: ["./src/main.ts"] } }),
	);
...
beforeEach(() => {
	...
	setAgentDir(path.join(home, ".reactor", "agent"));
});
...
test("project settings.json#extensions surfaces every sub-directory", async () => {
	writeFile(path.join(project, ".reactor", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("user settings.json#extensions also feeds sub-discovery", async () => {
	writeFile(path.join(home, ".reactor", "agent", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("file-extension entrypoints contribute zero sub-surface (the file has no siblings to scan)", async () => {
	...
	writeFile(path.join(project, ".reactor", "settings.json"), JSON.stringify({ extensions: [standaloneFile] }));
...
test("relative paths in settings resolve against the project cwd", async () => {
	...
	writeFile(path.join(project, ".reactor", "settings.json"), JSON.stringify({ extensions: [`./${relative}`] }));
...
test("installed plugins under `<plugins>/node_modules/` are surfaced (e.g. via `reactor plugin link`/`install`)", async () => {
	...
	const pluginsDir = path.join(home, ".reactor", "plugins");
	...
	writeFile(
		...
		JSON.stringify({ name: "reactor-plugins", dependencies: { "my-installed-ext": "1.0.0" } }),
	);
	// Plugin's own package.json must carry an `reactor`/`pi` manifest for the
	// loader to recognise it; the buildExtensionPackage fixture already wrote
	// one with `reactor.extensions`, which is sufficient.
...
test("project-scoped installed plugins surface project-level sub-discovery", async () => {
	const pluginsDir = path.join(project, ".reactor", "plugins");
	const installed = path.join(pluginsDir, "node_modules", "my-project-ext");
	fs.mkdirSync(installed, { recursive: true });
	fs.cpSync(ext, installed, { recursive: true });
	writeFile(
		path.join(pluginsDir, "reactor-plugins.lock.json"),
		JSON.stringify({
...
test("disabled installed plugins do not contribute sub-discovery", async () => {
	...
	writeFile(
		path.join(pluginsDir, "package.json"),
		JSON.stringify({ name: "reactor-plugins", dependencies: { "my-disabled-ext": "1.0.0" } }),
	);
	writeFile(
		path.join(pluginsDir, "reactor-plugins.lock.json"),
...
test("linked plugins (only in lockfile, not in package.json#dependencies) are surfaced", async () => {
	// `reactor plugin link ./local-ext` creates a symlink under
	...
	// still find the package — otherwise the documented `reactor install
