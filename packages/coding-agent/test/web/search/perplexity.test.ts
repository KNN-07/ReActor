...
import { PerplexityProvider, searchPerplexity } from "@reactor/coding-agent/web/search/providers/perplexity";
import { getAvailableAuthMethods } from "@reactor/coding-agent/web/search/providers/perplexity-auth";
...
describe("Perplexity API-key request shape", () => {
	...
	const savedResponsesMode = process.env.REACTOR_PERPLEXITY_RESPONSES;

	beforeEach(() => {
		process.env.PERPLEXITY_API_KEY = "test-key";
		delete process.env.PERPLEXITY_COOKIES;
		delete process.env.REACTOR_PERPLEXITY_RESPONSES;
	});
	...
	afterEach(() => {
		...
		if (savedResponsesMode === undefined) delete process.env.REACTOR_PERPLEXITY_RESPONSES;
		else process.env.REACTOR_PERPLEXITY_RESPONSES = savedResponsesMode;
	});
	...
	it("streams the Responses API and captures Perplexity search result events", async () => {
		process.env.REACTOR_PERPLEXITY_RESPONSES = "1";
		let body: Record<string, unknown> | undefined;
		const fetchMock: FetchImpl = async (input, init) => {
			const url = typeof input === "string" ? input : input instanceof URL ? input.toString() : input.url;
