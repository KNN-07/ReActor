...
import { __buildLegacyPiPackageRootOverrides } from "@reactor/coding-agent/extensibility/plugins/legacy-pi-compat";
...
// failed to resolve `@earendil-works/ai/oauth` because the override map
...
// the same `reactor-legacy-pi-bundled:` virtual namespace as package roots without
...
describe("legacy pi compat compiled-mode subpath overrides (issue #3442)", () => {
	it("does not evaluate unrelated host modules while loading the registry", async () => {
		using tempDir = TempDir.createSync("@reactor-legacy-pi-loaders-");
...
`,
	...
	it("serves @reactor/ai/oauth through the bundled virtual namespace in compiled mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@reactor/ai/oauth"]).toBe("reactor-legacy-pi-bundled:@reactor/ai/oauth");
	});

	it("expands wildcard exports for concrete on-disk targets (issue #3442 follow-up)", () => {
		// `ai/oauth/anthropic` is exposed via the `./oauth/*` wildcard export;
		// the original fix only bundled non-wildcard subpaths, so peer-only plugins
		// importing `@(scope)/ai/oauth/anthropic` (remapped via REACTOR_SUBPATH_REMAPS
		// from `@mariozechner/ai/utils/oauth/anthropic`) still hit the bunfs
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@reactor/ai/oauth/anthropic"]).toBe("reactor-legacy-pi-bundled:@reactor/ai/oauth/anthropic");
		// Sanity: the wildcard expansion also reaches deeper subroots so plugins
		// pinned to e.g. `@reactor/ai/providers/openai` keep resolving.
		expect(bundledModuleKeys.has("@reactor/ai/oauth/anthropic")).toBe(true);
		expect(bundledModuleKeys.has("@reactor/ai/oauth/openai-codex")).toBe(true);
	});

	it("expands web search provider wildcard exports for compiled plugin imports", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		const providerKeys = [
			"@reactor/coding-agent/web/search/providers/xai",
			"@reactor/coding-agent/web/search/providers/tinyfish",
			"@reactor/coding-agent/web/search/providers/firecrawl",
			"@reactor/coding-agent/web/search/providers/duckduckgo",
		] as const;

		for (const key of providerKeys) {
			expect(bundledModuleKeys.has(key)).toBe(true);
			expect(overrides[key]).toBe(`reactor-legacy-pi-bundled:${key}`);
		}
	});
	...
	it("does not enumerate root catch-all wildcards (./* / ./*.js)", () => {
		...
		// Concrete check: `@reactor/coding-agent/cli` is NOT bundled.
		expect(bundledModuleKeys.has("@reactor/coding-agent/cli")).toBe(false);
		expect(bundledModuleKeys.has("@reactor/coding-agent/main")).toBe(false);
	});
	...
	it("does not bundle main-thread-unsafe worker entrypoints", () => {
		...
		expect(bundledModuleKeys.has("@reactor/coding-agent/eval/js/worker-entry")).toBe(false);
	});
	...
	it("maps every bundled key (minus shimmed roots + typebox) to its virtual specifier in compiled mode", () => {
		...
		for (const key of bundledModuleKeys) {
			// ai/coding-agent roots intentionally use the legacy compat shims
			// (they re-attach `Type`, `defineTool`, etc. dropped from the canonical
			// package surface); typebox is served via TYPEBOX_SHIM_PATH.
			if (key === "@reactor/ai" || key === "@reactor/coding-agent" || key === "typebox") continue;
			if (overrides[key] !== `reactor-legacy-pi-bundled:${key}`) {
				missing.push(key);
			}
		}
	...
	it("keeps ai/coding-agent roots routed to their compat shims in compiled mode", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@reactor/ai"]).toBeDefined();
		expect(overrides["@reactor/ai"]).not.toBe("reactor-legacy-pi-bundled:@reactor/ai/oauth");
		expect(overrides["@reactor/coding-agent"]).toBeDefined();
	});

	it("does not register subpath overrides in dev/install mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(false);
		expect(overrides).not.toHaveProperty("@reactor/ai/oauth");
		expect(overrides).not.toHaveProperty("@reactor/coding-agent/tools");
