...
import { __computeBundledSelfPackageRoot } from "@reactor/coding-agent/extensibility/plugins/legacy-pi-compat";
...
// bundled host packages and shims through the `reactor-legacy-pi-bundled:`
...
describe("legacy pi compat bundled-self package root computation", () => {
	it("derives the npm prebuilt bundle package root from dist import.meta.dir", () => {
		...
		expect(__computeBundledSelfPackageRoot(posixMetaDir, path.posix)).toBe(
			"/home/me/.bun/install/global/node_modules/@reactor/coding-agent",
		);
	});

	it("derives the source package root when REACTOR_BUNDLED is used outside dist", () => {
