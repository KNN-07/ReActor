...
import { resizeImage } from "@reactor/coding-agent/utils/image-resize";
...
describe("resizeImage env wiring", () => {
	const prior = Bun.env.REACTOR_NO_WEBP;

	beforeEach(() => {
		delete (Bun.env as Record<string, string | undefined>).REACTOR_NO_WEBP;
	});

	afterEach(() => {
		if (prior === undefined) delete (Bun.env as Record<string, string | undefined>).REACTOR_NO_WEBP;
		else Bun.env.REACTOR_NO_WEBP = prior;
	});

	it("treats REACTOR_NO_WEBP=1 set at call time as exclusion (not baked at module load)", async () => {
		Bun.env.REACTOR_NO_WEBP = "1";
