...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@reactor/utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.reactor/`), plus
//    the legacy non-XDG `~/.reactor/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.reactor/agent` or `$REACTOR_CODING_AGENT_DIR`)
//    Handles `REACTOR_CODING_AGENT_DIR` overrides that relocate the agent dir
//    (and therefore `agent/extensions/`) out from under the config root.
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/reactor/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.reactor` when
//    XDG_DATA_HOME resolves the plugins dir somewhere else.
//
// We deliberately do NOT use `pathIsWithin` from utils here: that helper
// resolves symlinks via `fs.realpathSync` on both sides, so a contributor who
// symlinks `~/.reactor/agent/extensions/foo.ts` to a dotfiles repo would see the
