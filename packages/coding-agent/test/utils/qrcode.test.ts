...
import { QrCode, type QrEcLevel, renderQrHalfBlocks } from "@reactor/coding-agent/utils/qrcode";
...
describe("QR encoder", () => {
	...
	const vectors: ReadonlyArray<{
		...
		{
			text: "https://reactor.norman.id.vn/#mgAYTZwEnpRQtca0CTgn-Q.gdJU",
			ecl: "M",
			mask: 4,
			version: 4,
			size: 33,
			hash: "94545633a75c5d8f",
		},
		{
	...
	it("deterministically selects a penalty-minimizing mask when none is forced", () => {
		...
		const qr = QrCode.encodeText("https://reactor.norman.id.vn/#demo", "M");
		expect(qr.mask).toBe(2);
		expect(matrixFingerprint(qr)).toBe("34dc88b8dc13dba1");
	});
