...
import { getManagedSkillsDir } from "@reactor/coding-agent/autolearn/managed-skills";
import "@reactor/coding-agent/discovery";
import { loadSkills } from "@reactor/coding-agent/extensibility/skills";
import { removeWithRetries } from "@reactor/utils";
import { getAgentDir, setAgentDir } from "@reactor/utils/dirs";
...
describe("managed-skills discovery", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-managed-disco-home-"));
		// cwd MUST live under the fake home so loadSkills' ancestor walk is bounded
		// and cannot pick up ambient /tmp/.reactor or /.reactor fixtures (full-suite-safe).
	...
	it("surfaces a managed skill tagged with the reactor-managed provider", async () => {
		await writeSkill(managedDir, "foo", "A managed skill.");
		const { skills } = await loadSkills({ cwd: tempCwd });
		const foo = skills.find(s => s.name === "foo");
		expect(foo).toBeDefined();
		expect(foo?.source).toBe("reactor-managed:user");
	});
	...
	it("lets an authored skill win a name collision and drops the managed one", async () => {
		...
		expect(skills.some(s => s.name === "bar" && s.source === "reactor-managed:user")).toBe(false);
	});
	...
	it("lets an authored skill from a NON-native provider win over a managed skill", async () => {
		...
		expect(skills.some(s => s.name === "baz" && s.source === "reactor-managed:user")).toBe(false);
	});
	...
	it("keeps a managed skill visible even when a disabled provider has the same name", async () => {
		...
		expect(dises[0]?.source).toBe("reactor-managed:user");
	});
	...
	it("keeps managed skills dead-last behind an enabled authored fallback hidden by a disabled duplicate (#4648)", async () => {
		...
		expect(skills.some(s => s.name === "shadowed" && s.source === "reactor-managed:user")).toBe(false);
	});
	...
	it("skips a managed skill whose on-disk frontmatter name is unsafe", async () => {
		...
		expect(skills.some(s => s.source === "reactor-managed:user")).toBe(false);
	});
