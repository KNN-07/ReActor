...
import { createSessionManager } from "@reactor/coding-agent/main";
...
describe("createSessionManager — cross-project --resume cancellation (#1668)", () => {
	...
	beforeEach(async () => {
		existingProject = await fsp.mkdtemp(path.join(os.tmpdir(), "reactor-xproj-"));
	});
...
describe("createSessionManager — cross-project --resume relocation (moved worktree)", () => {
	...
	beforeEach(async () => {
		missingRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "reactor-moved-xproj-"));
