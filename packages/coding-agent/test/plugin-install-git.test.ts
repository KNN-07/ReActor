 ...
 *
 * Strategy: spy on the six `@reactor/utils` plugin-path getters so the
 ...
 * `piUtils.getInstallId`), so we know namespace spying on `utils` exports
...
describe("PluginManager.install with git sources", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-plugin-git-"));
	...
	test("installs from github: shorthand and resolves real package name from deps diff", async () => {
		...
		await Bun.write(
			pluginsPkgJson,
			JSON.stringify({ name: "reactor-plugins", private: true, dependencies: {} }, null, 2),
		);
