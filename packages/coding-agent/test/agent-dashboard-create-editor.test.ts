...
import { AgentDashboard } from "@reactor/coding-agent/modes/components/agent-dashboard";
import { initTheme } from "@reactor/coding-agent/modes/theme/theme";
import * as discovery from "@reactor/coding-agent/task/discovery";
...
async function makeTempCwd(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-agent-dashboard-"));
	tempDirs.push(dir);
	return dir;
}
