...
import { AuthStorage, type CredentialDisabledEvent } from "@reactor/ai";
import * as oauthUtils from "@reactor/ai/oauth";
...
import type { Extension, ExtensionError, ExtensionFactory } from "@reactor/coding-agent/extensibility/extensions";
import { ExtensionRunner } from "@reactor/coding-agent/extensibility/extensions";
import { ExtensionRuntime } from "@reactor/coding-agent/extensibility/extensions/loader";
...
describe("createAgentSession credential_disabled subscription", () => {
	...
	const baseOptions = (dirs: SessionDirs, authStorage: AuthStorage, extensions: ExtensionFactory[] = []) => ({
		...
		// loads the developer's real ~/.reactor models config on every construction (~100ms each,
