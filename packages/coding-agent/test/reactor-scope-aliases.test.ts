 ...
 * `@mariozechner` (original), `@earendil-works` (fork), and `@ReActor`
 ...
 *   - `@reactor/swarm-extension`         ⇒ `@reactor/utils`
 *   - `@plannotator/pi-extension`         ⇒ `@reactor/agent-core`
 *   - `@runfusion/fusion`                 ⇒ `@reactor/coding-agent/...`
 *
...
const canonicalCodingAgent = Bun.resolveSync("@reactor/coding-agent", import.meta.dir);
const canonicalCodingAgentExtensions = Bun.resolveSync(
	"@reactor/coding-agent/extensibility/extensions",
	import.meta.dir,
);
const canonicalUtils = Bun.resolveSync("@reactor/utils", import.meta.dir);
const canonicalTui = Bun.resolveSync("@reactor/tui", import.meta.dir);
// Subpath: upstream `ai/oauth` re-exported `utils/oauth/index`; our ai now
// exposes the same surface at the real `@reactor/ai/oauth` export, so the
// legacy `@mariozechner/ai/oauth` specifier canonicalizes straight to it.
const canonicalAiOauth = Bun.resolveSync("@reactor/ai/oauth", import.meta.dir);
...
const CASES: readonly AliasCase[] = [
	...
	// @ReActor self-import — canonical scope must still flow through the shim
	// so a duplicate copy is never dragged in from a plugin's own node_modules.
	{ id: "ohmyutils", aliasSpecifier: "@reactor/utils", canonicalPath: canonicalUtils, symbol: "logger" },
	{
		id: "ohmycoding-agent",
		aliasSpecifier: "@reactor/coding-agent",
	...
	// Subpath: legacy `ai/oauth` resolves to the real `@reactor/ai/oauth`.
	{
...
describe("pi-* scope aliases", () => {
	...
	beforeEach(() => {
		projectDir = TempDir.createSync("@reactor-scope-aliases-");
