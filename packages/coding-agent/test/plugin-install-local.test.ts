/**
 * Routing tests for `reactor plugin install <local-path>` (#1945).
 *
...
import { runPluginCommand } from "@reactor/coding-agent/cli/plugin-cli";
import { PluginManager } from "@reactor/coding-agent/extensibility/plugins/manager";
import { MarketplaceManager } from "@reactor/coding-agent/extensibility/plugins/marketplace";
import type { InstalledPlugin } from "@reactor/coding-agent/extensibility/plugins/types";
...
async function createLocalPlugin(root: string): Promise<string> {
	...
	await Bun.write(
		...
		JSON.stringify({
			name: "kimi-datasource",
			version: "1.0.0",
			reactor: { extensions: ["./src/extension.ts"] },
		}),
	);
	return localPlugin;
}
...
describe("runPluginCommand({ action: 'install', args: [<local>] })", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-plugin-install-local-"));
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(tmpRoot, "reactor-plugins.lock.json"));
	...
	test("real local plugin directory: install symlinks it like link would", async () => {
		...
		// symlink rather than copy-install, matching `reactor plugin link`.
	...
	test("doctor --fix preserves linked local plugin state without package dependencies", async () => {
		await Bun.write(
			...
			JSON.stringify({ name: "reactor-plugins", private: true, dependencies: {} }),
		);
		const localPlugin = await createLocalPlugin(tmpRoot);
