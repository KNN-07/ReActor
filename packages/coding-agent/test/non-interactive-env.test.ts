...
import { buildNonInteractiveEnv } from "@reactor/coding-agent/exec/non-interactive-env";
...
it("keeps launch .env.local values out of child shell config", async () => {
	const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-env-local-"));
	try {
		await Bun.write(
		...
		const script = [
			...
			"	inherited: env.REACTOR_TEST_INHERITED_MARKER ?? null,",
			"}));",
		].join("\n");
		const proc = Bun.spawn([process.execPath, "--no-install", "--eval", script], {
			...
			env: {
				...
				REACTOR_TEST_INHERITED_MARKER: "keep-me",
