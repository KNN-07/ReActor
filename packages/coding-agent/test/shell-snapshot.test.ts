...
import { getOrCreateSnapshot, sanitizeSnapshotForBrush } from "@reactor/coding-agent/utils/shell-snapshot";
...
describe("sanitizeSnapshotForBrush", () => {
	...
	it("leaves non-alias lines (functions, exports, unalias) untouched", () => {
		const snapshot = [
			"# Shell snapshot - generated by reactor agent",
			"unalias -a 2>/dev/null || true",
			"my_fn () { echo hi; }",
...
// `__reactor_emit_referenced_exports` (shell-snapshot-fn-env.sh) re-exports env
...
describe("getOrCreateSnapshot", () => {
	it("re-exports env vars referenced by snapshotted functions (issue #3470)", async () => {
		const home = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-snap-3470-"));
		await fs.writeFile(
		...
		const shellLink = path.join(home, "bash-reactor-3470");
		...
		// reason — UUID filenames shouldn't leak via `ls /tmp/reactor-shell-snapshots`.
	...
	it("keeps the snapshot file at 0600 even when the rc file resets umask to 022", async () => {
		...
		const home = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-snap-umask-"));
		await fs.writeFile(
			path.join(home, ".bashrc"),
		...
		const realBash = REAL_BASH;
		if (!existsSync(realBash)) return;
		const shellLink = path.join(home, "bash-reactor-umask");
		await fs.symlink(realBash, shellLink);
	...
	it("cleans up the empty snapshot file when the shell exits with a non-zero code", async () => {
		const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-snap-fail-"));
		...
		try {
			...
			const snapshotDir = path.join(testRoot, "reactor-shell-snapshots");
	...
	it("cleans up the empty snapshot file when the shell fails to spawn", async () => {
		const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-snap-spawn-fail-"));
		const originalTmpDir = process.env.TMPDIR;
		process.env.TMPDIR = testRoot;
		try {
	...
	it("cleans up the empty snapshot file when the shell execution times out", async () => {
		const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-snap-timeout-"));
		const originalTmpDir = process.env.TMPDIR;
		process.env.TMPDIR = testRoot;
		try {
