 ...
 * `BlobStore` keep writing to `~/.reactor/agent/...`. Reach for an object store
 * (S3, R2, GCS) if you need those off-host too.
 */

import { createAgentSession, RedisSessionStorage, SessionManager } from "@reactor/coding-agent";
...
const storage = await RedisSessionStorage.create({
	client: redis,
	prefix: "reactor:sessions:", // optional, this is the default
});
