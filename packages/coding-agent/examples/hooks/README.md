...
Example hooks for reactor-coding-agent.

## Usage

```bash
# Load a hook with --hook flag
reactor --hook examples/hooks/permission-gate.ts

# Or copy to hooks directory for auto-discovery
cp permission-gate.ts ~/.reactor/agent/hooks/
```
...
```typescript
import type { HookAPI } from "@reactor/coding-agent/hooks";
