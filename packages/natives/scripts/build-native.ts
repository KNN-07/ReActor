...
const rustDir = path.join(repoRoot, "crates/reactor-natives");
...
async function resolveBuiltAddonPath(outputDir: string, canonicalFilename: string): Promise<string> {
	...
	const generatedCandidates = entries.filter(entry => {
		if (!entry.startsWith(`reactor_natives.${targetPlatform}-${targetArch}`) || !entry.endsWith(".node")) {
			return false;
		}
		return true;
	});
...
const canonicalAddonFilename = `reactor_natives.${targetPlatform}-${targetArch}${variantSuffix}.node`;
const canonicalAddonPath = path.join(nativeDir, canonicalAddonFilename);

console.log(`Building reactor-natives for ${targetPlatform}-${targetArch}${variantSuffix}${profileSuffix}…`);
