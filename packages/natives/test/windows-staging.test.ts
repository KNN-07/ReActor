/**
 * Regression for the Windows `bun install -g` update path: when an `reactor`
 * process is running, bun cannot overwrite a locked
 * `node_modules/@reactor/natives/native/reactor_natives.win32-x64.node` during
 ...
 *      (per-package-version cache under `~/.reactor/natives/<version>/`) so the
...
const winNodeModulesNativeDir = "C:\\Users\\Admin\\node_modules\\@ReActor\\reactor-natives\\native";
const winWorkspaceNativeDir = "C:\\Users\\Admin\\dev\\ReActor\\packages\\natives\\native";
const posixNodeModulesNativeDir = "/home/u/proj/node_modules/@reactor/natives/native";
...
describe("windows native addon staging", () => {
	...
	it("prepends versionedDir candidates ahead of node_modules when staging on Windows", () => {
		const versionedDir = "C:\\Users\\Admin\\.reactor\\natives\\15.0.1";
		const userDataDir = "C:\\Users\\Admin\\AppData\\Local\\reactor";
		...
		const versionedBaseline = path.join(versionedDir, "reactor_natives.win32-x64-baseline.node");
		const versionedDefault = path.join(versionedDir, "reactor_natives.win32-x64.node");
		const nodeModulesBaseline = path.join(winNodeModulesNativeDir, "reactor_natives.win32-x64-baseline.node");
		...
		// addon anyway).
		const userDataBaseline = path.join(userDataDir, "reactor_natives.win32-x64-baseline.node");
	...
	it("falls back to the node_modules-only candidate list when staging is off", () => {
		...
		const versionedDir = "/home/u/.reactor/natives/15.0.1";
		const candidates = resolveLoaderCandidates({
		...
		const nodeModulesBaseline = path.join(posixNodeModulesNativeDir, "reactor_natives.linux-x64-baseline.node");
	...
	it("removes stale version directories after the current native version loads", async () => {
		const nativesDir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-natives-cache-"));
		try {
...
describe("reactor-natives version sentinel", () => {
	it("Rust `js_name` matches the package version", async () => {
		...
		const libRs = await Bun.file(path.join(import.meta.dir, "../../../crates/reactor-natives/src/lib.rs")).text();
