...
describe("generated native npm leaf packages", () => {
	it("builds an x64 leaf manifest that exposes all addon files without package exports", () => {
		const addonFiles = ["reactor_natives.linux-x64-baseline.node", "reactor_natives.linux-x64-modern.node"];
		...
		expect(manifest.name).toBe("@reactor/natives-linux-x64");
	...
	it("uses the default addon as the main entry for non-x64 leaves", () => {
		const addonFiles = ["reactor_natives.darwin-arm64.node"];
		const manifest = buildLeafManifest({
			tag: "darwin-arm64",
			os: "darwin",
			cpu: "arm64",
		...
		expect(manifest.name).toBe("@reactor/natives-darwin-arm64");
		expect(manifest.os).toEqual(["darwin"]);
		expect(manifest.cpu).toEqual(["arm64"]);
		expect(manifest.main).toBe("./reactor_natives.darwin-arm64.node");
		expect(addonFiles).toContain(manifest.main.slice("./".length));
		expect("exports" in manifest).toBe(false);
	});

	it("generates every leaf package by copying present addon files", async () => {
		const packageDir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-natives-npm-"));
		try {
			...
			const addonFiles = [
				"reactor_natives.linux-x64-baseline.node",
				"reactor_natives.linux-x64-modern.node",
				"reactor_natives.linux-arm64.node",
				"reactor_natives.darwin-x64-baseline.node",
				"reactor_natives.darwin-arm64.node",
				"reactor_natives.win32-x64-baseline.node",
			];
			...
			expect(
				await Bun.file(path.join(packageDir, "npm/linux-x64/reactor_natives.linux-x64-modern.node")).text(),
			).toBe("reactor_natives.linux-x64-modern.node");
			const manifest = await Bun.file(path.join(packageDir, "npm/linux-x64/package.json")).json();
			expect(manifest.main).toBe("./reactor_natives.linux-x64-baseline.node");
			expect("exports" in manifest).toBe(false);
		} finally {
	...
	it("reports missing leaves during dry runs without writing generated packages", async () => {
		const packageDir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-natives-npm-dry-"));
		const logSpy = spyOn(console, "log").mockImplementation(() => {});
		try {
			await fs.mkdir(path.join(packageDir, "native"));
			await Bun.write(path.join(packageDir, "package.json"), JSON.stringify({ version: "15.5.15" }));
			await Bun.write(path.join(packageDir, "native/reactor_natives.darwin-arm64.node"), "darwin");
