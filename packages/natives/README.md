...
```typescript
import { grep, find, encodeSixel } from "@reactor/natives";
...
`@reactor/natives` publishes a small core package plus generated
platform-specific optional dependency packages:

```
crates/reactor-natives/       # Rust source (workspace member)
...
native/                  # Core loader files and local/CI native build outputs
  ...
  reactor_natives.<platform>-<arch>-modern.node   # x64 modern ISA (local/CI artifact)
  reactor_natives.<platform>-<arch>-baseline.node # x64 baseline ISA (local/CI artifact)
  reactor_natives.<platform>-<arch>.node          # non-x64 build artifact
npm/<platform>-<arch>/   # Generated at publish time, not committed
  package.json           # @reactor/natives-<platform>-<arch>
