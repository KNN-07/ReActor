...
- Bundled-model lookups (`getBundledModel`, `GeneratedProvider`) now import from the new `@reactor/catalog` package instead of the `@reactor/ai` barrel, which no longer re-exports catalog values
- The session-sync worker re-enters the host CLI entry (`workerHostEntry()` + `__omp_stats_sync_worker` argv selector) when running inside omp — source, npm bundle, or compiled binary — and keeps loading its own `sync-worker.ts` module directly for standalone `reactor-stats`, bun test, and SDK hosts
