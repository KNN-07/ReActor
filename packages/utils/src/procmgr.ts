...
import { Process, ProcessStatus } from "@reactor/natives";
...
function buildSpawnEnv(shell: string): Record<string, string> {
	const noCI = $env.REACTOR_BASH_NO_CI || $env.CLAUDE_BASH_NO_CI;
	return {
 ...
 * Supports REACTOR_BASH_NO_LOGIN and CLAUDE_BASH_NO_LOGIN to skip -l.
 */
function getShellArgs(): string[] {
	const noLogin = $env.REACTOR_BASH_NO_LOGIN || $env.CLAUDE_BASH_NO_LOGIN;
...
function getShellPrefix(): string | undefined {
	return $env.REACTOR_SHELL_PREFIX || $env.CLAUDE_CODE_SHELL_PREFIX;
}
...
export function getShellConfig(customShellPath?: string): ShellConfig {
	...
	if (customShellPath) {
		...
		throw new Error(
			`Custom shell path not found: ${customShellPath}\nPlease update shellPath in ~/.reactor/agent/settings.json`,
		);
	}
	...
	if (process.platform === "win32") {
		...
		throw new Error(
			`No bash shell found. Options:\n` +
				...
				`  3. Set shellPath in ~/.reactor/agent/settings.json\n\n` +
