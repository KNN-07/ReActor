 ...
 * when `REACTOR_TIMING` is set) that record an inclusive module window plus resolved
 * static child edges:
 *
 ...
 * cheap to preload before utils (and winston) exist. The buffer is shared with
 ...
 * - **Dev runs only.** In the compiled `reactor` binary every module is pre-bundled
...
const MODULE_COMPLETE_KEY: symbol = Symbol.for("reactor.moduleLoadComplete");
const MODULE_BODY_START_KEY: symbol = Symbol.for("reactor.moduleBodyStart");
...
function bodyStartMarker(path: string): string {
	return `;globalThis[Symbol.for("reactor.moduleBodyStart")]?.(${JSON.stringify(path)});\n`;
}

function completionMarker(path: string): string {
	return `\n;globalThis[Symbol.for("reactor.moduleLoadComplete")]?.(${JSON.stringify(path)});\n`;
}
