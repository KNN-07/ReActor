/**
 * Centralized path helpers for reactor config directories.
 *
 * Uses REACTOR_CONFIG_DIR (default ".reactor") for the config root and
 * REACTOR_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/reactor/. This requires running `reactor config migrate` first to
 * move data to the new locations. No filesystem existence checks are performed
 * — if the env var is set, reactor trusts that the migration has been done.
 */
...
/** App name (e.g. "reactor") */
export const APP_NAME: string = "reactor";

/** Config directory name (e.g. ".reactor") */
export const CONFIG_DIR_NAME: string = ".reactor";
...
const PROFILE_ENV_KEYS = ["REACTOR_PROFILE"] as const;
 ...
 *
 * Exported so consumers of `@reactor/utils/dirs` (CLI bootstrap, tests,
...
export function normalizeProfileName(profile: string | undefined): string | undefined {
	...
	if (
		...
		throw new Error(
			`Invalid ReActor profile "${profile}". Profile names must match ${PROFILE_NAME_RE.source}, ` +
...
/**
 * Resolve the active profile from ReActor's canonical profile variable.
 */
export function resolveProfileEnv(reactor: string | undefined): string | undefined {
	return normalizeProfileName(reactor);
}

function getProfileFromEnv(): string | undefined {
	return resolveProfileEnv(process.env.REACTOR_PROFILE);
}

/**
 * Module-load profile resolution. Unlike {@link getProfileFromEnv}, an invalid
 * REACTOR_PROFILE value does NOT throw here — a bad env var must not
 ...
 * user still gets a clean "Invalid ReActor profile" message.
 */
...
/** Get the config directory name relative to home (e.g. ".reactor" or REACTOR_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.REACTOR_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".reactor/agent" or REACTOR_CONFIG_DIR + "/agent"). */
...
/**
 * Resolves and caches all reactor directory paths. On Linux, when XDG environment
 * variables are set, paths are redirected under $XDG_*_HOME/reactor/. A new
...
class DirResolver {
	...
	// With XDG on Linux, they point to $XDG_*_HOME/reactor/.
	...
	constructor(options: { agentDirOverride?: string; profile?: string } = {}) {
		...
		// resolves under $XDG_*_HOME/reactor once `reactor config init-xdg` has migrated
		...
		// profile could resolve to `~/.reactor/profiles/<name>` on first activation
		// (when no $XDG_*_HOME/reactor exists yet) and then silently move to
		// `$XDG_*_HOME/reactor/profiles/<name>` the moment the base appeared, orphaning
		...
		// XDG flattens the agent/ prefix: ~/.reactor/agent/sessions → $XDG_DATA_HOME/reactor/sessions
		this.#agentDirs = {
...
/**
 * Decide which `REACTOR_CODING_AGENT_DIR` value to capture as the pre-profile
 ...
 * mode honors a non-profile `REACTOR_CODING_AGENT_DIR` (see
...
/**
 * Snapshot of `REACTOR_CODING_AGENT_DIR` from before the first named-profile
 ...
 * with `REACTOR_CODING_AGENT_DIR=/custom` then `setProfile("work")` then
 * `setProfile(undefined)` would silently lose `/custom` and fall back to
 * `~/.reactor/agent`. Captured at module load — ignoring a profile-derived value
 ...
 * mode, `REACTOR_CODING_AGENT_DIR`) loaded from a profile/agent `.env` only reach
...
/** Get the config root directory (~/.reactor). */
...
export function setAgentDir(dir: string): void {
	...
	process.env.REACTOR_CODING_AGENT_DIR = dir;
...
/**
 * Test-only: reset the pre-profile `REACTOR_CODING_AGENT_DIR` snapshot to whatever
...
export function setProfile(profile: string | undefined): void {
	...
	if (next && !activeProfile) {
		...
		// current REACTOR_CODING_AGENT_DIR so a later reset can restore the user's
	...
	if (activeProfile) {
		...
		if (preProfileAgentDirEnv === undefined) {
			...
			process.env.REACTOR_CODING_AGENT_DIR = preProfileAgentDirEnv;
		}
...
/** Get the agent config directory (~/.reactor/agent). */
export function getAgentDir(): string {
	return dirs.agentDir;
}

/** Get the project-local config directory (.reactor). */
export function getProjectAgentDir(cwd: string = getProjectDir()): string {
	return path.join(cwd, CONFIG_DIR_NAME);
}

// =============================================================================
// Config-root subdirectories (~/.reactor/*)
// =============================================================================

/** Get the reports directory (~/.reactor/reports). */
export function getReportsDir(): string {
	return dirs.rootSubdir("reports", "state");
}

/** Get the logs directory (~/.reactor/logs). */
export function getLogsDir(): string {
	return dirs.rootSubdir("logs", "state");
}

/** Get this process's dated log path (~/.reactor/logs/reactor.YYYY-MM-DD.PID.log). */
...
/**
 * Get the plugins directory (~/.reactor/plugins or its XDG equivalent).
 *
...
/** Where npm installs packages (~/.reactor/plugins/node_modules). */
export function getPluginsNodeModules(home?: string): string {
	return path.join(getPluginsDir(home), "node_modules");
}

/** Plugin manifest (~/.reactor/plugins/package.json). */
export function getPluginsPackageJson(home?: string): string {
	return path.join(getPluginsDir(home), "package.json");
}

/** Plugin lock file (~/.reactor/plugins/reactor-plugins.lock.json). */
export function getPluginsLockfile(home?: string): string {
	return path.join(getPluginsDir(home), "reactor-plugins.lock.json");
}

/** Get the remote mount directory (~/.reactor/remote). */
 ...
 * (PR checkout, task isolation) and cleanup (`reactor worktree`). A relative value
 * would resolve against whatever cwd happened to launch `reactor`, so checkout and
 ...
 * isolation, and `reactor worktree` cleanup all read the same base). Driven by the
 * `worktree.base` setting in coding-agent; pass `undefined`/empty to clear and
 * fall back to `REACTOR_WORKTREE_DIR` or the `~/.reactor/wt` default.
 *
 ...
 * `REACTOR_WORKTREE_DIR` env var, then the {@link setWorktreesDir} override (the
 * `worktree.base` setting), then the `~/.reactor/wt` default. The env var and the
 * override are both `~`-expanded and must be absolute; a relative value is
 * ignored and resolution falls through.
 */
export function getWorktreesDir(): string {
	return (
		resolveWorktreeBase(process.env.REACTOR_WORKTREE_DIR) ?? worktreesDirOverride ?? dirs.rootSubdir("wt", "data")
	);
}

/** Get the SSH control socket directory (~/.reactor/ssh-control). */
export function getSshControlDir(): string {
	return dirs.rootSubdir("ssh-control", "state");
}

/** Get the remote host info directory (~/.reactor/remote-host). */
export function getRemoteHostDir(): string {
	return dirs.rootSubdir("remote-host", "data");
}

/** Get the managed Python venv directory (~/.reactor/python-env). */
export function getPythonEnvDir(): string {
	return dirs.rootSubdir("python-env", "data");
}

/** Get the shared Python gateway state directory (~/.reactor/agent/python-gateway; XDG default: $XDG_STATE_HOME/reactor/python-gateway). */
export function getPythonGatewayDir(): string {
	return dirs.agentSubdir(undefined, "python-gateway", "state");
}

/** Get the puppeteer sandbox directory (~/.reactor/puppeteer). */
 ...
 * (e.g. PR-checkout and task-isolation worktree dirs under `~/.reactor/wt/`).
...
/** Get the path to a single worktree directory (~/.reactor/wt/<segment>). */
export function getWorktreeDir(segment: string): string {
	return path.join(getWorktreesDir(), segment);
}

/** Get the GPU cache path (~/.reactor/gpu_cache.json). */
export function getGpuCachePath(): string {
	return dirs.rootSubdir("gpu_cache.json", "cache");
}

/**
 * Get the GitHub view cache database path (~/.reactor/cache/github-cache.db).
 * Honors the `REACTOR_GITHUB_CACHE_DB` env var when set so tests can isolate the
 * cache file without touching the rest of the config root.
 */
export function getGithubCacheDbPath(): string {
	const override = process.env.REACTOR_GITHUB_CACHE_DB;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "github-cache.db"), "cache");
}

/**
 * Get the encrypted auth-broker snapshot cache path (~/.reactor/cache/auth-broker-snapshot.enc).
 * Honors the `REACTOR_AUTH_BROKER_SNAPSHOT_CACHE` env var when set so tests and
 * operators can isolate or relocate the cache file.
 */
export function getAuthBrokerSnapshotCachePath(): string {
	const override = process.env.REACTOR_AUTH_BROKER_SNAPSHOT_CACHE;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "auth-broker-snapshot.enc"), "cache");
}

/** Get the local FastEmbed model cache directory (~/.reactor/cache/fastembed). */
export function getFastembedCacheDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed"), "cache");
}

/** Get the on-demand fastembed runtime install root (~/.reactor/cache/fastembed-runtime). */
export function getFastembedRuntimeDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed-runtime"), "cache");
}

/** Get the natives directory (~/.reactor/natives). */
export function getNativesDir(): string {
	return dirs.rootSubdir("natives", "cache");
}

/** Get the stats database path (~/.reactor/stats.db). */
export function getStatsDbPath(): string {
	return dirs.rootSubdir("stats.db", "data");
}

/** Get the autoresearch state directory (~/.reactor/autoresearch). */
export function getAutoresearchDir(): string {
	return dirs.rootSubdir("autoresearch", "state");
}

/** Get the per-project autoresearch state directory (~/.reactor/autoresearch/<encoded-project>). */
export function getAutoresearchProjectDir(encodedProject: string): string {
	return path.join(getAutoresearchDir(), encodedProject);
}

/** Get the per-project autoresearch SQLite database path (~/.reactor/autoresearch/<encoded-project>.db). */
export function getAutoresearchDbPath(encodedProject: string): string {
	return path.join(getAutoresearchDir(), `${encodedProject}.db`);
}

/** Get the per-run artifact directory (~/.reactor/autoresearch/<encoded-project>/runs/<runId>). */
...
// =============================================================================
// Agent subdirectories (~/.reactor/agent/*)
// =============================================================================
...
/** Get the last-seen-changelog-version marker file (~/.reactor/agent/last-changelog-version). */
...
/** Get the tiny title model cache directory (~/.reactor/agent/cache/tiny-models). */
...
/** Get the document conversion cache directory (~/.reactor/agent/cache/document-conversions; XDG default: $XDG_CACHE_HOME/reactor/cache/document-conversions). */
...
/**
 * Persistent per-install UUID stored at `~/.reactor/install-id`.
 *
 ...
 * `~/.reactor/agent/` does not regenerate it. Server-side dedup for grievance
 * pushes (and similar telemetry) keys on this id.
 *
 * Anchored to the base config root (`~/.reactor/install-id`) regardless of the
