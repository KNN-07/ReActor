...
import {
	...
	setAgentDir,
	setProfile,
} from "@reactor/utils/dirs";
import { Snowflake } from "@reactor/utils/snowflake";
...
describe("profile directories", () => {
	...
	beforeEach(async () => {
		...
		tempRoot = path.join(os.tmpdir(), "utils-profiles", Snowflake.next());
		configDir = `.reactor-profile-test-${Snowflake.next()}`;
		...
		// `REACTOR_CODING_AGENT_DIR` baseline matching the env we just configured.
	...
	afterEach(async () => {
		...
		if (originalConfigDir === undefined) {
			...
			process.env.REACTOR_CONFIG_DIR = originalConfigDir;
		}
	...
	it("keeps XDG-backed named profile state under profile-specific roots", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "reactor", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "reactor", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "reactor", "profiles", "work"), { recursive: true });

		setProfile("work");

		expect(getAgentDbPath()).toBe(path.join(process.env.XDG_DATA_HOME, "reactor", "profiles", "work", "agent.db"));
		expect(getSessionsDir()).toBe(path.join(process.env.XDG_DATA_HOME, "reactor", "profiles", "work", "sessions"));
		expect(getPythonGatewayDir()).toBe(
			path.join(process.env.XDG_STATE_HOME, "reactor", "profiles", "work", "python-gateway"),
		);
	});
	...
	it("does not silently switch a named profile to XDG once the base app dir appears", async () => {
		...
		// Fresh install: XDG vars are set (typical Linux) but no $XDG/reactor exists yet.
		...
		// Later, the base XDG app dir materializes (e.g. via `reactor config init-xdg`
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "reactor"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "reactor"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "reactor"), { recursive: true });
	...
	it("rejects path-like profile names", () => {
		expect(() => setProfile("../work")).toThrow("Invalid ReActor profile");
		expect(() => setProfile("work/team")).toThrow("Invalid ReActor profile");
	});
	...
	it("restores the pre-profile REACTOR_CODING_AGENT_DIR override on reset", () => {
		const customAgentDir = path.join(tempRoot, "custom-agent");
		setAgentDir(customAgentDir);
		expect(getAgentDir()).toBe(customAgentDir);
		expect(process.env.REACTOR_CODING_AGENT_DIR).toBe(customAgentDir);
	...
	it("clears REACTOR_CODING_AGENT_DIR on reset when nothing was set originally", () => {
		delete process.env.REACTOR_CODING_AGENT_DIR;
		...
		setProfile("work");
		setProfile(undefined);
		expect(process.env.REACTOR_CODING_AGENT_DIR).toBeUndefined();
	});
	...
	it("does not restore a profile-derived agent dir as the default baseline", () => {
		// Reproduces a child process that inherited REACTOR_PROFILE=work plus the
		// profile-derived REACTOR_CODING_AGENT_DIR that setProfile propagates to
		...
		expect(process.env.REACTOR_CODING_AGENT_DIR).toBe(workAgentDir);

		// Re-snapshot exactly as module load would, now that REACTOR_PROFILE and the
		// profile-derived REACTOR_CODING_AGENT_DIR are present in the environment.
		__resetProfileSnapshotForTests();
...
describe("profile env + name validation", () => {
	it("honors REACTOR_PROFILE validation", () => {
		// REACTOR_PROFILE is the canonical variable.
	...
	it("rejects uppercase profile names so isolation is filesystem-independent", () => {
		...
		expect(() => normalizeProfileName("WORK")).toThrow("Invalid ReActor profile");
		expect(() => normalizeProfileName("Work")).toThrow("Invalid ReActor profile");
...
describe("dirs module import behavior", () => {
	it("does not scrub inherited macOS malloc logging env variables on import", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "utils-dirs-import-"));
		try {
	...
	it("exposes worker-host without loading agent env", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "utils-worker-host-import-"));
		try {
			const workerHostUrl = import.meta.resolve("@reactor/utils/worker-host");
			const agentDir = path.join(root, "agent");
			await fs.mkdir(agentDir, { recursive: true });
			await Bun.write(path.join(agentDir, ".env"), "REACTOR_WORKER_HOST_PROBE=from-agent-env\n");
			const probePath = path.join(root, "probe.ts");
			await Bun.write(
				probePath,
				[
					...
					"process.stdout.write(JSON.stringify({",
					"	envProbe: process.env.REACTOR_WORKER_HOST_PROBE ?? null,",
			...
			const childEnv: Record<string, string | undefined> = {
				...process.env,
				REACTOR_CODING_AGENT_DIR: agentDir,
			};
			delete childEnv.REACTOR_WORKER_HOST_PROBE;
			const proc = Bun.spawn([process.execPath, probePath], {
				stdout: "pipe",
				stderr: "pipe",
				env: childEnv,
	...
	it("ignores an inherited profile agent dir when REACTOR_PROFILE selects default", async () => {
		...
		try {
			...
			for (const reactorProfile of ["", "default"]) {
				const probePath = path.join(root, `default-profile-${reactorProfile || "empty"}.ts`);
				await Bun.write(
					probePath,
					[
	...
	it("honors XDG dir keys from a profile .env applied after the resolver froze", async () => {
		if (process.platform === "win32") return;
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "utils-profile-env-xdg-"));
		const homeDir = path.join(root, "home");
		const xdgStateRoot = path.join(root, "xdg-state");
		const profileConfigDir = `.reactor-env-xdg-${Snowflake.next()}`;
		try {
			...
			// Named profiles only adopt XDG when their own XDG path already exists.
			const xdgProfileRoot = path.join(xdgStateRoot, "reactor", "profiles", "work");
			...
			const childEnv: Record<string, string | undefined> = {
				...process.env,
				HOME: homeDir,
				REACTOR_CONFIG_DIR: profileConfigDir,
