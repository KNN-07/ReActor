...
describe("wrapFetchForExtraCa", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-extra-ca-"));
	});
...
describe("withExtraCaFetch", () => {
	...
	beforeEach(async () => {
		__resetExtraCaCache();
		originalEnv = Bun.env.NODE_EXTRA_CA_CERTS;
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "reactor-extra-ca-opts-"));
	});
