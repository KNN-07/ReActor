...
import { getAgentDir, getConfigDirName, getPythonGatewayDir, setAgentDir } from "@reactor/utils/dirs";
...
describe("python gateway directory", () => {
	...
	beforeEach(async () => {
		...
		tempRoot = path.join(os.tmpdir(), "utils-python-gateway", Snowflake.next());
	...
	it("uses XDG state for the default agent profile", async () => {
		...
		process.env.REACTOR_CONFIG_DIR = `.reactor-test-${Snowflake.next()}`;
		...
		expect(getPythonGatewayDir()).toBe(path.join(process.env.XDG_STATE_HOME, "reactor", "python-gateway"));
	});
