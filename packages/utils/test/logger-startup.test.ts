...
/** Run `fn` with REACTOR_DEBUG_STARTUP set, capturing `[startup]` stderr markers. */
function withMarkerCapture<T>(fn: () => T): { result: T; markers: string[] } {
	const prev = process.env.REACTOR_DEBUG_STARTUP;
	process.env.REACTOR_DEBUG_STARTUP = "1";
	...
	try {
		...
		if (prev === undefined) {
			delete process.env.REACTOR_DEBUG_STARTUP;
		} else {
			process.env.REACTOR_DEBUG_STARTUP = prev;
		}
	}
}

describe("REACTOR_DEBUG_STARTUP streaming markers", () => {
	// Contract: with REACTOR_DEBUG_STARTUP set, every logger.time phase leaves a
	...
	// must work without startTiming() (markers are independent of REACTOR_TIMING).
	...
	it("emits nothing when REACTOR_DEBUG_STARTUP is unset", () => {
		const prev = process.env.REACTOR_DEBUG_STARTUP;
		delete process.env.REACTOR_DEBUG_STARTUP;
		const writes: string[] = [];
		const writeSpy = spyOn(fs, "writeSync").mockImplementation(((_fd: number, data: string) => {
		...
		try {
			...
			writeSpy.mockRestore();
			if (prev !== undefined) process.env.REACTOR_DEBUG_STARTUP = prev;
		}
