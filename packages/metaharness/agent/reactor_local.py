...
this runs the working tree at `/work/reactor`. Install modes (`REACTOR_BENCH_INSTALL`):
  ...
  * binary (`--binary`): a self-contained compiled reactor binary is uploaded.

Auth never enters the container: a generated `~/.reactor/agent/models.yml` routes the
configured providers' `baseUrl` at the host's pm2 auth-gateway (default
`http://host.docker.internal:4000`, `transport: reactor-native`), so the gateway
...
(see `REACTOR_BENCH_*` below); the agent reads them from `os.environ` directly.

Selected via `harbor run --agent-import-path reactor_local:ReactorLocal` with the
...
def _patch_harbor_cleanup_cancellation() -> None:
    ...
    if getattr(Trial, "_reactor_cleanup_cancellation_patch", False):
        return
    ...
    Trial._reactor_cleanup_cancellation_patch = True
...
def _patch_apple_container_dns() -> None:
    ...
    sets REACTOR_BENCH_CONTAINER_DNS for apple-container jobs; absent, no-op.
    """
    dns = os.environ.get("REACTOR_BENCH_CONTAINER_DNS")
    if not dns:
        return
    from harbor.environments.apple_container import AppleContainerEnvironment

    if getattr(AppleContainerEnvironment, "_reactor_dns_patch", False):
        return
    ...
    AppleContainerEnvironment._reactor_dns_patch = True
...
_TARBALL_DST = "/tmp/reactor-local.tgz"
_MODELS_DST = "/tmp/reactor-models.yml"
_CONFIG_DST = "/tmp/reactor-config.yml"
_OUTPUT_FILENAME = "reactor.txt"
...
class ReactorLocal(BaseInstalledAgent):
    ...
    def __init__(self, *args, **kwargs) -> None:  # noqa: D401 - thin wrapper
        ...
        self._install_mode = _env("REACTOR_BENCH_INSTALL", "source")
        self._tarball = _env("REACTOR_BENCH_TARBALL")
        self._pkg_version = _env("REACTOR_BENCH_VERSION", "latest")
        self._models_yaml_path = _env("REACTOR_BENCH_MODELS_YAML")
        self._gateway_url = _env(
            "REACTOR_BENCH_GATEWAY_URL", "http://host.docker.internal:4000"
        )
        self._gateway_token = _env("REACTOR_BENCH_GATEWAY_TOKEN", "no-auth-dummy")
        self._gateway_providers = [
            p.strip()
            for p in _env(
                "REACTOR_BENCH_GATEWAY_PROVIDERS", "anthropic,openai-codex"
            ).split(",")
            if p.strip()
        ]
        self._thinking = _env("REACTOR_BENCH_THINKING")
        self._auto_approve = _truthy(_env("REACTOR_BENCH_AUTO_APPROVE", "1"))
        # Extra CLI args forwarded verbatim to the in-container reactor invocation,
        # JSON-array-encoded by the runner (REACTOR_BENCH_AGENT_ARGS) so multi-word
        # values survive without a second layer of shell quoting.
        self._agent_args = self._parse_agent_args()
        self._bun_version = _env("REACTOR_BENCH_BUN_VERSION", "1.3.14")
        self._gateway_on = _env("REACTOR_BENCH_GATEWAY", "1") != "0"
        ...
        self._web_search = _truthy(_env("REACTOR_BENCH_WEB_SEARCH", "0"))
        # Extra env (REACTOR_* dialect knobs, explicit --env) the runner forwards into
        # the in-container reactor run, JSON-encoded in REACTOR_BENCH_FORWARD_ENV.
        self._forward_env = self._parse_forward_env()
        # Source-mount paths (defaults must match the runner's compose overlay).
        self._source_dir = _env("REACTOR_BENCH_SOURCE_DIR", "/opt/reactor/src")
        self._source_bun = _env("REACTOR_BENCH_SOURCE_BUN", "/opt/reactor/bin/bun")
        self._source_arch = _env("REACTOR_BENCH_SOURCE_ARCH")
        # Resolved during install(); reused by version + run commands.
        self._home = "/root"
        self._bun = "/root/.bun/bin/bun"
        self._cli = "/root/.reactor-bench/app/dist/cli.js"
        self._binary_arm64 = _env("REACTOR_BENCH_BINARY_ARM64")
        self._binary_x64 = _env("REACTOR_BENCH_BINARY_X64")
    ...
    def _wrap(self, command: str) -> str:
        ...
        reactor spawns Bun worker subprocesses at runtime, so `bun` must resolve on
    ...
    async def install(self, environment: BaseEnvironment) -> None:
        ...
        if self._binary:
            ...
            await self.exec_as_root(
                ...
                command=(
                    ...
                    "  echo 'ERROR: Alpine/musl base image; @reactor/natives ships no musl prebuilt' >&2; exit 3; "
        ...
        # 3) Auth + model config under $HOME/.reactor/agent.
        if self._gateway_on:
    ...
    async def _install_source(self, environment: BaseEnvironment) -> str:
        """Verify the read-only repo + linux deps mounts and run reactor from TS source.
        ...
        await self.exec_as_agent(
            ...
            command=(
                "set -e; "
                f"test -x {q(self._source_bun)} || {{ echo 'reactor source mode: bun mount missing' >&2; exit 5; }}; "
                f"test -f {q(cli)} || {{ echo 'reactor source mode: repo mount missing' >&2; exit 5; }}; "
                f"test -d {q(self._source_dir + '/node_modules/@ReActor')} || "
                "{ echo 'reactor source mode: linux deps mount missing' >&2; exit 5; }; "
    ...
    async def _install_local(self, environment: BaseEnvironment) -> str:
        if not self._tarball:
            raise RuntimeError(
                "REACTOR_BENCH_INSTALL=local requires REACTOR_BENCH_TARBALL (host tarball path)"
            )
        await environment.upload_file(self._tarball, _TARBALL_DST)
        app = f"{self._home}/.reactor-bench/app"
        await self.exec_as_agent(
            environment,
            command=self._wrap(
                "set -e; "
                ...
                'echo "pinning native @reactor/natives-linux-$na@$ver"; '
                'bun add --production "@reactor/natives-linux-$na@$ver"'
            ),
            timeout_sec=900,
        )
    ...
    async def _install_binary(self, environment: BaseEnvironment) -> str:
        """Probe container arch, upload only the matching self-contained reactor binary."""
        arch = (
            await self.exec_as_agent(environment, command="uname -m")
        ).stdout.strip()
        ...
        if not hostbin:
            raise RuntimeError(
                f"binary mode: no reactor binary provided for container arch {arch}"
            )
        app_dir = f"{self._home}/.reactor-bench"
        dst = f"{app_dir}/reactor"
        staging = "/tmp/reactor-bin"
        await self.exec_as_agent(
    ...
    async def _install_published(self, environment: BaseEnvironment) -> str:
        app = f"{self._home}/.reactor-bench/app"
        spec = f"@reactor/coding-agent@{self._pkg_version}"
        await self.exec_as_agent(
            environment,
            command=self._wrap(
                "set -e; "
        ...
        return f"{app}/node_modules/@reactor/coding-agent/dist/cli.js"
    ...
    async def _write_models_yaml(self, environment: BaseEnvironment) -> None:
        if self._models_yaml_path and os.path.isfile(self._models_yaml_path):
            ...
            staged = _MODELS_DST
            heredoc = (
                f"cat > {_MODELS_DST} <<'REACTOR_MODELS_EOF'\n{content}\nREACTOR_MODELS_EOF"
            )
            await self.exec_as_agent(environment, command=heredoc)
        await self.exec_as_agent(
            environment,
            command=(
                f'mkdir -p "$HOME/.reactor/agent"; '
                f'cp {shlex.quote(staged)} "$HOME/.reactor/agent/models.yml"'
            ),
        )
    ...
    async def _write_config(self, environment: BaseEnvironment) -> None:
        """Write $HOME/.reactor/agent/config.yml: the web_search toggle.
        ...
        content = "\n".join(lines)
        heredoc = f"cat > {_CONFIG_DST} <<'REACTOR_CONFIG_EOF'\n{content}\nREACTOR_CONFIG_EOF"
        await self.exec_as_agent(environment, command=heredoc)
        await self.exec_as_agent(
            environment,
            command=(
                f'mkdir -p "$HOME/.reactor/agent"; '
                f'cp {shlex.quote(_CONFIG_DST)} "$HOME/.reactor/agent/config.yml"'
            ),
        )

    @staticmethod
    def _parse_forward_env() -> dict[str, str]:
        """Extra run-time env from the runner (REACTOR_BENCH_FORWARD_ENV = JSON object)."""
        raw = _env("REACTOR_BENCH_FORWARD_ENV")
        if not raw:
            return {}
        try:
    ...
    def _parse_agent_args() -> list[str]:
        """Extra CLI args from the runner (REACTOR_BENCH_AGENT_ARGS = JSON array)."""
        raw = _env("REACTOR_BENCH_AGENT_ARGS")
        if not raw:
            return []
        try:
            parsed = json.loads(raw)
    ...
    async def run(
        ...
        # markdown bullet, as in pytorch-model-recovery). Without this, reactor parses
        ...
        # Exec env for the reactor run. Direct-auth (no-gateway) mode contributes the
        # selected providers' keys (via exec env, never argv); forwarded REACTOR_* /
    ...
    def _sum_main(self, path: Path, acc: "_Usage") -> None:
        """Sum assistant `message_end` usage from reactor's stdout JSONL.
