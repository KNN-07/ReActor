...
/**
 * Harbor benchmark runner for the local `reactor` build.
 *
...
const AGENT_IMPORT_PATH = "reactor_local:ReactorLocal";

/** Container-side mount points for `--install source` (must match reactor_local.py defaults). */
const SOURCE_SRC_MOUNT = "/opt/reactor/src";
const SOURCE_BIN_MOUNT = "/opt/reactor/bin";
...
/**
 * Resolver injected into Apple Container runs (REACTOR_BENCH_CONTAINER_DNS overrides).
 ...
 */
const CONTAINER_DNS = process.env.REACTOR_BENCH_CONTAINER_DNS || "1.1.1.1";
...
export interface Config {
	...
	/** Extra args forwarded verbatim to the in-container reactor CLI invocation (repeatable). */
	agentArgs: string[];
...
const HELP = `metaharness runner (local reactor)
...
Model / agent:
  -m, --model <provider/model>   Model (repeatable). Default anthropic/claude-sonnet-4-6
      ...
      --tarball <path>           Reuse a prebuilt reactor tarball (implies --install local, --no-build)
      --no-build                 Skip packing; reuse newest tarball in bench dir (--install local)
      --agent-arg <arg>          Extra arg forwarded verbatim to the in-container reactor CLI (repeatable)
      --env <KEY[=VALUE]>        Forward env into reactor container (repeatable).
                                 KEY alone forwards host value; host REACTOR_* auto-forwarded.
...
Gateway (auth, no keys in container):
      ...
      --web-search               Enable reactor web_search (off by default; can't auth via gateway)
 ...
 * `agent/reactor.txt` JSONL. Only bytes appended since the previous call are read
 ...
 */
function probeTrialCost(reactorLogPath: string): CostProbe | null {
	let size: number;
	try {
		size = fs.statSync(reactorLogPath).size;
	} catch {
		return costProbes.get(reactorLogPath) ?? null;
	}
	let probe = costProbes.get(reactorLogPath);
	if (!probe || size < probe.offset) {
		...
		costProbes.set(reactorLogPath, probe);
	}
	if (size === probe.offset) return probe;
	let fd: number;
	try {
		fd = fs.openSync(reactorLogPath, "r");
	} catch {
		return probe;
	}
	try {
...
function parseTrial(dir: string, name: string): Trial | null {
	...
	if (!fs.existsSync(resultPath)) {
		...
		// Realtime cost from the live agent reactor.txt log, parsed incrementally.
		const probe = probeTrialCost(path.join(dir, "agent", "reactor.txt"));
	...
	costProbes.delete(path.join(dir, "agent", "reactor.txt"));
...
function buildTarball(benchDir: string): string {
	process.stdout.write(dim("packing local reactor (bun pm pack)…\n"));
...
/** Linux deps tree + mount plan for running reactor straight from the mounted repo. */
...
function writeComposeOverlay(benchDir: string, cfg: Config, source: SourceMount | null): string | null {
	...
	const file = path.join(benchDir, "reactor-compose-overlay.yaml");
...
function writeModelsYaml(benchDir: string, cfg: Config): string {
	...
	for (const p of providers) {
		...
		lines.push("    transport: reactor-native");
...
function buildHarborArgs(
	...
	if (cfg.agent === "reactor") {
		// Config + secrets travel via env (REACTOR_BENCH_*); the agent reads os.environ.
...
/**
 * Env vars injected into the in-container reactor run: every host `REACTOR_*` knob (minus
...
export function collectForwardEnv(cfg: Config): Record<string, string> {
	...
	for (const [k, v] of Object.entries(process.env)) {
		if (v === undefined || !k.startsWith("REACTOR_") || FORWARD_ENV_DENYLIST.has(k)) continue;
		out[k] = v;
	}
...
export function buildHarborEnv(
	...
	// Drop any stale REACTOR_BENCH_FORWARD_ENV inherited from the caller's shell before
	// the agent-type early return, so it never leaks (incl. into the dry-run dump).
	delete env.REACTOR_BENCH_FORWARD_ENV;
	if (cfg.agent !== "reactor") return env;
	const prepend = (k: string, v: string): void => {
		env[k] = env[k] ? `${v}:${env[k]}` : v;
	};
	prepend("PYTHONPATH", AGENT_DIR);
	env.REACTOR_BENCH_INSTALL = cfg.install;
	env.REACTOR_BENCH_VERSION = cfg.version ?? version;
	if (tarball) env.REACTOR_BENCH_TARBALL = tarball;
	if (source) {
		env.REACTOR_BENCH_SOURCE_DIR = SOURCE_SRC_MOUNT;
		env.REACTOR_BENCH_SOURCE_BUN = `${SOURCE_BIN_MOUNT}/bun`;
		env.REACTOR_BENCH_SOURCE_ARCH = source.arch;
	}
	if (cfg.binaryArm64) env.REACTOR_BENCH_BINARY_ARM64 = cfg.binaryArm64;
	if (cfg.binaryX64) env.REACTOR_BENCH_BINARY_X64 = cfg.binaryX64;
	if (cfg.thinking) env.REACTOR_BENCH_THINKING = cfg.thinking;
	if (cfg.agentArgs.length > 0) env.REACTOR_BENCH_AGENT_ARGS = JSON.stringify(cfg.agentArgs);
	if (cfg.webSearch) env.REACTOR_BENCH_WEB_SEARCH = "1";
	env.REACTOR_BENCH_GATEWAY = cfg.gateway ? "1" : "0";
	if (cfg.gateway) {
		env.REACTOR_BENCH_MODELS_YAML = modelsYaml;
		env.REACTOR_BENCH_GATEWAY_URL = cfg.gatewayUrl;
		env.REACTOR_BENCH_GATEWAY_TOKEN = cfg.gatewayToken;
		env.REACTOR_BENCH_GATEWAY_PROVIDERS = deriveProviders(cfg).join(",");
	}
	if (cfg.envType === "apple-container") env.REACTOR_BENCH_CONTAINER_DNS = CONTAINER_DNS;
	const forward = collectForwardEnv(cfg);
	if (Object.keys(forward).length > 0) env.REACTOR_BENCH_FORWARD_ENV = JSON.stringify(forward);
	return env;
}
...
async function runBenchmark(cfg: Config): Promise<BenchmarkRun> {
	...
	if (cfg.agent === "reactor" && cfg.envType === "docker" && !which("docker")) {
	...
	if (cfg.agent === "reactor" && cfg.install === "local" && !cfg.binaryArm64 && !cfg.binaryX64) {
		if (tarball) {
	...
	if (cfg.agent === "reactor" && cfg.install === "source" && !cfg.binaryArm64 && !cfg.binaryX64) {
		source = prepareSourceDeps(cfg);
	}

	// models.yml (gateway)
	let modelsYaml = "";
	if (cfg.agent === "reactor" && cfg.gateway) {
		modelsYaml = writeModelsYaml(benchDir, cfg);
		if (!gatewayHealthOk(cfg.gatewayUrl)) {
			process.stderr.write(
				yellow(
					`warning: gateway ${cfg.gatewayUrl} health check failed (continuing). Is the pm2 'reactor-auth-gateway' running?\n`,
				),
			);
		}
	}
	...
	if (cfg.dryRun) {
		...
		process.stdout.write(bold("reactor env:\n"));
		for (const key in harborEnv) {
			if (key === "REACTOR_BENCH_FORWARD_ENV") continue;
			if (key.startsWith("REACTOR_BENCH_") || key === "PYTHONPATH")
				process.stdout.write(`  ${key}=${harborEnv[key]}\n`);
		}
		if (harborEnv.REACTOR_BENCH_FORWARD_ENV) {
			const parsedForwardEnv: unknown = JSON.parse(harborEnv.REACTOR_BENCH_FORWARD_ENV);
			if (parsedForwardEnv !== null && typeof parsedForwardEnv === "object" && !Array.isArray(parsedForwardEnv)) {
				...
				process.stdout.write(`  REACTOR_BENCH_FORWARD_ENV=${keys.join(",")} (values hidden)\n`);
			}
		}
		process.stdout.write(`\njob dir: ${jobDir}\nbench dir: ${benchDir}\n`);
