...
1. **Local reactor, not npm.** By default the runner bind-mounts the repo
   read-only into each task container (`--install source`) and runs reactor
   ...
   `/opt/reactor/bin` — so trial setup needs zero outbound network. Alternatives:
   `--install local` (pack a tarball per run) or `--binary` (prebuilt
   `dist/reactor-linux-*` self-contained binaries).
...
| `--env <KEY[=VALUE]>` | — | Forward env into the reactor container (repeatable); `KEY` alone forwards the host value |
| `--binary <path>` | — | Prebuilt reactor binary (repeat for arm64+x64) |
| `--install <source\|local\|published>` | `source` | `source` = repo bind-mount, `local` = tarball pack, `published` = npm `@reactor/coding-agent` |
...
arc; ~$0.001 per report). API keys resolve through reactor's auth storage.
...
- **`--install source` reflects local TS changes** with no rebuild, but Rust
  natives load from the in-tree `packages/natives/native/reactor_natives.linux-*.node`
