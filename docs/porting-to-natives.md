# Porting to reactor-natives (N-API) — Field Notes

This is a practical guide for moving hot paths into `crates/reactor-natives` and wiring them through the generated native package entrypoint. It exists to avoid the same failures happening twice.
...
`@reactor/natives` no longer has a `packages/natives/src/<module>` TypeScript wrapper layer. The package root points at generated native artifacts:
...
Consumers import directly from `@reactor/natives`. The generated declarations and explicit ESM exports are produced during `bun --cwd=packages/natives run build`.

## Anatomy of a native export

**Rust side:**

- Implementation lives in `crates/reactor-natives/src/<module>.rs`.
- If you add a new module, register it in `crates/reactor-natives/src/lib.rs`.
...
- `packages/natives/package.json` exposes only the package root (`@reactor/natives`) as the import surface. At publish time the binaries are split out: the core ships the loader only (no `.node`), and each platform's `.node` is published as an optional-dependency leaf package `@reactor/natives-<tag>` (`scripts/ci-release-publish.ts` + `packages/natives/scripts/gen-npm-packages.ts`). This is transparent to importers — you still `import` from `@reactor/natives`.
...
- If it is a new module, add it to `crates/reactor-natives/src/lib.rs`.
...
- Import the new export directly from `@reactor/natives`.
...
- `modern`: `reactor_natives.<tag>-modern.node`, then `...-baseline.node`, then `reactor_natives.<tag>.node`.
- `baseline`: `reactor_natives.<tag>-baseline.node`, then `reactor_natives.<tag>.node`.

Non-x64 uses `reactor_natives.<tag>.node`.
...
```bash
rm packages/natives/native/reactor_natives.<platform>-<arch>.node
rm packages/natives/native/reactor_natives.<platform>-<arch>-modern.node
rm packages/natives/native/reactor_natives.<platform>-<arch>-baseline.node
bun --cwd=packages/natives run build
```

For compiled binaries or Windows staging, delete the versioned addon cache shown in the loader error (normally under `~/.reactor/natives/<version>` unless `$XDG_DATA_HOME/reactor` is used).
...
```bash
bun -e 'import { createRequire } from "node:module"; const require = createRequire(import.meta.url); const mod = require(process.argv[2]); console.log(Object.keys(mod).sort())' -- /path/from/loader/error/reactor_natives.<tag>[-variant].node
```
