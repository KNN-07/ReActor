...
description: Use when creating a new reactor marketplace. Covers marketplace.json schema, source types, install commands, and publishing.
---

# Authoring Marketplaces

A marketplace is a Git repository (or local directory) that contains a catalog file at either `.reactor-plugin/marketplace.json` (preferred for reactor-specific catalogs) or `.claude-plugin/marketplace.json` (Claude Code-compatible; used as the fallback). Anyone can author one. Users add it with `/marketplace add owner/repo` and then install individual plugins from it.
...
The catalog file lives at either `.reactor-plugin/marketplace.json` or `.claude-plugin/marketplace.json` in the repository root. reactor prefers the `.reactor-plugin/` path and falls back to the Claude path; a repository may publish both to expose tool-specific catalogs from a single source tree.
...
```json
{
  ...
  "metadata": {
    "description": "Official Acme plugins for ReActor"
  },
  "plugins": [
    {
      "name": "acme-linter",
    ...
    },
    {
      ...
      "source": {
        "source": "github",
        "repo": "acme-corp/reactor-deploy-plugin",
        "ref": "main"
      }
    }
  ]
...
> Note: extension modules declared via `package.json` `reactor.extensions` are **not** loaded from marketplace installs — that mechanism only applies to npm-installed or `reactor plugin link`ed plugins. Ship marketplace plugin behavior through the conventional directories above.
...
```
reactor plugin marketplace add owner/repo
reactor plugin install name@marketplace-name
```

Scope behavior:

- **user** (default) — installed in `~/.reactor/plugins/installed_plugins.json`, available in all projects
- **project** — installed in `<project>/.reactor/plugins/installed_plugins.json`, available only in that project
...
1. Create `marketplace.json` at `.reactor-plugin/marketplace.json` (reactor-only) or `.claude-plugin/marketplace.json` (shared with Claude Code) in a new Git repo.
