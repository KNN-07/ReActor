...
An `ReActor` extension that demonstrates `tool_call` blocking. It intercepts `bash` tool calls and returns `{ block: true, reason: "..." }` when the command contains `rm -rf /` with normal whitespace, preventing the tool from executing.
...
```
cp -r . ~/.reactor/agent/extensions/safety-hook
```

Restart `reactor`. The hook is active for all sessions.

Or load once:

```
reactor --extension ./safety-hook
```
