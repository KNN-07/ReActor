...
description: Use when creating a new reactor extension. Covers ExtensionAPI, factory signature, tool/command/event registration, and local-dev testing.
---

# Authoring Extensions

Extensions are the primary way to add capabilities to `ReActor`. A single extension module can register tools the LLM can call, slash commands users can invoke, and event handlers that run throughout the session lifecycle — all from one TypeScript file.
...
That is a working extension. Drop it into `~/.reactor/agent/extensions/hello.ts` and restart reactor to see the notification.
...
reactor loads extension modules from these sources:

1. Native `.reactor` locations discovered through the capability system:
   - `<cwd>/.reactor/extensions/`
   - `~/.reactor/agent/extensions/`
   - legacy extension paths listed in `.reactor/settings.json#extensions` or `~/.reactor/agent/settings.json#extensions`
2. Installed plugins under `~/.reactor/plugins/node_modules` (`reactor plugin install` npm/git specs, or `reactor plugin link`) via their `reactor.extensions`/`pi.extensions` manifests. Marketplace cache installs do not feed extension modules — they surface skills/commands/hooks/tools/MCP only.
3. Explicit configured paths passed by the CLI (`reactor --extension ./my-ext.ts`, also `-e`; `--hook` is treated as an alias) and by the `extensions:` setting in config.

The runtime de-duplicates by resolved absolute path — first seen wins.

When a path points to a directory, reactor resolves the entry point in this order:

1. `package.json` with `reactor.extensions` (or legacy `pi.extensions`) field
2. `index.ts`
3. `index.js`

When scanning an `extensions/` directory, reactor also loads direct `*.ts`/`*.js` files and one-level subdirectories that have `index.ts`, `index.js`, or a manifest.

Extension packages can also bundle sibling capability directories. When a package is loaded through `extensions:` or `--extension`/`-e`, the `reactor-plugins` provider discovers its `skills/`, `hooks/pre|post/`, `tools/`, `commands/`, `rules/`, `prompts/`, and `.mcp.json`.

## package.json manifest

To package an extension as an installable plugin, add an `reactor` field to `package.json`:

```json
{
  "name": "my-reactor-extension",
  "reactor": {
...
| Legacy hook module already exists | **Hook** (`HookAPI` from `@reactor/coding-agent/extensibility/hooks`) |
...
reactor writes structured logs to a rotating file under `~/.reactor/logs/` (debug level is always on; nothing is written to the console, which would corrupt the TUI). Tail today's log to see extension load diagnostics:

```
tail -f ~/.reactor/logs/reactor.$(date +%F).log
```
