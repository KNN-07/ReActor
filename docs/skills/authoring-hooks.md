...
description: Use when creating a new reactor hook. Covers HookAPI, event catalog, blocking/overriding tool calls, and context modification.
---
...
```ts
import type { HookAPI } from "@reactor/coding-agent/extensibility/hooks";

export default function myHook(reactor: HookAPI): void {
  reactor.on("tool_call", async (event, ctx) => {
...
```ts
reactor.on("tool_result", async (event, ctx) => {
...
```ts
reactor.on("context", async (event, ctx) => {
...
export default function rmRfBlocker(reactor: HookAPI): void {
  reactor.on("tool_call", async (event, ctx) => {
...
export default function apiKeyRedactor(reactor: HookAPI): void {
  reactor.on("tool_result", async (event) => {
...
export default function contextFilter(reactor: HookAPI): void {
  reactor.on("context", async (event) => {
