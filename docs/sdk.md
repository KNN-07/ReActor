...
The SDK is the in-process integration surface for `@reactor/coding-agent`.
...
```bash
bun add @reactor/coding-agent
```

## Entry points

`@reactor/coding-agent` exports the SDK APIs from the package root (and also via `@reactor/coding-agent/sdk`).
...
```ts
import { createAgentSession } from "@reactor/coding-agent";
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.reactor/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@reactor/coding-agent";
...
```ts
import { SessionManager } from "@reactor/coding-agent";
