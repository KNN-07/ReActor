...
1. Capability providers (`toolCapability`), including:
   - Native ReActor config (`~/.reactor/agent/tools`, `.reactor/tools`)
   - Claude config (`~/.claude/tools`, `.claude/tools`)
   - Codex config (`~/.codex/tools`, `.codex/tools`)
   - Claude marketplace plugin cache provider
2. Installed plugin manifests (`~/.reactor/plugins/node_modules/*` via plugin loader)
...
```ts
import type { CustomToolFactory } from "@reactor/coding-agent";
...
- `pi`: injected `@reactor/coding-agent` exports
