...
`@reactor/natives` is a two-layer package around an ESM loader:
...
There is no current `packages/natives/src` TypeScript wrapper layer. Consumers import functions/classes/enums directly from `@reactor/natives`; the type contract is the generated `native/index.d.ts` plus the explicit named exports generated into `native/index.js` by `scripts/gen-enums.ts`.
...
- Default: `reactor_natives.<platform>-<arch>.node`
- x64 variant: `reactor_natives.<platform>-<arch>-modern.node` or `...-baseline.node`
...
2. Check legacy compiled-binary location:
   - Windows: `%LOCALAPPDATA%/reactor` (fallback `%USERPROFILE%/AppData/Local/reactor`)
   - non-Windows: `~/.local/bin`
3. Fall back to packaged `native/` and executable directory candidates.

`getNativesDir()` uses `$XDG_DATA_HOME/reactor/natives` when `$XDG_DATA_HOME/reactor` exists; otherwise it uses `~/.reactor/natives`.
...
`crates/reactor-natives/src/lib.rs` declares exported module ownership:
...
- `iso`
- `keys`
- `language` (re-exported from `reactor_ast`)
- `power`
- `prof`
- `ps`
- `pty`
...
- **Rust ownership (`crates/reactor-natives/src`)**
...
- **Platform leaf package**: Per-platform npm package `@reactor/natives-<tag>` that carries one platform's prebuilt `.node`. The core depends on every leaf via `optionalDependencies`; the package manager installs only the host-matching one (`os`/`cpu`).
