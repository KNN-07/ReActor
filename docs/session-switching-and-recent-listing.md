...
- `~/.reactor/agent/sessions/<dir-encoded>/*.jsonl` (home-relative `-<rel>` names, `-tmp-<rel>` for temp paths, legacy `--<abs>--` otherwise)
...
1. Read terminal-scoped breadcrumb (`~/.reactor/agent/terminal-sessions/<terminal-id>`)
