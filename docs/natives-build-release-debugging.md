...
This runbook describes how `@reactor/natives` produces `.node` addons, generated declarations, and compiled-binary embedded payloads, and how to debug loader/build failures.
...
- `crates/reactor-natives/Cargo.toml`
...
- `bun scripts/gen-npm-packages.ts` (`gen:npm`) → generate per-platform npm leaf packages (`@reactor/natives-<platform>-<arch>`, installed as optional dependencies of the core package) under `npm/` from built addon files.
...
- `napi build`
- `--manifest-path crates/reactor-natives/Cargo.toml`
...
`crates/reactor-natives/Cargo.toml` declares `crate-type = ["cdylib"]`; napi-rs emits `.node` artifacts plus generated `index.d.ts` in an isolated temporary output directory under `packages/natives/native/.build/`.
...
2. normalizes its name to `reactor_natives.<platform>-<arch>(-variant).node` when needed;
...
- x64: `reactor_natives.<platform>-<arch>-modern.node` or `...-baseline.node`
- non-x64: `reactor_natives.<platform>-<arch>.node`
...
- `REACTOR_NATIVE_VARIANT`: x64 runtime override; valid values are `modern` and `baseline`.
- `REACTOR_COMPILED`: legacy compiled-mode signal. A populated embedded-addon manifest is also a compiled-mode signal; compiled release builds additionally define `process.env.REACTOR_COMPILED="true"` during `bun build --compile`.
...
4. **Compile**: run napi-rs against `crates/reactor-natives` into an isolated output directory.
5. **Locate artifact**: accept the canonical filename or a single napi-rs-generated `reactor_natives.<platform>-<arch>*.node` candidate.
...
2. Loader resolves platform npm leaf-package candidates (`@reactor/natives-<platform>-<arch>`, when resolvable), then package-local `native/` and executable-dir fallback candidates.
3. Generated declarations in `native/index.d.ts` describe the public TS API.

## Shipped/compiled binary workflow

In compiled mode (`REACTOR_COMPILED`, Bun embedded URL markers, or populated embedded manifest):
...
3. Runtime candidate order includes:
   ...
   - versioned cache dir,
   - legacy compiled-binary dir (`%LOCALAPPDATA%/reactor` on Windows, `~/.local/bin` elsewhere),
...
| x64 machine loads baseline when modern expected                        | `REACTOR_NATIVE_VARIANT=baseline`, no AVX2 detected, or modern file unavailable                  | Check env and filenames in `native/`                              | Build modern variant (`TARGET_VARIANT=modern ... build`) and ship it                                  |
...
1. `crates` (whole subtree — reactor-natives transitively depends on other workspace crates)
2. `Cargo.lock`
3. `Cargo.toml`
...
- Root: `/data/cache/reactor-natives` (provisioned by `entrypoint.sh` alongside the cargo caches, owned `root:reactor`, mode `02770` setgid so cached files inherit `gid=reactor` and stay readable by every slot user).
...
### Configuration (settings on `reactor-worker.config.Settings`)

| Env var                                     | Default                  | Effect                                                                                              |
| ------------------------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------- |
| `REACTOR_WORKER_NATIVES_CACHE_ENABLED`              | `true`                   | Master switch. When false the populate/capture hooks no-op and every workspace builds from scratch. |
| `REACTOR_WORKER_NATIVES_CACHE_ROOT`                 | `/data/cache/reactor-natives` | Cache root directory. Must be `root:reactor 02770` for cross-slot reads.                                |
| `REACTOR_WORKER_NATIVES_CACHE_MAX_ENTRIES_PER_REPO` | `8`                      | LRU entry-count cap, per repo slug.                                                                 |
| `REACTOR_WORKER_NATIVES_CACHE_MAX_BYTES`            | `4294967296` (4 GiB)     | LRU byte cap, per repo slug.                                                                        |
| `REACTOR_WORKER_NATIVES_CACHE_GC_INTERVAL_SECONDS`  | `3600`                   | Period of the background GC loop in `WorkerPool`.                                                   |

### Manual invalidation

- One key: `rm -rf /data/cache/reactor-natives/<repo-slug>/<sha256>`.
- One repo: `rm -rf /data/cache/reactor-natives/<repo-slug>`.
- Everything: `rm -rf /data/cache/reactor-natives/*` (preserve the root so its setgid mode survives).
- Stuck lock: `rm /data/cache/reactor-natives/<repo-slug>/.lock` (only when no orchestrator process is touching the repo).
