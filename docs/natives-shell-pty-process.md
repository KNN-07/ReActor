...
This document covers execution/process/terminal primitives in `@reactor/natives`: `shell`, `pty`, `ps`, and `keys`, using the architecture terms from `docs/natives-architecture.md`.

## Implementation files

- `crates/reactor-natives/src/shell.rs`
- `crates/reactor-shell/src/shell.rs`
- `crates/reactor-shell/src/fixup.rs`
- `crates/reactor-shell/src/windows.rs` (Windows-only PATH enrichment)
- `crates/reactor-shell/src/process.rs`
- `crates/reactor-natives/src/pty.rs`
- `crates/reactor-natives/src/ps.rs`
- `crates/reactor-natives/src/keys.rs`
- `crates/reactor-natives/src/task.rs`
- `packages/natives/native/index.d.ts`

## Layer ownership

- **Package entrypoint** (`packages/natives/native/index.js`): loads the `.node` addon and exports generated N-API bindings.
- **Rust N-API module layer** (`crates/reactor-natives/src/*`): JS-facing shell/PTY/process/key exports and callback bridging.
- **Runtime core** (`crates/reactor-shell/src/*`): brush shell execution, cancellation cleanup, minimizer integration, command fixups, and cross-platform process references.
...
- `applyBashFixups(command)` strips safe trailing `| head`/`| tail` pipeline caps and redundant trailing `2>&1` according to `reactor_shell::fixup` rules. It returns `{ command, stripped }` and does not execute anything.
...
- Windows-only path enrichment (`reactor-shell/src/windows.rs`) appends discovered Git-for-Windows paths when present and not already included.
...
- `CancelToken` is constructed from `timeoutMs` and optional `AbortSignal`, then converted into the shared `reactor_shell::cancel::CancelToken`.
...
The platform-specific implementation lives in `reactor_shell::process`; `crates/reactor-natives/src/ps.rs` is a N-API shim plus re-exports used by PTY termination.
