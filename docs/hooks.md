...
- JS/TS hook factories discovered through `hookCapability` (for example `.reactor/hooks/pre/*.ts`) are loaded as extension modules so their `pi.on(...)` handlers bind to the runtime event bus
...
`src/extensibility/hooks/index.ts` and the package subpath `@reactor/coding-agent/extensibility/hooks` export:
...
The package root (`@reactor/coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
