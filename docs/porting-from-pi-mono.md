...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/coding-agent` → `@reactor/coding-agent`
  - `@mariozechner/agent-core` → `@reactor/agent-core`
  - `@mariozechner/tui` → `@reactor/tui`
  - `@mariozechner/ai` → `@reactor/ai`
  - `@mariozechner/utils` → `@reactor/utils`
- Some upstream packages publish under the `@earendil-works/*` scope instead of `@mariozechner/*`. Map it the same way (`@earendil-works/coding-agent` → `@reactor/coding-agent`, and so on).
- The bare `typebox` package is not an `@ReActor/*` scope; do not rewrite it as one. See the Extensions divergence in section 15 for how tool-parameter schemas map.
...
- In `packages/coding-agent`, use `logger` from `@reactor/utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@reactor/natives` exports generated native bindings from `packages/natives/native`.
...
| `ctx.ui.addAutocompleteProvider()`          | Wired in interactive mode; no-op stubs in ACP/RPC/headless contexts | Factory wrapping matches upstream; reactor's editor has no custom `triggerCharacters`, so wrapped providers surface at the built-in trigger points |
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@reactor/natives` | Native implementation with a small TS wrapper |
...
| `pkg.pi` manifest field                                          | `pkg.reactor` preferred; fallback to `pkg.pi` remains                                                  |
| `StringEnum` from `ai`                                        | `Type.Enum` from the `pi.typebox` shim (or author the schema with `pi.zod`); `ai` no longer exports `StringEnum` |
| `formatSize` from `coding-agent`                              | `formatBytes` from `@reactor/utils`                                                            |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@reactor/natives`
