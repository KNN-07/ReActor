...
The compiled macOS `reactor` binaries shipped on GitHub Releases are signed with a
...
submission (see [#776](https://github.com/KNN-07/ReActor/issues/776)).
...
- `curl https://reactor.norman.id.vn/install | sh` — `curl` sets no quarantine bit, so
...
Drop these into a working directory (default `~/reactor-signing`):
...
```sh
scripts/ci-macos-upload-secrets.sh ~/reactor-signing --dry-run   # validate first
scripts/ci-macos-upload-secrets.sh ~/reactor-signing             # upload all five
gh secret list --repo KNN-07/ReActor                       # confirm
```
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/reactor-darwin-arm64
```
