...
ACP (`reactor acp`) uses the same settings resolver as normal ReActor launches. Global `~/.reactor/agent/config.yml` applies, project config for the ACP session `cwd` applies, and any `--config <file>` overlays passed to the ACP server process apply to sessions created by that process.
