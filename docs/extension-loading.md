...
- Project directory: `<cwd>/.reactor/extensions`
- User directory: `~/.reactor/agent/extensions`
- Native legacy/settings JSON entries: `<cwd>/.reactor/settings.json#extensions` and `~/.reactor/agent/settings.json#extensions`

The project root is the native provider's `.reactor` directory (`SOURCE_PATHS.native.projectDir`), cwd-only; it does not walk ancestors. The user root is the active profile's agent directory via `getAgentDir()`, so under `reactor --profile <name>` it becomes `~/.reactor/profiles/<name>/agent/extensions` (and it honors `REACTOR_CODING_AGENT_DIR`). See [Profiles](./config-usage.md#profiles).

Notes:

- Native auto-discovery is currently `.reactor` based.
...
Plugin extension entries come from package `reactor.extensions` / `pi.extensions` manifests, including enabled feature entries.
...
- User: `~/.reactor/agent/config.yml` (or custom agent dir via `REACTOR_CODING_AGENT_DIR`)
- Project/native settings capability: `<cwd>/.reactor/config.yml` and `<cwd>/.reactor/settings.json`

Native extension-module discovery also reads legacy JSON extension lists from:

- `~/.reactor/agent/settings.json`
- `<cwd>/.reactor/settings.json`
...
```json
{
  "extensions": ["./.reactor/extensions/my-extra"]
}
```
...
1. `package.json` in that directory with `reactor.extensions` (or legacy `pi.extensions`) -> use declared entries
2. `index.ts`
3. `index.js`
4. Otherwise scan one level for extension entries:
   - direct `*.ts` / `*.js`
   - subdir `index.ts` / `index.js`
   - subdir `package.json` with `reactor.extensions` / `pi.extensions`
