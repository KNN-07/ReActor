...
- Worktree path is `getWorktreeDir("<number>-<repo-hash>")` = `path.join(getWorktreesDir(), "<number>-<repo-hash>")`, where `getWorktreesDir()` is `~/.reactor/wt`, `<number>` is the PR number, and `<repo-hash>` is `hashPath(primaryRepoRoot)` (a 7-hex digest of the primary repo root); effective path is `~/.reactor/wt/<number>-<repo-hash>`. `resolveAvailableWorktreePath()` appends a `-2`/`-3`… suffix when that path is already registered with git or present on disk.
...
- The branch push metadata is persisted with `git config` under the repository's shared `.git/config` as:
  ...
  - `branch.pr-<number>.reactorPrHeadRef`
  - `branch.pr-<number>.reactorPrUrl`
  - `branch.pr-<number>.reactorPrIsCrossRepository`
  - `branch.pr-<number>.reactorPrMaintainerCanModify`
...
Push target resolution reads the `branch.<name>.reactorPrHeadRef`, `pushRemote`/`remote`, `reactorPrUrl`, `reactorPrMaintainerCanModify`, and `reactorPrIsCrossRepository` git-config keys written by `pr_checkout`. If the current checked-out branch matches the target branch, the source ref is `HEAD`; otherwise it pushes `refs/heads/<branch>`. The refspec is `HEAD:refs/heads/<headRef>` or `refs/heads/<branch>:refs/heads/<headRef>`.
...
- Filesystem
  ...
  - `pr_checkout` may create worktree directories named `<pr-number>-<repo-hash>` directly under `~/.reactor/wt/` and add git worktrees there.
...
- Local validation errors throw `ToolError`, including:
  ...
  - `pr_push` on a branch without `reactorPrHeadRef` metadata
