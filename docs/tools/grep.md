...
- Key collaborators:
  ...
  - `crates/reactor-natives/src/grep.rs` — native regex/file search implementation.
...
4. Internal URLs are resolved before filesystem scope resolution:
   ...
   - `reactor://` expands to every embedded documentation file via URL completion;
...
9. It calls native `grep()` from `@reactor/natives` with:
...
10. Native execution happens in `crates/reactor-natives/src/grep.rs`:
...
5. **Internal URL paths**
   ...
   - `reactor://` expands to all embedded documentation files so it can be used as a docs search root.
...
- Native per-file size cap: `4 * 1024 * 1024` bytes (`MAX_FILE_BYTES` in `crates/reactor-natives/src/grep.rs`, mirrored as `NATIVE_GREP_MAX_FILE_BYTES` in `grep.ts`). Oversized files are silently skipped by native grep; `grep.ts` surfaces a `Skipped oversized file(s)` note (with names for explicit file targets, a count for directory scans).
