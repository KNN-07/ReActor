...
## Native `.reactor` files

The native provider is the recommended format for new projects. It reads from your user agent directory and from `.reactor/` directories inside a project, and it has the highest discovery priority, so its files win over every other convention at the same scope.

| File | Scope | Behavior |
|---|---|---|
| `~/.reactor/agent/AGENTS.md` | User | User-level context for every session unless the `native` provider is disabled. |
| `<ancestor>/.reactor/AGENTS.md` | Project | Project context. `reactor` walks upward from the current directory to the repository root and uses the **nearest** non-empty `.reactor/AGENTS.md`. Farther native project files are not also included. |
| `~/.reactor/agent/RULES.md` | User | User-level sticky rule content. Loaded as an always-apply rule, not as a context file. |
| `<ancestor>/.reactor/RULES.md` | Project | Project sticky rule content. Same nearest-ancestor walk-up as above. Loaded as an always-apply rule. |

Two details matter:

- **Walk-up to the repository root.** Discovery starts in the current working directory and climbs through each ancestor up to the repository root, stopping at the first ancestor that has a usable `.reactor/` directory. The *nearest* match wins; ancestors above it are not loaded as native context.
- **The `.reactor/` directory must be non-empty.** An empty `.reactor/` directory is skipped during the walk-up, so the search continues to the next ancestor. An empty `AGENTS.md` or `RULES.md` file contributes nothing.

`~/.reactor/agent` is the user base. If `REACTOR_CODING_AGENT_DIR` is set, it relocates that base, so the user files become `$REACTOR_CODING_AGENT_DIR/AGENTS.md` and `$REACTOR_CODING_AGENT_DIR/RULES.md`.
...
- The native context file is `repo/packages/api/.reactor/AGENTS.md` (the nearest one). `repo/.reactor/AGENTS.md` is **not** also included.
- The project sticky rule is `repo/packages/api/.reactor/RULES.md` if present; otherwise the walk-up continues and `repo/.reactor/RULES.md` is used.
...
`reactor` also discovers the context and rule files of other agent tools so existing projects keep working without migration.

| Provider id | Convention path | Scope | Notes |
|---|---|---|---|
| `native` | `.reactor/AGENTS.md` | User + project | Recommended `reactor` format. User file at `~/.reactor/agent/AGENTS.md`; project file is the nearest non-empty `.reactor/AGENTS.md` walking up to the repo root. |
...
Providers marked "(no ancestor walk-up)" only look in the current working directory's config directory. If you need ancestor walk-up behavior, prefer the native `.reactor/AGENTS.md` format or a standalone `AGENTS.md` (the `agents-md` provider), or launch `reactor` from the directory that holds the config directory.
...
- **One user context file** is kept across all providers. Because `native` has the highest priority, `~/.reactor/agent/AGENTS.md` shadows every other user-level context file.
...
- If you add `repo/packages/api/.reactor/AGENTS.md`, `native` (priority 100) wins depth 0 outright, shadowing both lower-priority files.
...
- It is read **only** at the native locations — `~/.reactor/agent/RULES.md` and the nearest `<ancestor>/.reactor/RULES.md` from the cwd up to the repo root. A `RULES.md` anywhere else is not a context-file convention and is ignored.
...
Turn a provider off with the `disabledProviders` setting in `~/.reactor/agent/config.yml`, a project's `.reactor/config.yml`, or a `--config` overlay:
...
- Native project context must live at `.reactor/AGENTS.md`, and the `.reactor/` directory must be non-empty; an empty `.reactor/` is skipped and the walk-up continues to the next ancestor.
...
At one user scope or project depth, the higher-priority provider shadows the others (native > claude > agents/codex > gemini > opencode > github > agents-md). To force deterministic behavior, move your guidance into `.reactor/AGENTS.md` (native always wins) or disable the competing discovery provider.

### User context disappeared

Only one user-level context file survives, and `~/.reactor/agent/AGENTS.md` has the highest priority. If it exists, it shadows user-level `~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, `~/.gemini/GEMINI.md`, `~/.config/opencode/AGENTS.md`, `~/.copilot/copilot-instructions.md`, and `~/.agent`/`~/.agents` files. Consolidate user guidance into the native file or remove the native one if you prefer another tool's file.

### A `RULES.md` file is ignored

Only the native `RULES.md` locations are sticky: `~/.reactor/agent/RULES.md` and the nearest `<ancestor>/.reactor/RULES.md` from cwd to the repo root. A `RULES.md` in any other directory is not a recognized convention and will not be loaded.
