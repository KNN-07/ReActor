...
- **Include** root docs coverage for package-local CLIs, extension features, dashboards, and benchmark runners that users can run directly or through `reactor`.
...
- Package: `@reactor/swarm-extension`; bin: `reactor-swarm`.
- Feature: multi-agent DAG orchestration from YAML swarms, supporting `pipeline`, `parallel`, and `sequential` modes.
- Standalone CLI: `reactor-swarm path/to/swarm.yaml` runs until completion or process termination.
...
Sources: [`packages/terminal-bench/README.md`](../packages/terminal-bench/README.md), [`packages/terminal-bench/package.json`](../packages/terminal-bench/package.json), [`packages/terminal-bench/src/runner.ts`](../packages/terminal-bench/src/runner.ts), [`packages/terminal-bench/agent/reactor_local.py`](../packages/terminal-bench/agent/reactor_local.py).

- Package: private `@reactor/terminal-bench`; bin: `tb2`.
- Feature: runs `harbor-framework/terminal-bench-2` against a local or published `reactor` build with a live progress, spend, token, ETA, and pass/fail dashboard.
- CLI: `bun src/runner.ts [options] [-- <extra harbor args>]`; package bin exposes `tb2`.
- Modes: default `reactor` agent, `oracle`/`nop`/any Harbor agent via `--agent`; local source packing by default, published npm install via `--install published`; `cleanup` command removes leftover Harbor Docker resources.
...
- Package: `@reactor/stats`; bin: `reactor-stats`; main user path: `reactor stats`.
- Feature: local observability dashboard for AI usage statistics from session JSONL logs.
- CLI modes: `reactor stats` starts the dashboard server, opens `http://localhost:3847`, and keeps running; `reactor stats --port <port>` changes the port; `reactor stats --summary` prints a console summary; `reactor stats --json` prints JSON and exits.
- Programmatic API: exports helpers such as `syncAllSessions()` and `getDashboardStats()` for embedding.
- Inputs/storage: reads `~/.reactor/agent/sessions/`; stores aggregates in `~/.reactor/stats.db`.
...
- Package: private `@reactor/typescript-edit-benchmark`; bin: `typescript-edit-benchmark`.
