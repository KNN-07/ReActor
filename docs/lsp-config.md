# LSP configuration in ReActor

This guide explains how to configure language servers for the ReActor coding agent.
...
When no LSP config file is present, ReActor auto-detects servers by intersecting two conditions:
...
ReActor merges LSP config from multiple files, lowest to highest priority:

| Priority    | Location                                                                                                                    |
| ----------- | --------------------------------------------------------------------------------------------------------------------------- |
| 5 (lowest)  | `~/lsp.json`, `~/.lsp.json`, `~/lsp.yaml`, `~/.lsp.yaml`, `~/lsp.yml`, `~/.lsp.yml`                                         |
| 4           | Plugin LSP configs (marketplace / `--plugin-dir` roots)                                                                     |
| 3           | User config dirs: `~/.reactor/agent/lsp.*`, `~/.claude/lsp.*`, `~/.codex/lsp.*`, `~/.gemini/lsp.*`                              |
| 2           | Project config dirs: `<project>/.reactor/lsp.*`, `<project>/.claude/lsp.*`, `<project>/.codex/lsp.*`, `<project>/.gemini/lsp.*` |
...
- User-wide preferences → `~/.reactor/agent/lsp.json`
- Project-specific overrides → `<project>/.reactor/lsp.json`

> **Note:** Auto-detection is skipped only when at least one config file contributes server overrides. A config file that only sets `idleTimeoutMs` still lets ReActor auto-detect built-in servers. When server overrides exist, ReActor merges them with defaults and then loads servers that have matching `rootMarkers`, an available binary, and are not explicitly `disabled`.
...
The `capabilities` object enables optional server-specific features that ReActor supports on a per-server basis:
...
Place the override in `<project>/.reactor/lsp.json`:
...
The user-level config in `~/.reactor/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
