...
```text
Config sources (.reactor/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.reactor/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.reactor/mcp.json` (project) or `~/.reactor/agent/mcp.json` (user) for explicit control.
...
ReActor-native MCP config (`.reactor/mcp.json`, `~/.reactor/agent/mcp.json`, plus their `.mcp.json` variants) expands `${VAR}` and `${VAR:-default}` placeholders recursively before converting to runtime config. It also accepts boolean/string forms for `enabled` (`true`, `false`, `1`, `0`) and numeric strings for `timeout`.
