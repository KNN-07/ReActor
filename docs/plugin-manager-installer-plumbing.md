...
This document describes how `reactor plugin` npm/git/link operations mutate plugin state on disk and how installed npm/git/link plugins become runtime capabilities (tools and extensions today, hooks/commands path resolution available). Marketplace installs use separate marketplace registries and cache plumbing; see `docs/marketplace.md`.
...
```text
reactor plugin <npm/link action> ...
  -> src/commands/plugin.ts
  -> runPluginCommand(...) in src/cli/plugin-cli.ts
  -> PluginManager method (install/list/uninstall/link/...)
  -> mutate ~/.reactor/plugins/{package.json,node_modules,reactor-plugins.lock.json}
...
reactor plugin install name@marketplace / reactor install name@marketplace
  -> MarketplaceManager
  -> mutate ~/.reactor/marketplaces.json, ~/.reactor/plugins/installed_plugins.json, cache dirs
...
Global plugin state lives under `~/.reactor/plugins`:
...
- `reactor-plugins.lock.json` — runtime state for npm/link plugins:
...
- `<cwd>/.reactor/plugin-overrides.json`
...
- `~/.reactor/marketplaces.json` — configured marketplace catalogs
- `~/.reactor/plugins/installed_plugins.json` — user-scoped marketplace installs
- `<cwd>/.reactor/plugins/installed_plugins.json` — project-scoped marketplace installs when available
- `~/.reactor/plugins/cache/{marketplaces,plugins}/` — cached catalogs and plugin directories
...
- A package missing `reactor`/`pi` is still installable and listable.
- Runtime plugin loading (`getEnabledPlugins`) skips packages without `reactor`/`pi` manifest.
...
3. Ensure plugin `package.json` exists (`reactor-plugins`, private dependencies map).
4. Run `bun install <packageSpec>` in `~/.reactor/plugins`.
...
- `reactor plugin install pkg@newVersion` updates dependency and lockfile version.
...
1. Read plugin dependency map from `~/.reactor/plugins/package.json`.
2. Load lockfile runtime config (missing file -> empty defaults).
3. Load project overrides (`<cwd>/.reactor/plugin-overrides.json`, parse/read errors -> empty object with warning).
...
`link` supports local plugin development by symlinking a local package into `~/.reactor/plugins/node_modules/<pkg.name>`.
...
- skip if manifest (`reactor`/`pi`) absent
