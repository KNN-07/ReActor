...
| `packages/stats`        | Local observability dashboard (`reactor stats`)          |
| `packages/utils`        | Shared utilities (logger, streams, temp files)       |
| `crates/reactor-natives`     | Rust crate for performance-critical text/grep ops    |

**Catalog import convention**: code in this repo imports catalog *values* (bundled models, model-thinking helpers, identity, descriptors, model manager/cache) from `@reactor/catalog/<module>` — never via `@reactor/ai`. The ai barrel re-exports only the model/effort *types* its own signatures use (`Model`, `Api`, `ThinkingConfig`, `Effort`, …); type-only imports of those from `@reactor/ai` are fine.
...
This checkout is the public `KNN-07/ReActor` fork. The product surface is
`reactor`, `@reactor/*`, `.reactor`, and `REACTOR_*`; do not add an legacy command alias,
legacy dot-directory fallback, or legacy-data importer. Upstream remains available as
the `upstream` remote and is pinned in `.upstream/oh-my-pi.json`.

To synchronize a reviewed upstream range, keep the worktree clean and run
`bun run sync:upstream --to <sha>`. It creates a `sync/oh-my-pi-<short-sha>`
branch, applies `.upstream/rename-map.json`, reports skipped protected paths,
and leaves changes uncommitted. Resolve and review the patch, then run
`bun run sync:upstream:finalize`; that command runs the full check, test, smoke,
Python, install-method, and release-binary gates before making the attributed
sync commit. Never merge a floating upstream branch or overwrite
`packages/autonomy` while syncing.

Reliable autonomy is explicitly opt-in through `reactor run` or `/react start`.
Its default limits are eight continuations, 60 active minutes, two equivalent
failures, and required verification evidence. Approval requests, user input,
interrupts, exhausted limits, repeated equivalent failures, and indeterminate
tool calls pause the goal. Restarted goals load paused; external side effects
cannot be rolled back, so the lifecycle guarantee is never blindly replaying a
started tool call.
...
- **Worker scripts**: workers re-enter the CLI entrypoint; never spawn separate worker entry modules. `cli.ts` declares itself as the worker host at startup (`declareWorkerHostEntry()` from `@reactor/utils/env`) and dispatches hidden argv selectors (`__reactor_worker_stats_sync`, `__reactor_worker_tab`, `__reactor_worker_js_eval`, `__reactor_worker_tiny_inference`) before loading the command registry. Spawn sites use:
  ```ts
  import { workerHostEntry } from "@reactor/utils";
  const hostEntry = workerHostEntry();
  const worker = hostEntry
  ? new Worker(hostEntry, { type: "module", argv: ["__reactor_worker_<name>"] })
  : new Worker(new URL("./<worker>.ts", import.meta.url).href, { type: "module" });
  ```
  When the process was started from the reactor CLI — source `cli.ts`, npm-bundle `dist/cli.js`, or compiled binary — `workerHostEntry()` is `Bun.main` and the worker re-enters the single entry module, so no per-worker `--compile` entrypoints or bundle entries exist. Outside a CLI host (`bun test`, SDK embedding, standalone `reactor-stats`) it returns `null` and the direct-module fallback loads the worker source. New worker kinds MUST add their selector to the dispatch table in `cli.ts` and keep the fallback branch.
  ...
  Validate any new worker with the dedicated smoke probe: `reactor --smoke-test` spawns the stats sync worker and the tiny-model subprocess, pings them, and exits — it's wired into `ci:test:smoke` and `scripts/install-tests/run-ci.sh` so binary, source-link, and tarball installs all exercise it. Add a sibling smoke if the new worker is on a different module graph.
...
| Binary lookup   | `$which("git")` from `@reactor/utils` | `spawnSync(["which", "git"])`   |
...
Logs go to `~/.reactor/logs/reactor.YYYY-MM-DD.log` with automatic rotation.
...
**Rules:**
- **Tabs → spaces** via `replaceTabs()` (from `@reactor/tui` or `../tools/render-utils`).
...
**Attribution:**
- Internal (from issues): `Fixed foo bar ([#123](https://github.com/KNN-07/ReActor/issues/123))`.
- External contributions: `Added feature X ([#456](https://github.com/KNN-07/ReActor/pull/456) by [@username](https://github.com/username))`.
