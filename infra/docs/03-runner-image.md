...
Build context lives at `/root/reactor-kata-runner-image/`. The Dockerfile below is
...
> the `/root/reactor-kata-runner-image/` copy is overwritten from it by the
> repo-driven reload script (see section 3 below).

```dockerfile
# syntax=docker/dockerfile:1
# Preloaded reactor-kata runner image.
#
...
#
# Rebuild + reimport (see /root/reactor-kata-runner.md) after bumping the ARGs below
...
`/root/reactor-kata-runner-image/reload.sh` does the whole cycle: build, in-image
...
```bash
#!/usr/bin/env bash
# Rebuild the preloaded reactor-kata runner image, import it into k3s containerd,
...
#
#   ./reload.sh              # build tag reactor-kata-runner:YYYY-MM-DD-HHMMSS
#   ./reload.sh 2026-06-20   # build tag reactor-kata-runner:2026-06-20
...
sed -i "s#image: reactor-kata-runner:.*#image: $IMAGE#" /root/arc-reactor-values.yaml
helm upgrade reactor-kata --namespace arc-runners --version 0.14.2 \
  -f /root/arc-reactor-values.yaml \
  oci://ghcr.io/actions/actions-runner-controller-charts/gha-runner-scale-set >/dev/null

echo "==> [5/5] verifying rollout"
live="$(kubectl get autoscalingrunnerset reactor-kata -n arc-runners -o jsonpath='{.spec.template.spec.containers[0].image}')"
...
```bash
cd /root/reactor-kata-runner-image
./reload.sh
```
...
`$1`: no arg gives a timestamped `reactor-kata-runner:YYYY-MM-DD-HHMMSS`; an argument
containing a colon (`foo:bar`) is used as an explicit `repo:tag`; anything else
is treated as a tag suffix on `reactor-kata-runner:`.

**[1/5] build.** `DOCKER_BUILDKIT=1 docker build` tags the result twice: the
immutable `$IMAGE` (dated) and the moving `reactor-kata-runner:preloaded` alias.
...
`image: reactor-kata-runner:...` line in `/root/arc-reactor-values.yaml` to the new tag,
...
- `docker save | k3s ctr -n k8s.io images import` places the image directly into
  ...
  reference `reactor-kata-runner:<tag>` to `docker.io/library/reactor-kata-runner:<tag>`
...
| --- | --- | --- | --- | --- |
| `reactor-kata-runner:YYYY-MM-DD-HHMMSS` | immutable | yes | yes | the build of record; what runners actually boot |
| `reactor-kata-runner:preloaded` | moving | no | no | docker-local alias to the most recent build |
...
- Live example at time of writing: the scale set references
  `reactor-kata-runner:2026-06-15-002621`, held in containerd as
  `docker.io/library/reactor-kata-runner:2026-06-15-002621`.
...
```bash
docker run --rm --entrypoint bash reactor-kata-runner:preloaded -lc '
  set -e
  for b in gh fd rg magick bun cargo rustc pkg-config clang lld sccache zig cargo-nextest cargo-zigbuild cargo-xwin; do
    command -v "$b" >/dev/null || { echo "MISSING: $b"; exit 1; }
  done
...
```bash
export KUBECONFIG=/etc/rancher/k3s/k3s.yaml
kubectl get autoscalingrunnerset reactor-kata -n arc-runners \
  -o jsonpath='{.spec.template.spec.containers[0].image}{"\n"}'
k3s ctr -n k8s.io images ls | grep reactor-kata-runner
```
