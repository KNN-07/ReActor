...
`scripts/probe.ts` routes through `@reactor/ai` (`completeSimple`) so model/auth/provider behavior matches production.

```bash
bun .reactor/skills/tool-prompt-optimization/scripts/probe.ts \
...
```bash
bun .reactor/skills/tool-prompt-optimization/scripts/probe-builtin.ts --tool <name> [--no-summary] [--show]
```
...
```ts
import { arkToWireSchema } from "@reactor/ai"; // or toolWireSchema(tool)
