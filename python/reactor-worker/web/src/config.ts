...
// `__REACTOR_WORKER_CONFIG__` sentinel in `static/index.html` with a JSON blob so the
...
function readConfig(): AppConfig {
  const node = document.getElementById("reactor_worker-config");
  const text = node?.textContent?.trim();
  if (!text || text === "__REACTOR_WORKER_CONFIG__") {
