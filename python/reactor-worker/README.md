...
- `question` → one comment, suffixed with a 👎-to-keep-open prompt; if the
  issue author doesn't react 👎 within `REACTOR_WORKER_QUESTION_AUTOCLOSE_HOURS`
...
Follow-up issue comments and PR review comments resume the same reactor session
...
- **reactor_worker** — FastAPI + sqlite event queue + `WorkerPool` running `reactor` in
...
- **gh-proxy** — sibling on an `internal: true` network. Holds `GITHUB_TOKEN`,
  verifies HMAC-signed requests from reactor_worker, executes REST + `git push`.
...
→ `worker.run_task` spawns `reactor --mode rpc` with `cwd=worktree`,
persistent `session_dir`, model randomly drawn from `REACTOR_WORKER_MODEL` (CSV).

The agent uses reactor's built-in tools (`read`/`edit`/`bash`/`lsp`, scoped to
...
Bot account needs **Write** on every repo in `REACTOR_WORKER_REPO_ALLOWLIST`. A
...
comment out `REACTOR_WORKER_GH_PROXY_URL` / `REACTOR_WORKER_GH_PROXY_HMAC_KEY` and set
...
The container entrypoint is `python -m reactor_worker serve`. Other commands run
inside the running container:

```bash
docker compose exec reactor_worker reactor_worker triage  owner/repo#123   # synthesize an issues.opened and wait
docker compose exec reactor_worker reactor_worker replay  <delivery_id>    # re-enqueue a stored event and wait
docker compose exec reactor_worker reactor_worker status                   # dump issues table
...
REACTOR_WORKER_INTEGRATION=1 pytest -x tests/test_worker_smoke.py
```

The integration test spawns a real `reactor --mode rpc` against an
`httpx.MockTransport` GitHub and a local bare repo, so it needs `reactor` on
...
- gh-proxy has no host port. The `reactor_worker_internal` network is
...
- Agent subprocess env is scrubbed of `GITHUB_TOKEN` /
  `REACTOR_WORKER_GH_PROXY_HMAC_KEY` / friends via `worker._SCRUBBED_ENV_KEYS`.
...
- Pre-push gates (`gh_push_branch`): branch matches the workspace
  ...
  `origin/<default>..HEAD` carries `REACTOR_WORKER_GIT_AUTHOR_NAME` +
  `REACTOR_WORKER_GIT_AUTHOR_EMAIL`. Commit messages carrying shell-literal
...
- **Crash recovery.** On startup, `db.reset_stuck_running()` flips
  ...
  `REACTOR_WORKER_SHUTDOWN_DRAIN_TIMEOUT_SECONDS` (25s) +
  `REACTOR_WORKER_SHUTDOWN_KILL_TIMEOUT_SECONDS` (5s); compose
  `stop_grace_period: 30s` covers both.
- **Logs.** Structured JSON on stdout, rotated to
  `/data/logs/reactor_worker.log.jsonl`.
...
src/
  ...
  worker.py          synchronous reactor RPC driver, prompt assembly, env scrubbing
...
tests/               pytest unit suite + one REACTOR_WORKER_INTEGRATION=1 smoke test
