...
Webhook → durable queue → async dispatcher → per-issue git worktree → reactor RPC subprocess + host tools.
...
4. `queue.WorkerPool._dispatch_loop` atomically claims `state='queued'` rows under `BEGIN IMMEDIATE`, guarded by an in-process `_inflight` set keyed by `(owner, repo, number)` to serialize per-issue work. Cap: `REACTOR_WORKER_MAX_CONCURRENCY` (default 8).
...
6. `tasks.*` dispatchers build `TaskInputs` and call `worker.run_task()` which spawns `reactor --mode rpc` with `cwd=worktree`, persistent `session_dir`, and a randomly-picked model from `REACTOR_WORKER_MODEL` (CSV pool). When `<session_dir>/*.jsonl` already exists the worker passes `--continue`, so both follow-up events and crash-restarted events resume the same session.
7. Inside the subprocess the agent uses **built-in reactor tools** (read/edit/write/bash/lsp, scoped to the worktree) and **host tools** from `host_tools.py` (the only surface allowed to mutate GitHub or write audit rows).
...
- **Async style**: FastAPI handlers and `queue.WorkerPool` are async. `worker.run_task` is **synchronous** and runs in a worker thread because `reactor-rpc` is blocking — keep it that way; don't try to async it. CLI commands wrap with `asyncio.run`.
- **Config**: `pydantic-settings` `Settings` in `config.py` with `REACTOR_WORKER_*` env prefix (e.g. `REACTOR_WORKER_MAX_CONCURRENCY`, `REACTOR_WORKER_REPO_ALLOWLIST`). Access only via `get_settings()` (`@cache` singleton). Tests must call `reset_settings_cache()` after mutating env.
...
- `src/worker.py` — synchronous reactor RPC driver, prompt assembly via `persona`.
...
- **Fixtures** (`tests/conftest.py`):
  - `env` — `monkeypatch`-sets all required `REACTOR_WORKER_*` env vars and calls `reset_settings_cache()` before/after.
...
- **Integration**: `tests/test_worker_smoke.py` is gated by `REACTOR_WORKER_INTEGRATION=1` (uses `pytestmark.skipif`) and needs `reactor` on `PATH`. Don't enable it in default `bun run test:py`.
