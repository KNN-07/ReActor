...
from reactor_worker.dashboard import reset_index_cache, static_dir
from reactor_worker.db import Database, close_database

# Minimum HTML the dashboard handler needs to render: `<title>` plus a script
# block carrying the `__REACTOR_WORKER_CONFIG__` sentinel. The real Vite-built bundle
# adds JS/CSS asset links; tests only care about the rendering contract.
_PLACEHOLDER_INDEX_HTML = (
    "<!doctype html>\n"
    '<html lang="en">\n'
    '  <head><meta charset="utf-8"><title>reactor_worker</title></head>\n'
    "  <body>\n"
    '    <div id="app"></div>\n'
    '    <script id="reactor_worker-config" type="application/json">__REACTOR_WORKER_CONFIG__</script>\n'
    "  </body>\n"
    "</html>\n"
)
...
def _baseline_env(tmp_path: Path) -> dict[str, str]:
    return {
        ...
        "REACTOR_WORKER_GH_PROXY_URL": "http://gh-proxy.invalid:8081",
        "REACTOR_WORKER_GH_PROXY_HMAC_KEY": "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "GITHUB_WEBHOOK_SECRET": "test-webhook-secret",
        "REACTOR_WORKER_BOT_LOGIN": "reactor_worker-bot",
        "REACTOR_WORKER_GIT_AUTHOR_NAME": "reactor_worker-bot",
        "REACTOR_WORKER_GIT_AUTHOR_EMAIL": "reactor_worker-bot@example.invalid",
        "REACTOR_WORKER_REPO_ALLOWLIST": "octo/widget",
        "REACTOR_WORKER_MODEL": "anthropic/claude-sonnet-4-5",
        "REACTOR_WORKER_THINKING": "high",
        "REACTOR_WORKER_WORKSPACE_ROOT": str(tmp_path / "workspaces"),
        "REACTOR_WORKER_SQLITE_PATH": str(tmp_path / "reactor_worker.sqlite"),
        "REACTOR_WORKER_LOG_DIR": str(tmp_path / "logs"),
        # Production default is `/data/cache/reactor-natives` (provisioned by the
        ...
        # cache flip `REACTOR_WORKER_NATIVES_CACHE_ENABLED=true` explicitly.
        "REACTOR_WORKER_NATIVES_CACHE_ROOT": str(tmp_path / "natives-cache"),
        "REACTOR_WORKER_NATIVES_CACHE_ENABLED": "false",
        ...
        "REACTOR_WORKER_ISSUE_INDEX_SYNC_SECONDS": "0",
    }
...
def env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    monkeypatch.delenv("REACTOR_WORKER_PROVIDER", raising=False)
    monkeypatch.setenv("REACTOR_WORKER_REPLAY_TOKEN", "")
...
def proxy_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    baseline.pop("REACTOR_WORKER_GH_PROXY_URL", None)
    baseline.pop("REACTOR_WORKER_GH_PROXY_HMAC_KEY", None)
    ...
    monkeypatch.setenv("REACTOR_WORKER_GH_PROXY_URL", "")
    monkeypatch.setenv("REACTOR_WORKER_GH_PROXY_HMAC_KEY", "")
    monkeypatch.delenv("REACTOR_WORKER_PROVIDER", raising=False)
    monkeypatch.setenv("REACTOR_WORKER_REPLAY_TOKEN", "")
    reset_settings_cache()
    yield baseline
