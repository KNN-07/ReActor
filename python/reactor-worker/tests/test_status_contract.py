"""Contract test for the reactor_worker server API response vs status-contract.json.

Regenerate the committed fixture with:
    REACTOR_WORKER_UPDATE_STATUS_CONTRACT=1 pytest tests/test_status_contract.py
"""
...
from reactor_worker.db import get_database
...
_UPDATE_ENV = "REACTOR_WORKER_UPDATE_STATUS_CONTRACT"
...
def test_status_contract(settings: Settings) -> None:
    ...
    with TestClient(app) as client:
        ...
        # pytest runs assert equality; regenerate only when REACTOR_WORKER_UPDATE_STATUS_CONTRACT=1.
