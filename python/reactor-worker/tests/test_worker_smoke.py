...
Runs only when REACTOR_WORKER_INTEGRATION=1 and `reactor` is available on PATH (or via
REACTOR_WORKER_REACTOR_COMMAND). Spins up:

- a local bare git repo with a trivial failing test,
- a fake GitHub API via httpx.MockTransport that records comments + PRs,
- a real `reactor --mode rpc` subprocess driven by `worker.run_task`.
...
INTEGRATION = os.environ.get("REACTOR_WORKER_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="REACTOR_WORKER_INTEGRATION=1 required to run the reactor-backed smoke test",
)
...
def test_triage_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from reactor_worker.sandbox import LocalGitTransport, SandboxManager
    from reactor_worker.tasks import triage_issue

    bare = _seed_failing_repo(tmp_path)

    monkeypatch.setenv("REACTOR_WORKER_GH_PROXY_URL", "http://gh-proxy.invalid:8081")
    monkeypatch.setenv("REACTOR_WORKER_GH_PROXY_HMAC_KEY", "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    monkeypatch.setenv("GITHUB_TOKEN", "")
    monkeypatch.setenv("GITHUB_WEBHOOK_SECRET", "secret")
    monkeypatch.setenv("REACTOR_WORKER_BOT_LOGIN", "reactor_worker-bot")
    monkeypatch.setenv("REACTOR_WORKER_REPO_ALLOWLIST", "octo/widget")
    monkeypatch.setenv("REACTOR_WORKER_WORKSPACE_ROOT", str(tmp_path / "workspaces"))
    monkeypatch.setenv("REACTOR_WORKER_SQLITE_PATH", str(tmp_path / "reactor_worker.sqlite"))
    monkeypatch.setenv("REACTOR_WORKER_LOG_DIR", str(tmp_path / "logs"))
    monkeypatch.setenv("REACTOR_WORKER_TASK_TIMEOUT_SECONDS", "300")
