...
These tests swap `reactor_worker.worker.RpcClient` for a recording fake so we can
observe the `extra_args` and `set_todos` decisions the driver takes based on
whether the workspace's reactor session directory already holds a JSONL transcript.
"""
...
from reactor_worker import worker
from reactor_worker.config import Settings
from reactor_worker.git_ops import DirtyState
...
def _patch_worker(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr("reactor_worker.worker.RpcClient", _FakeRpcClient)
    monkeypatch.setattr("reactor_worker.worker._AGENT_HOME_STAGE", tmp_path / "missing-agent-home-stage")
    monkeypatch.setattr("reactor_worker.worker.host_tools.build", lambda _b: ())
    monkeypatch.setattr(
        "reactor_worker.worker.persona.system_append",
        lambda *, repo, issue, workspace, bot_login: "SYS",
    )
    monkeypatch.setattr(
        "reactor_worker.worker.persona.seed_phases",
...
async def test_run_rpc_omits_continue_when_session_empty(
    ...
    assert client_kwargs["env"]["REACTOR_WORKER_REPLAY_TOKEN"] == ""
    assert client_kwargs["env"]["REACTOR_WORKER_GH_PROXY_HMAC_KEY"] == ""
...
def test_build_extra_env_stages_agent_home(tmp_path: Path, settings: Settings, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    reactor_agent_dir = stage_home / ".reactor" / "agent"
    agent_rules_dir.mkdir(parents=True)
    reactor_agent_dir.mkdir(parents=True)
    (agent_dir / "AGENTS.md").write_text("agent instructions\n", encoding="utf-8")
    (agent_rules_dir / "rule.md").write_text("rule\n", encoding="utf-8")
    (reactor_agent_dir / "models.yml").write_text("models: []\n", encoding="utf-8")
    ...
    assert (agent_home / ".reactor" / "agent" / "models.yml").is_file()
    ...
    assert (agent_home / ".reactor" / "agent").stat().st_mode & 0o777 == 0o755
    assert (agent_home / ".reactor" / "agent" / "models.yml").stat().st_mode & 0o777 == 0o644
...
async def test_run_rpc_uses_workspace_xdg_dirs_without_slot(tmp_path: Path, settings: Settings) -> None:
    ...
    xdg_root = inputs.workspace.root / ".reactor-xdg"
    for key in ("XDG_DATA_HOME", "XDG_STATE_HOME", "XDG_CACHE_HOME"):
        path = Path(env[key])
        assert path.is_relative_to(xdg_root)
        assert (path / "reactor").is_dir()
    tmpdir = inputs.workspace.root / ".reactor-tmp"
...
async def test_run_rpc_uses_workspace_xdg_dirs_for_slot_without_chown(
    ...
    monkeypatch.setattr("reactor_worker.sandbox.os.chown", lambda path, uid, gid: chown_calls.append((Path(path), uid, gid)))
...
@pytest.mark.asyncio
async def test_run_rpc_passes_slot_uid_user_slot_group_and_reactor_extra_group(tmp_path: Path, settings: Settings) -> None:
    inputs, bindings = _make_inputs(tmp_path, settings, session_has_jsonl=False, slot_uid=2001)
    loop = asyncio.new_event_loop()
    try:
        worker._run_rpc_blocking(
    ...
    assert client_kwargs["extra_groups"] == ["reactor"]
...
async def test_run_rpc_arms_hard_timeout_timer(
    ...
    monkeypatch.setattr("reactor_worker.worker.threading.Timer", FakeTimer)
...
async def test_run_rpc_hard_timeout_stops_client_and_fails(
    ...
    monkeypatch.setattr("reactor_worker.worker.threading.Timer", FiringTimer)
    inputs, bindings = _make_inputs(tmp_path, settings, session_has_jsonl=False)
    loop = asyncio.new_event_loop()
    try:
    ...
    # `_closed_error` unset (reactor_rpc bug), so the worker would hang otherwise.
    assert len(fake.mark_closed_calls) == 1
    from reactor_rpc import RpcProcessExitError
...
async def test_run_rpc_cancel_hook_stops_and_marks_closed(
    ...
    an upstream reactor_rpc bug where `stop()` does not set `_closed_error`, leaving
    `_wait_for_agent_end` blocked until timeout."""
    captured: list = []
    monkeypatch.setattr("reactor_worker.worker.register_cancel_hook", lambda hook: captured.append(hook))
    monkeypatch.setattr("reactor_worker.worker.unregister_cancel_hook", lambda: None)
...
async def test_run_rpc_sends_reminder_when_pr_class_quits_early(tmp_path: Path, settings: Settings) -> None:
    ...
    try:
        ...
        loop.close()
    fake = _FakeRpcClient.instances[0]
    # kickoff + 2 reminders (default REACTOR_WORKER_TASK_COMPLETION_MAX_REMINDERS=2)
...
def _make_capture_inputs(
    ...
    *,
    ...
    if with_native_artifacts:
        ...
        (native_dir / "reactor_natives.linux-arm64.node").write_bytes(b"ELFx")
