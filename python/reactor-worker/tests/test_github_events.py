...
from reactor_worker.github_events import (
    extract_mention,
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@reactor_worker-bot please re-review",
            },
            "issue": {"number": 9, "user": {"login": "contributor"}, "pull_request": {"url": "x"}},
            "repository": {"full_name": "octo/widget"},
        },
...
def test_extract_mention_returns_body_minus_mention() -> None:
    assert extract_mention("hey @reactor_worker-bot please look", "reactor_worker-bot") == "hey please look"
...
def test_extract_mention_returns_none_without_mention() -> None:
    assert extract_mention("hello there", "reactor_worker-bot") is None
    assert extract_mention(None, "reactor_worker-bot") is None
    assert extract_mention("", "reactor_worker-bot") is None


def test_extract_mention_is_case_insensitive() -> None:
    assert extract_mention("yo @REACTOR_WORKER-BOT", "reactor_worker-bot") == "yo"


def test_extract_mention_respects_hyphen_word_boundary() -> None:
    # @reactor_worker-bot-helper must NOT match @reactor_worker-bot.
    assert extract_mention("@reactor_worker-bot-helper hi", "reactor_worker-bot") is None


def test_extract_mention_handles_multiple_occurrences() -> None:
    assert extract_mention("@reactor_worker-bot one, then @reactor_worker-bot two", "reactor_worker-bot") == "one, then two"
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@reactor_worker-bot go ahead and push",
            },
            "issue": {"number": 9},
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@reactor_worker-bot go ahead with the plan",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@reactor_worker-bot change the indentation in foo.py",
            },
    ...
    decision = route(
        "pull_request_review_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@reactor_worker-bot use a generator here",
            },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@reactor_worker-bot /model gpt /thinking low\nrefactor X",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
