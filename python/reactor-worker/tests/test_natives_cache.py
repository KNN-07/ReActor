"""Unit tests for `reactor_worker.natives_cache`.
...
from reactor_worker.natives_cache import (
    CACHE_KEY_PATHS,
    NativesCache,
    _atomic_link,
    compute_key,
...
def _populate_built_artifacts(repo_dir: Path, *, body: bytes = b"\x7fELF...native") -> Path:
    ...
    (native_dir / "reactor_natives.linux-arm64.node").write_bytes(body)
...
def test_compute_key_changes_when_each_input_changes(tmp_path: Path) -> None:
    ...
    mutations: dict[str, tuple[str, str]] = {
        "crates": ("crates/reactor-natives/src.rs", "// new comment\n"),
...
def test_compute_key_handles_missing_inputs(tmp_path: Path) -> None:
    ...
    crates = repo / "crates" / "reactor-natives"
    crates.mkdir(parents=True)
...
def test_capture_then_populate_shares_node_inode_but_copies_companions(tmp_path: Path) -> None:
    ...
    assert "reactor_natives.linux-arm64.node" in manifest["node_files"]
    ...
    assert {p.name for p in hit.files} >= {
        "reactor_natives.linux-arm64.node",
        "index.d.ts",
        "index.js",
        "embedded-addon.js",
    }
    ...
    workspace_node = dst_native / "reactor_natives.linux-arm64.node"
...
def test_capture_skips_when_artifacts_incomplete(tmp_path: Path) -> None:
    ...
    (native_dir / "reactor_natives.linux-arm64.node").write_bytes(b"x")
...
def test_populate_cross_device_falls_back_to_copy(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cached_node = cache.entry_dir(REPO, key) / "reactor_natives.linux-arm64.node"
    copied_node = dst_native / "reactor_natives.linux-arm64.node"
...
def test_populate_replaces_existing_file_atomically(tmp_path: Path) -> None:
    ...
    target = dst_native / "reactor_natives.linux-arm64.node"
...
def _stamp_entry(cache: NativesCache, repo: str, key: str, captured_at: float) -> Path:
    ...
    (entry / "reactor_natives.linux-arm64.node").write_bytes(b"x" * 1024)
    (entry / "index.d.ts").write_text("")
    (entry / "index.js").write_text("")
    (entry / "embedded-addon.js").write_text("")
    (entry / "manifest.json").write_text(
        json.dumps({"key": key, "captured_at": captured_at, "node_files": ["reactor_natives.linux-arm64.node"]})
    )
    return entry
...
def test_gc_preserves_workspace_hardlinks(tmp_path: Path) -> None:
    ...
    ws_node = tmp_path / "ws" / "reactor_natives.linux-arm64.node"
    ws_node.parent.mkdir(parents=True)
    os.link(entry / "reactor_natives.linux-arm64.node", ws_node)
    cache.gc(REPO)
...
def test_gc_drops_entry_with_missing_manifest(tmp_path: Path) -> None:
    ...
    (incomplete / "reactor_natives.linux-arm64.node").write_bytes(b"x")
    cache.gc(REPO)
