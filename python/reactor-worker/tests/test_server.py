...
from reactor_worker.config import Settings, reset_settings_cache
from reactor_worker.dashboard import tail_jsonl
from reactor_worker.db import Database, close_database, get_database, issue_key
from reactor_worker.github_client import GitHubClient
from reactor_worker.manual_triage import InvalidIssueRef, ManualTriageTimeout, await_terminal_state, parse_issue_ref
from reactor_worker.sandbox import LocalGitTransport
from reactor_worker.server import create_app
...
def test_index_serves_dashboard_html(settings: Settings) -> None:
    ...
    assert "<title>reactor_worker</title>" in resp.text
    assert 'id="app"' in resp.text
    assert 'id="reactor_worker-config"' in resp.text
    # The sentinel must have been substituted — neither the literal sentinel
    # nor an empty script body is acceptable.
    assert "__REACTOR_WORKER_CONFIG__" not in resp.text
...
def test_index_substitutes_replay_token(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("REACTOR_WORKER_REPLAY_TOKEN", "secret-token-7")
...
def test_api_status_reports_runtime_counts_and_inflight(settings: Settings) -> None:
    ...
    assert runtime["bot_login"] == "reactor_worker-bot"
...
def test_api_logs_tails_jsonl_file(settings: Settings) -> None:
    log_path = settings.log_dir / "reactor_worker.log.jsonl"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    payloads = [
        {"ts": "2026-05-14T21:28:28Z", "level": "INFO", "logger": "reactor_worker.queue", "msg": "dispatch loop online"},
        {
            "ts": "2026-05-14T21:28:54Z",
            "level": "INFO",
            "logger": "reactor_worker.server",
            "msg": "skip",
            "event": "issues",
            "reason": "issues.labeled ignored",
        },
        {"ts": "2026-05-14T21:30:00Z", "level": "WARNING", "logger": "reactor_worker.queue", "msg": "tool_end", "ok": False},
    ]
...
def test_parse_issue_ref_accepts_github_issue_urls() -> None:
    cases = (
        "https://github.com/KNN-07/ReActor/issues/1348",
        "http://github.com/KNN-07/ReActor/issues/1348",
        "github.com/KNN-07/ReActor/issues/1348",
        "https://www.github.com/KNN-07/ReActor/issues/1348",
        "https://github.com/KNN-07/ReActor/issues/1348/",
        "https://github.com/KNN-07/ReActor/issues/1348?foo=bar",
        "https://github.com/KNN-07/ReActor/issues/1348#issuecomment-99",
        "  https://github.com/KNN-07/ReActor/issues/1348  ",
    )
    for case in cases:
        assert parse_issue_ref(case) == ("KNN-07/ReActor", 1348), case
...
def _enable_replay(monkeypatch: pytest.MonkeyPatch) -> str:
    ...
    monkeypatch.setenv("REACTOR_WORKER_REPLAY_TOKEN", token)
    reset_settings_cache()
    return token
...
def rate_limited_settings(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> Settings:
    monkeypatch.setenv("REACTOR_WORKER_RATE_LIMIT_DEFAULT", "2")
    monkeypatch.setenv("REACTOR_WORKER_RATE_LIMIT_CONTRIBUTOR", "4")
    monkeypatch.setenv("REACTOR_WORKER_RATE_LIMIT_WINDOW_SECONDS", "3600")
    monkeypatch.setenv("REACTOR_WORKER_RATE_LIMIT_UNLIMITED", "can1357")
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    return cfg
...
def _allowlist(monkeypatch: pytest.MonkeyPatch, repos: str) -> None:
    monkeypatch.setenv("REACTOR_WORKER_REPO_ALLOWLIST", repos)
    reset_settings_cache()
...
def test_webhook_directive_on_unknown_issue_is_queued_with_metadata(env) -> None:
    ...
    with TestClient(app) as client:
        resp = _post_issue_comment(
            ...
            user="can1357",
            number=77,
            body="@reactor_worker-bot please refactor X",
            association="OWNER",
        )
        assert resp.status_code == 202
    ...
    assert row is not None
    assert row.state == "queued"
    directive = row.payload.get("_reactor_worker_directive")
...
def test_webhook_maintainer_bypasses_rate_limit(
    ...
    """Login in REACTOR_WORKER_MAINTAINER_LOGINS is always unlimited, even with NONE association."""
    monkeypatch.setenv("REACTOR_WORKER_MAINTAINER_LOGINS", "can1357")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    ...
    with TestClient(app) as client:
        ...
        for i in range(4):
            resp = _post_issue_comment(
                ...
                user="can1357",
                number=300 + i,
                body=("@reactor_worker-bot do X" if i == 3 else "comment"),
                association="NONE",
            )
            assert resp.status_code == 202
            states.append(resp.json()["state"])
    ...
    assert directive_event.payload.get("_reactor_worker_directive") == {
        "body": "do X",
        "author": "can1357",
        "pragmas": [],
...
def stub_run_task(monkeypatch: pytest.MonkeyPatch) -> list[dict]:
    ...
    from reactor_worker import tasks as tasks_module
...
async def test_handle_pr_conversation_unmapped_bot_pr_uses_pr_branch(
    ...
    from reactor_worker import tasks
    from reactor_worker.github_client import CommentInfo, IssueInfo, PullRequestInfo, RepoInfo
...
async def test_handle_pr_conversation_skips_review_workspace_rows(
    ...
    from reactor_worker import tasks
    from reactor_worker.github_client import GitHubClient, PullRequestInfo
    ...
    payload = {
        ...
        "comment": {"user": {"login": "can1357"}, "body": "@reactor_worker-bot please re-review", "id": 12},
        "repository": {"full_name": "octo/widget"},
        "_reactor_worker_directive": {"body": "please re-review", "author": "can1357"},
    }
    await tasks.handle_pr_conversation(
        settings=settings,
        db=db,
...
async def test_review_pr_retries_when_ranked_but_not_submitted(
    ...
    from reactor_worker import tasks
    from reactor_worker.github_client import GitHubClient, IssueInfo, PullRequestInfo, RepoInfo
...
async def test_handle_comment_directive_bootstraps_untriaged_issue(
    ...
    from reactor_worker import tasks
    from reactor_worker.github_client import GitHubClient, IssueInfo, RepoInfo
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_reactor_worker_directive": {"body": "please refactor X", "author": "can1357"},
    }
...
async def test_handle_comment_directive_reopens_finalized_issue(
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_reactor_worker_directive": {"body": "redo the fix", "author": "can1357"},
    }
    await tasks.handle_comment(
        settings=settings,
        db=db,
...
async def test_directive_handler_attaches_thread_from_github(
    ...
    from reactor_worker import tasks
    from reactor_worker.github_client import (
        CommentInfo,
        GitHubClient,
        IssueInfo,
        RepoInfo,
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_reactor_worker_directive": {"body": "do X", "author": "can1357"},
    }
...
async def test_triage_issue_skips_when_a_closing_pr_already_exists(
    ...
    from reactor_worker import tasks
    from reactor_worker.github_client import IssueInfo, RepoInfo
...
async def test_triage_issue_fails_open_when_timeline_fetch_errors(
    ...
    from reactor_worker import tasks
    from reactor_worker.github_client import GitHubError, IssueInfo, RepoInfo
