...
each one names exactly one operation reactor_worker performs.
"""
...
from reactor_worker.github_client import GitHubClient, GitHubError
from reactor_worker.proxy_hmac import HEADER_SIGNATURE, HEADER_TIMESTAMP, verify
from reactor_worker.sandbox import _safe_directory_env, _slot_subprocess_kwargs
from reactor_worker.sandbox import workspace_key as compute_workspace_key
...
def _resolve_hmac_key(cfg: Settings) -> bytes:
    if cfg.gh_proxy_hmac_key is None:
        raise HTTPException(500, "gh-proxy: REACTOR_WORKER_GH_PROXY_HMAC_KEY not configured")
...
def create_proxy_app(settings: Settings) -> FastAPI:
    ...
    app = FastAPI(title="reactor_worker-gh-proxy", version="0.1.0", lifespan=lifespan)
    ...
    # ---- git transport ----
    #
    # The underlying `reactor_worker.git_ops` primitives are blocking `subprocess.run`
