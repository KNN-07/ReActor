...
The napi-rs build of ``reactor_natives.<platform>-<arch>[-variant].node`` takes
...
Ownership: cache root is provisioned ``root:reactor 02770`` by ``entrypoint.sh``
so slot subprocesses (group ``reactor``) can capture under setgid inheritance.
...
# napi build reads: all workspace crates (reactor-natives transitively depends on
# reactor-ast/reactor-iso/reactor-shell), the workspace Cargo manifest + lock, the rust
...
_CACHED_NODE_GLOB = "reactor_natives.*.node"
...
def _git_safe_directory_env(repo_dir: Path) -> dict[str, str]:
    ...
    ``reactor_worker.sandbox._safe_directory_env`` but kept local to avoid a circular
...
class NativesCache:
    """Per-repo content-addressed cache of reactor-natives build outputs."""
        ...
        *,
        ...
        with _flock(self.lockfile(repo)):
            ...
            try:
                ...
                # the setgid `reactor` model that lets other slots read it.
                # A copy creates a fresh inode owned by the orchestrator
                # (root) and inherits gid `reactor` from the setgid 2770
                # cache root.
