...
from reactor_worker.db import issue_key
from reactor_worker.pragmas import parse_pragmas
...
def extract_mention(body: str | None, bot_login: str) -> str | None:
    ...
    part of the token, so `@reactor_worker-bot` does NOT match `@reactor_worker-bot-extra`).
    """
    ...
    *,
    ...
    if event_type == "pull_request" and action in ("opened", "reopened", "ready_for_review"):
        ...
        if pr_review_trigger == "vouched_label":
            # Defer to the vouch gate. reactor_worker reviews ONLY on the `labeled`
