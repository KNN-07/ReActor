...
from reactor_worker.sandbox import Workspace
...
def _load(name: str) -> str:
    return resources.files("reactor_worker.prompts").joinpath(name).read_text(encoding="utf-8")
...
def resume_triage(*, repo: RepoInfo, issue: IssueInfo, workspace: Workspace) -> str:
    """Resume prompt for a `triage_issue` task whose reactor session already exists."""
