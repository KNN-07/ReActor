...
from reactor_worker.db import Database, IssueRow, IssueState, issue_key
...
from reactor_worker.sandbox import GitTransport, SandboxManager
from reactor_worker.worker import DirectiveInfo, TaskInputs, ThreadMessage, run_task
...
def _directive_from_payload(payload: Mapping[str, Any]) -> DirectiveInfo | None:
    ...
    raw = payload.get("_reactor_worker_directive")
...
async def triage_issue(
    *,
    ...
    if db.get_issue(key) is None:
        ...
        # resources (workspace, reactor session) to this issue.
        try:
...
async def handle_pr_conversation(
    *,
    ...
    # _reactor_worker_directive; _directive_from_payload rejects it but the key
    # being present tells us a mention happened. Reply cheaply without reactor.
    if directive is None and payload.get("_reactor_worker_directive") is not None:
        comment = _comment_from_payload(payload)
        log.info(
