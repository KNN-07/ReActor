...
```
@reactor_worker-bot /model gpt /thinking low
...
- `/model <alias>` — pick the first id in `REACTOR_WORKER_MODEL` whose model id
  contains `<alias>` (case-insensitive). Falls back to the normal random
  pool selection if no member matches.
- `/thinking <level>` — override `REACTOR_WORKER_THINKING` for this run. Accepts
