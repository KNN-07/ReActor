...
from reactor_worker import host_tools, persona, pragmas
from reactor_worker.cancellation import register_cancel_hook, unregister_cancel_hook
from reactor_worker.config import Settings
from reactor_worker.db import Database, issue_key
from reactor_worker.git_ops import DirtyState, inspect_dirty_state
...
from reactor_worker.host_tools import AbortController, ToolBindings, _git_identity_env
...
from reactor_worker.sandbox import GitTransport, Workspace, _prepare_slot_runtime_env, _safe_directory_env
...
_SCRUBBED_ENV_KEYS: tuple[str, ...] = (
    ...
    "REACTOR_WORKER_GH_PROXY_HMAC_KEY",
)
...
def _stage_agent_home() -> None:
    ...
    for rel in (Path(".agent"), Path(".reactor/agent")):
    ...
    for root, dirs, files in os.walk(_AGENT_HOME):
        root_path = Path(root)
        if root_path == _AGENT_HOME / ".reactor":
            # ~/.reactor/run is slot-writable daemon presence state, not template
...
def _ensure_agent_run_dir() -> None:
    """Keep ``~/.reactor/run`` writable by every sandbox slot.

    reactor registers daemon project presence under ``~/.reactor/run`` at startup,
    ...
    The tree stays group ``reactor``, setgid, group-writable; slot subprocesses
    spawn with umask 0002 so their entries inherit group write.
    """
    if os.geteuid() != 0:
        return
    run_dir = _AGENT_HOME / ".reactor" / "run"
    try:
        gid = grp.getgrnam("reactor").gr_gid
    except KeyError:
        return
    try:
...
def _build_extra_env(settings: Settings) -> dict[str, str]:
    """Build the env overlay passed to the reactor subprocess.

    `reactor_rpc` merges this dict on top of `os.environ`, so overlaying empty
...
def _run_rpc_blocking(
    ...
    *,
    ...
    with RpcClient(
        executable=settings.reactor_command,
        ...
        extra_groups=["reactor"] if inputs.slot_uid is not None else None,
    ) as client:
        # Arm cancellation: from this point the API can kill the reactor subprocess
        ...
        # it's idempotent.
        #
        # NOTE: reactor_rpc.RpcClient.stop() has a bug where it sets `_stopping=True`
        ...
        try:
            ...
            if hard_timeout_fired.is_set():
                raise TimeoutError("reactor task exceeded hard timeout")
            if turn is not None and turn.assistant_message is not None:
                ...
                if stop_reason == "error":
                    ...
                    raise RuntimeError(f"reactor agent error (stopReason=error): {error_msg}")
            log.info(
                "rpc_done",
                extra={
                    "issue": bindings.issue_key,
