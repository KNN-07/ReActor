"""Host tools exposed to the agent through `reactor_rpc.host_tool`.
...
from reactor_rpc import HostTool, HostToolContext, RpcCommandError, host_tool

from reactor_worker import persona
from reactor_worker.config import Settings
from reactor_worker.db import Database, IssueState, issue_key
from reactor_worker.git_ops import GitCommandError, HeadDriftError
from reactor_worker.github_backend import GitHubBackend
from reactor_worker.github_client import GitHubError, IssueInfo, PullRequestFileInfo, RepoInfo
from reactor_worker.issue_index import parse_search_query
...
class AbortController:
    ...
    teardown of the reactor subprocess. The worker pre-populates `stop` with a
...
class ToolBindings:
    ...
    # Set by the worker before launching reactor. Carries the abort-task signal
...
def _repair_commit_message_escapes(bindings: ToolBindings, args: Mapping[str, Any], *, tool_name: str) -> None:
    ...
    update = _run_repo_command(
        bindings,
        ["git", "update-ref", "-m", "reactor_worker: repaired commit message escapes", "HEAD", new_head, old_head],
    )
...
def _build_abort_task(bindings: ToolBindings) -> HostTool[Any, Any]:
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        ...
        # races against the imminent reactor teardown.
