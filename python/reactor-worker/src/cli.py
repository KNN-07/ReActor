...
from reactor_worker.db import INACTIVE_EVENT_STATES, get_database
from reactor_worker.logging_config import configure_logging
...
from reactor_worker.proxy_client import GitHubProxyClient
...
def triage(issue_ref: str, wait_timeout: float | None) -> None:
    ...
    if not cfg.allows(repo_full):
        click.echo(f"refusing: {repo_full} not in REACTOR_WORKER_REPO_ALLOWLIST", err=True)
        sys.exit(2)
