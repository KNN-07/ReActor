...
from reactor_worker import github_events, issue_index
from reactor_worker.autoclose import AutocloseScheduler
from reactor_worker.config import Settings, get_settings
from reactor_worker.dashboard import render_index, static_dir, tail_jsonl
from reactor_worker.db import (
...
from reactor_worker.github_client import GitHubError, IssueSummary
from reactor_worker.issue_index import IssueIndexSync
from reactor_worker.manual_triage import (
    InvalidIssueRef,
...
from reactor_worker.proxy_client import GitHubProxyClient, ProxyGitTransport
from reactor_worker.queue import WorkerPool
...
def _require_proxy_mode(cfg: Settings) -> tuple[str, bytes]:
    if cfg.github_token is not None:
        raise SystemExit(
            "reactor_worker orchestrator refuses to start with GITHUB_TOKEN set in env. "
            "The PAT must live only in the gh-proxy container."
        )
    if cfg.gh_proxy_url is None or cfg.gh_proxy_hmac_key is None:
        raise SystemExit(
            "reactor_worker orchestrator requires REACTOR_WORKER_GH_PROXY_URL and "
            "REACTOR_WORKER_GH_PROXY_HMAC_KEY (run gh-proxy in a sibling container)."
        )
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="reactor_worker", version="0.1.0", lifespan=lifespan)
    ...
    async def webhook(
        ...
        if decision.directive:
            ...
            payload["_reactor_worker_directive"] = {
    ...
    async def replay(
        ...
        if x_reactor_worker_token != cfg.replay_token.get_secret_value():
    ...
    def _require_trigger_token(cfg: Settings, token: str | None) -> None:
        if cfg.replay_token is None:
            raise HTTPException(404, "trigger disabled (set REACTOR_WORKER_REPLAY_TOKEN to enable)")
    ...
    async def api_github_issues(
        ...
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        _require_trigger_token(cfg, x_reactor_worker_token)
    ...
    async def api_trigger(
        ...
        if mode == "triage":
            ...
            if not cfg.allows(repo_full):
                raise HTTPException(403, f"{repo_full} not in REACTOR_WORKER_REPO_ALLOWLIST")
            try:
    ...
    async def api_logs(request: Request, limit: int = 400) -> dict[str, Any]:
        ...
        entries = tail_jsonl(cfg.log_dir / "reactor_worker.log.jsonl", limit=capped)
