...
PAT injection via `--config-env` (see `reactor_worker.git_ops`); the `ProxyGitTransport`
in `reactor_worker.proxy_client` forwards the same set of operations over HMAC RPC.
...
1. **Workspace tree** (`/data/workspaces/<key>/`, including `repo/`,
   `.reactor-session/`, `context/`, `artifacts/`, `.reactor-tmp/`, `.reactor-xdg`):
   single-owner. Owned by the active slot UID/GID (`reactor-N`) when slot
...
2. **Clone pool** (`/data/workspaces/_pool/<owner>__<repo>/`): genuinely
   multi-slot. Owned by `root:reactor` (gid 2000) with setgid `02770`; cross-slot
...
3. **Language tool caches** (`/data/cache/{cargo,cargo-target,rustup,bun-cache}`):
   multi-slot. Owned by `root:reactor` with setgid `02770`; provisioned by
   `entrypoint.sh`.
...
from reactor_worker.natives_cache import CacheHit, NativesCache
...
class GitTransport(Protocol):
    ...
    - `reactor_worker.proxy_client.ProxyGitTransport`: forwards over HMAC RPC.
    """
...
def _prepare_slot_tmpdir(workspace: Workspace, slot_uid: int | None) -> Path:
    ...
    - replaces any non-directory at ``.reactor-tmp`` (symlink-protection: a user
    ...
    tmpdir = workspace.root / ".reactor-tmp"
    try:
        st = tmpdir.lstat()
...
def _slot_subprocess_kwargs(slot_uid: int | None) -> dict[str, Any]:
    ...
    if not _slot_permissions_active(slot_uid):
        return {}
    assert slot_uid is not None
    return {"user": slot_uid, "group": slot_uid, "extra_groups": [_SHARED_REACTOR_GID], "umask": 0o002}
...
def _prepare_slot_runtime_env(workspace: Workspace, slot_uid: int | None) -> dict[str, str]:
    ...
    and are group-shared via ``reactor``. Bun's install cache is explicitly
    ...
    xdg_root = workspace.root / ".reactor-xdg"
    ...
    for base in (xdg_data, xdg_state, xdg_cache):
        ...
        (base / "reactor").mkdir(parents=True, exist_ok=True)
...
def _provision_runtime_dirs(ws_root: Path) -> None:
    ...
    picks up ``.reactor-tmp`` and the per-workspace XDG tree. Without this,
    ...
    Symlink-safe on ``.reactor-tmp`` (replaces a planted non-directory in place).
    """
    tmpdir = ws_root / ".reactor-tmp"
    try:
        st = tmpdir.lstat()
    except FileNotFoundError:
        pass
...
def _share_git_metadata_with_slots(repo_dir: Path, slot_uid: int | None) -> None:
    ...
    `reactor-N` user, so the pool-side worktree gitdir, refs, reflogs, and object
    directories must stay writable through the shared `reactor` group.
    """
    if not _slot_permissions_active(slot_uid):
        return
...
class SandboxManager:
    ...
    def ensure_workspace(
        ...
        *,
        ...
        with self._repo_lock(repo):
            ...
            session_dir = ws_root / ".reactor-session"
            ...
            # keeps its `root:reactor` ownership (the slot reads through group `reactor`);
    ...
    def _populate_natives_cache(self, workspace: Workspace, *, slot_uid: int | None = None) -> None:
        """Try to hardlink cached reactor-natives artifacts into the worktree.
        ...
        `.node` files are LEFT at `root:reactor` ownership — chowning them
        would chown the cache file too (shared inode), breaking the
        cross-slot sharing model. The slot reads them via group `reactor`.
        """
    ...
    def _chown_natives_for_slot(native_dir: Path, hit: CacheHit, *, slot_uid: int) -> None:
        ...
        hardlinks back into the root:reactor cache and the slot reads them via
        group `reactor`. Everything else (the directory itself, copied
