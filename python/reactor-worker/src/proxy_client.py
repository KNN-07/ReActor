"""Client half of the reactor-worker ↔ gh-proxy channel.
...
routing clone/fetch/push through the proxy too — reactor-worker never holds the PAT.
...
from reactor_worker.git_ops import GitCommandError, HeadDriftError, PushResult
...
class GitHubProxyClient:
    """HMAC-signed REST client speaking to a `reactor_worker.proxy.server` instance.
