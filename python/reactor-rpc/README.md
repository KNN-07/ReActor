...
Typed Python bindings for the `reactor --mode rpc` protocol used by the coding agent.
...
- typed startup options for common `reactor --mode rpc` flags such as thinking level,
...
```python
from reactor_rpc import RpcClient
...
```python
from reactor_rpc import MessageUpdateEvent, RpcClient
...
from reactor_rpc import RpcClient, host_tool
...
```python
from reactor_rpc import RpcClient, host_uri
...
```python
from reactor_rpc import assistant_text, assistant_text_with_thinking
