...
def _terminate_process_group(process: subprocess.Popen[Any], pgid: int | None) -> None:
    ...
    reactor is spawned with `start_new_session=True`, so it leads a session/group
...
class RpcClient:
    ...
    def start(self) -> RpcClient:
        ...
        self._stdout_thread = threading.Thread(
            target=self._read_stdout_loop, name="reactor-rpc-stdout", daemon=True
        )
        self._stderr_thread = threading.Thread(
            target=self._read_stderr_loop, name="reactor-rpc-stderr", daemon=True
        )
    ...
    def _handle_host_tool_call(self, payload: JsonObject) -> None:
        ...
        threading.Thread(
            target=run_tool, name=f"reactor-rpc-host-tool:{tool_name}", daemon=True
        ).start()
    ...
    def _handle_host_uri_request(self, payload: JsonObject) -> None:
        ...
        threading.Thread(
            target=run, name=f"reactor-rpc-host-uri:{scheme}:{operation}", daemon=True
        ).start()
