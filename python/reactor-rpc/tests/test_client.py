...
from reactor_rpc import RpcClient, RpcCommandError, RpcConcurrencyError, RpcError, host_tool
...
class TerminatesProcessGroupTests(unittest.TestCase):
    ...
    the reactor process. Before the fix, stop() signalled only the leader pid, so
    such grandchildren reparented to the container init and kept running —
    once ballooning to tens of GB of RAM. reactor is now spawned in its own
    ...
    def test_stop_kills_grandchild_spawned_by_server(self) -> None:
        ...
        # Fake reactor server: spawn the long-lived grandchild, signal ready, then
